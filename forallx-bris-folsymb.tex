...
\part{First-order logic symbolisation}
\label{ch:FOL}
\addtocontents{toc}{\protect\mbox{}\protect\hrulefill\par}


	\chapter{Common quantifier phrases}
	\label{sec:SymbolisingComplexFOL}
	We have already seem the key idea of FOL symbolisation in \cref{sec:FOLBuildingBlocks}. Our strategy for symbolising sentences extends the strategy that we used for TFL in \cref{sec:SymbolisingTFL - simple connectives complex form}
	...
	\begin{highlighted}
		\begin{center}
			\begin{tabular}{ll}
				...
				\hline
				For all $x$, \ldots &$\forall x \ldots$\\
				There exists some $x$ such that\ldots &$\exists x \ldots$\\
	...
	What we will now do is discuss further symbolisation tricks to add to our symbolisation guide. 
	
	\section{Restricted quantifiers}
	We have already discussed two of them: 
	...
	So we symbolise 
		\begin{samplesent}
		\item Every dog is energetic. 
	\end{samplesent}
	as $$\forall x(Dx\eif Ex)$$
	and 
		\begin{samplesent}
		\item Some dog is energetic. 
	\end{samplesent}
	as $$\exists x(Dx\eand Ex)$$
	
		Notice that we needed to use a conditional with the universal quantifier, but we used a conjunction with the existential quantifier. 
		
		Suppose we had instead written $\exists x(Dx\eif Ex)$. So long as there is a non-dog, $o$, in the model, then $Dx\eif Ex$ is true under $x\mapsto o$. After all, a conditional with a false antecedent is always true. So it would be far too easy to make $\exists x(Dx\eif Ex)$ true: it would be true in any model which contains a non-dog. 
		
		Similarly, suppose we had instead written $\forall x(Dx\eand Ex)$. So long as there is a non-dog, $o$, in the model, then $Dx\eand Ex$ is false under $x\mapsto o$. So it would be far too hard to make $\exists x (Dx\eand Ex)$ true. It would be false in any model which contains non-dogs. 
		
		
		
		Recall that this is a general strategy which can also be applied with more complex sentences
		\begin{samplesent}
		...
		\begin{center}
			...
			\end{forest}
		\end{center}$$\forall x((Ex\eand Sx)\eif\enot Tx)$$		
		or which can be iterated
		\begin{samplesent}
		...
		These are just new tools which we add to our symbolisation toolbox. 
		
		Now, we will provide some additional  symbolisation guidance.


	
%	And we can add to our symbolisation guide a useful pattern with quantifiers in FOL: 
%	\factoidbox{
%		If a sentence can be paraphrased in English as \begin{center}
%			\begin{tabular}{l}`every F is G',\\`all Fs are Gs', or\\ `any F is a G',
%			\end{tabular}
...
%	\begin{center}
%		\begin{forest}
%			[Some dog likes playing with Finley\\There is some thing $x$ such that \ldots\\${\exists x}\ldots$
%			[$x$ is a dog who likes playing with Finley\\ \ldots and \ldots\\$\ldots {\eand} \ldots$
%			[$x$ is a dog\\$Dx$]
...
%		\section{Other common quantifier phrases}
	
	
	\section{None}
	\begin{samplesent}
		\item No dog is energetic
	\end{samplesent}
	We have to paraphrase this sentence using the operators that we have. There are two different ways to do this:
		\begin{samplesent}
		\item Every dog is not energetic.
		\item It is not the case that there exists an energetic dog.
	\end{samplesent}
	These different paraphrasing will give different symbolisations
	
	
	\begin{center}
		\begin{forest}
			[No dog is energetic\\Every dog is not energetic\\for all $x$: if\ldots then \ldots\\
			$\forall x(\ldots\eif\ldots)$
			[$x$ is a dog\\$Dx$]
			[$x$ is not energetic\\$\enot Ex$]
			]
		\end{forest}
	\end{center}
	Giving $$\forall x(Dx\eif \enot Ex)$$
	
		\begin{center}
		\begin{forest}
			[No dog is energetic\\It is not the case that\ldots\\$\enot\ldots$
			[there exists an energetic dog\\$\exists x(\ldots\eand \ldots)$
			[$x$ is a dog\\$Dx$]
			[$x$ is energetic\\$Ex$]
			]
			]
		\end{forest}
	\end{center}
	Giving $$\enot\exists  x(Dx\eand Ex)$$
	
	These two sentences are logically equivalent and both appropriate symbolisations. 
		
		We give an example of semantic reasoning to show that they are logically equivalent. 
	\begin{quote}
	Observe that in any model, $\model$, and any variable assignment, $\alpha$,
	\begin{align*}
		&Dx\eand Ex \text{ is true in } \model \text{ under } \alpha\\
		&\text{ iff both } Dx \text{ and } Ex \text{ are true in } \model \text{ under } \alpha\\
		&\text{ iff $Dx$ is true in $\model$ under $\alpha$ and $Ex$ is false in $\model$ under $\alpha$}\\
		&\text{ iff } Dx\eif\enot Ex \text{ is false in } \model \text{ under } \alpha.
	\end{align*}
	Then we can reason as follows:
	\begin{align*}
		&\enot\exists x(Dx\eand Ex) \text{ is true in } \model\\
		&\text{ iff there is no } o\in\domain_\model \\
		&\qquad \text{ such that } Dx\eand Ex \text{ is true in } \model \text{ under } x\mapsto o \\
		&\text{ iff there is no } o\in\domain_\model \\
		&\qquad \text{ such that } Dx\eif\enot Ex \text{ is false in } \model \text{ under } x\mapsto o \\
		&\text{ iff for every } o\in\domain_\model,\ Dx\eif\enot Ex \text{ is true in } \model \text{ under } x\mapsto o \\
		&\text{ iff } \forall x(Dx\eif\enot Ex) \text{ is true in } \model.
	\end{align*}
	\end{quote}
	...
	This can also be embedded in more complex sentences
	\begin{samplesent}
		\item No cat owner likes every dog.
	\end{samplesent}

	\begin{center}
	\begin{forest}
		[No cat owner likes every dog.\\It is not the case that there exists some $x$ which \ldots and \ldots\\
		$\enot\exists x(\ldots\eand\ldots)$
		[$x$ is a cat owner\\ $x$ owns a cat\\$\exists y(Cy\eand Oxy)$]
		[$x$ likes every dog\\$\forall y(Dy\eif Lxy)$]]
		]
	\end{forest}
\end{center}offering:
	$$\enot \exists x(\exists y(Cy\eand Oxy)\eand\forall y(Dy\eif Lxy))$$
	Alternatively:
	\begin{center}
		\begin{forest}
			[No cat owner likes every dog.\\For all $x$: if \ldots then not \ldots\\
			$\forall x(\ldots\eif\enot\ldots)$
			[$x$ is a cat owner\\$\exists y(Cy\eand Oxy)$]
			[$x$ likes every dog\\$\forall y(Dy\eif Lxy)$]]
			]
		\end{forest}
	\end{center}
	offering: $$\forall x(\exists y(Cy\eand Oxy)\eif \enot\forall y(Dy\eif Lxy))$$


	
		\section{Not all}
		
		There is one final structure to complete our selection: we have $\forall$, $\exists$, $\enot\exists = \forall\enot$; the final structure to see is $\enot\forall = \exists \enot$. 
		

		

	\begin{samplesent}
		\item\label{sent:quan3} Not all dogs are enthusiastic. 
	\end{samplesent}
	$$\enot\forall x(Dx\eif Ex)$$
	
	In fact, this is logically equivalent to $\exists x(Dx\eand\enot Ex)$: \begin{samplesent}
	\item Some dog is not enthusiastic.
\end{samplesent} 

We have resulted in four things we might say about whether dogs are enthusiastic, or, more generally, whether Fs are Gs. 

	\begin{center}
		\begin{tikzpicture}[
			squareopp/.style={draw, rounded corners, align=center, inner sep=6pt, text width=.3\linewidth},
			squareoppline/.style={-Stealth,shorten >=4pt, shorten <=4pt},
			squareopplineboth/.style={<->,shorten >=2pt, shorten <=2pt},
			squareopplabel/.style={inner sep=1pt}
			]
			\node[squareopp] (A) at (0,3.2) {All Fs are Gs\\$\forall x(\meta{F}x\eif \meta{G}x)$};
					\node[above=0cm of A]{$=\enot\exists x(\meta{F}x\eand \enot\meta{G}x)$};
			\node[squareopp] (E) at (7,3.2) {No Fs are Gs\\$\enot\exists x(\meta{F}x\eand \meta{G}x)$\\$\forall x(\meta{F}x\eif\enot\meta{G}x))$};
			\node[squareopp] (I) at (0,0) {Some Fs are Gs\\$\exists x(\meta{F}x\eand \meta{G}x)$};
					\node[below=0cm of I]{$=\enot\forall x(\meta{F}x\eif \enot\meta{G}x)$};
			\node[squareopp] (O) at (7,0) {Not all Fs are Gs\\$\enot\forall x(\meta{F}x\eif\meta{G}x)$\\Some Fs are non-Gs\\$\exists x(\meta{F}x\eand \enot\meta{G}x)$};
			

				\draw[squareoppline] (A) -- node[squareopplabel, sloped, above, pos=.3]{not:} (O);
				\draw[squareoppline] (I) -- node[squareopplabel, sloped, below, pos=.5]{not:} (E);
				
				\draw[squareoppline,blue] (A) -- node[squareopplabel, left]{log conseq,\\assuming $\exists xFx$} (I);
				\draw[squareoppline,blue] (E) -- node[squareopplabel, right]{log conseq,\\assuming $\exists xFx$} (O);
				\draw[squareoppline,blue] (A) -- node[squareopplabel, above]{inconsistent,\\assuming $\exists xFx$} (E);
%			\draw[squareoppline,blue] (I) -- node[squareopplabel, below]{inconsistent,\\assuming $\exists xFx$} (O);
			\end{tikzpicture}
	\end{center}
	...
	\section{Only}\label{sec:SymboliseFOLOnly}

	
		\begin{samplesent}
		\item Only dogs are enthusiastic
	\end{samplesent}
	We will paraphrase this as:
	\begin{samplesent}
		\item Everything which is enthusiastic is a dog.
		\item Nothing which is enthusiastic is non-dog.
	\end{samplesent}
	
	We can offer symbolisation options:
	\begin{gather*}
		\forall x(Ex\eif Dx)\\
		\enot\exists x(Ex\eand \enot Dx)
	\end{gather*}
	
Compare this to:
			\begin{samplesent}
		\item All dogs are enthusiastic
	\end{samplesent}
	is symbolised as 
		\begin{gather*}
		\forall x(Dx\eif Ex)\\
		\enot\exists x(Dx\eand \enot Ex)
	\end{gather*}
	It can only be made false due to a dog who is not enthusiastic
	\begin{samplesent}
		\item Only dogs are enthusiastic
	\end{samplesent}
		can only be made false due to a non-dog who is enthusiastic
	...
	\item[Cx]\gap{x} is a candy
				\item[\atom{H}{x}] \gap{x} has chocolate in it.
...
%	
	...
	%with subscripts, as needed: $A_1, Z_2,A_{25},J_{375},\ldots$.\footnote{Each predicate will have a number of places associated with it. We should thus really introduce:\begin{description}
			...
			%\end{practiceproblems}
%			
%				\chapter{Choosing a symbolisation}
%			
%			\section{How to split a predicate}
%			
...
%			\end{samplesent}
%			
%			Should this be simply symbolised as $Ka$ using a predicate 
%				\begin{ekey}
%					\item[Kx] \gap{x} is a skilled surgeon				
%				\end{ekey}
%				
%				Or should we split it up into 
			
%			As in \cref{sec:SymbTruthConds}, we should not understand it as 
%						\begin{samplesent}
%				\item\label{sent:surgeon1} Adina is skilled and Adina is a surgeon.
%			\end{samplesent}
%		Instead, we would rather use a single predicate, $Kx$, for `$x$ is a skilled surgeon', and let $a$ mean Adina. \Cref{sent:surgeon1} is simply $Ka$.
%			
%			
%			Suppose instead that we want to symbolize this argument:
%			\begin{quote}
%				The hospital will only hire a surgeon if they are skilled. All surgeons are greedy. Billy is a surgeon, but is not skilled. Therefore, Billy is greedy, but the hospital will not hire him.
%			\end{quote}
...
%			\begin{earg}
%				\label{surgeon2}
%				\prem $\forall x\bigl(Hx\eif  (Rx \eand Kx)\bigr)$
...
%			
			...
			\end{samplesent}
			If we think of this sentence as of the form `every $F$ is not~$G$' where $F{x}$ symbolizes `\gap{x} glitters' and ${G}{x}$ is `\gap{x} is gold', we would symbolize it as:
			\begin{itemize}
			...
			\section{Ambiguous quantifier order}\todo{sections repeated here!}
		In English, the order of quantifiers is sometimes not completely determined.  When both universal (`all') and existential (`some', `a') quantifiers are  involved, this can result in scope ambiguities. Consider:
			\begin{samplesent}
			...
			To see that these are not logically equivalent consider this model:
			\begin{interp}
				\item[\domain] $\{1,2,3,4\}$
				\item[\denote{P}] $\{1,2\}$
				\item[\denote{M}] $\{3,4\}$
				\item[\denote{W}] $\{\langle 1,3\rangle,\langle 2,4\rangle\}$
			\end{interp}
			The premise is true (person $1$ went to see movie $3$, person $2$ went to see movie $4$), but the conclusion is false (no single movie was seen by both people).
...
%				\section{The order of the quantifiers}\label{sec:MultipleGenerality}
...
%			\Cref{sent:loveconverge} is symbolized by $\exists y \forall x Lxy$. \Cref{sent:loveconverge} is \emph{not} true in the situation just described. Again, suppose that our domain of discourse is restricted to Imre, Juan and Karl. This requires that all of Juan, Imre and Karl converge on (at least) one object of love.
			...
			\section{Indefinite article used as universal quantifier}
		
			Consider
			\begin{samplesent}
				\item A lion is dangerous.
			\end{samplesent}
			Despite starting with `a', where we might think we need to symbolise with `exists', instead the most natural paraphrase for this sentence is: 
			\begin{samplesent}
				\item Every lion is dangerous.
			\end{samplesent} 
			which we would symbolise as $\forall x(Lx\eif Dx)$
			Sometimes the English indefinite article, `a', is used generically, which we best capture with `for all'. 

		
			This can create ambiguity, where context isn't clear enough to determine how to symbolise it. 
			\begin{samplesent}
				\item A philosopher is happy. 
			\end{samplesent}
			Should it be understood to say that every philosopher is happy, $\forall x(Px\eif Hx)$ or that there exists a philosopher who is happy, $\exists x(Px\eand Hx)$? It might be ambiguous. 
			...
			\section{Ambiguity with `only'}
			
			In \cref{sec:SymboliseFOLOnly} we discussed how to symbolize sentences involving `only'. Consider the sentence:
			\begin{samplesent}
...
%			
%			
%			\chapter{Empty predicates and non-referring terms}
