...
We have resulted in four things we might say about whether dogs are enthusiastic, or, more generally, whether Fs are Gs. 
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\node[squareopp] (O) at (7,0) {Not all Fs are Gs\\$\enot\forall x(\meta{F}x\eif\meta{G}x)$\\Some Fs are not Gs\\$\exists x(\meta{F}x\eand \enot\meta{G}x)$};
			

				\draw[squareoppline] (A) -- node[squareopplabel, sloped, above, pos=.3]{negation} (O);
				\draw[squareoppline] (I) -- node[squareopplabel, sloped, below, pos=.5]{negation} (E);
...
%			\draw[squareoppline,blue] (I) -- node[squareopplabel, below]{inconsistent,\\assuming $\exists xFx$} (O);
	...
	\begin{samplesent}
		\item Everything on this table is a dime. 
		\item Nothing on this table is a non-dime.
	\end{samplesent}
