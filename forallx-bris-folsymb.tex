...
\addtocontents{toc}{\protect\mbox{}\protect\hrulefill\par}
	...
	\chapter{FOL symbolisation tips}
...
%	
	...
	It can be paraphrased in two different ways: 
		\begin{samplesent}
	...
	\begin{center}
		\begin{forest}
			[No dog is energetic\\for all $x$: if\ldots then \ldots\\
			$\forall x(\ldots\eif\ldots)$
			[$x$ is a dog\\$Dx$]
	...
	This is because, on any variable assignment, $x\mapsto o$, $Dx\eand Ex$ is true iff $Dx\eif\enot Ex$ is false.  
	Then we can reason as follows:
	\begin{center}
		\begin{tabular}{@{}r@{\ }l@{}}
			&$\enot\exists x(Dx\eand Ex)$ is true in a model $\model$ \\
			iff & there is no $o\in\domain_\model$ such that $Dx\eand Ex$ is true under $x\mapsto o$ \\
			iff & there is no $o\in\domain_\model$ such that $Dx\eif\enot Ex$ is false under $x\mapsto o$ \\
			iff & for every $o\in\domain_\model$, $Dx\eif\enot Ex$ is true under $x\mapsto o$ \\
			iff & $\forall x(Dx\eif\enot Ex)$ is true in $\model$.
		\end{tabular}
	\end{center}
...
%	\chapter{Choosing a symbolisation key}
