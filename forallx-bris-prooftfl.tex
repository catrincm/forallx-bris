...
First `$\metaX$' and `\metaY' are metavariables. They are not particular sentences of TFL but are there to play the role of any particular sentence (see \cref{sec:UseMention}).
...
\begin{fitchproof}
	\hypo{ab}{{[}(A\eor B)\eif(C\eor D){]} \eand \enot(E\eor F)}
	\have{a}{{[}(A\eor B)\eif(C\eor D){]}} \andE{ab}
	\have{b}{\enot(E\eor F)} \andE{ab}
\end{fitchproof}
...
\begin{fitchproof}
	...
	\have{a}{{[}(A\eor B)\eif(C\eor D){]}} \andE{ab}
	\have{b}{\enot(E\eor F)} \andE{ab}
	\have{ba}{\enot(E\eor F) \eand {[}(A\eor B)\eif(C\eor D){]}} \andI{b,a}
\end{fitchproof}
...
It is worth giving another example. Way back in \cref{sec:MoreBracketingConventions}, we noted that this argument is valid:
...
\begin{highlighted}
	{These are all of the basic rules for the proof system for TFL.} For ease of reference, they're listed again in \cref{glossary:basic rules TFL}.
\end{highlighted}
...
\end{highlighted}
In this chapter, we explain a bit more about how such an argument would go. Soundness is proved in more detail in \cref{ch:Soundness}.
