...
If assuming something leads you to a contradiction, then the assumption must be wrong. We have a contradiction when we both have $\metaY$ and $\enot\metaY$ for some sentence $\metaY$. So if assuming $\metaX$ leads to both $\metaY$ and $\enot\metaY$, then $\metaX$ must be false.  This thought motivates the following rule:
\factoidbox{\begin{pf}
\open
	\hypo[m]{a}{\metaX}
	\ellipsesline
...
The next rule is quite similar. If assuming that something is false leads you to a contradiction, then that assumption must be wrong --- and so that `something' must in fact be true. In other words, if we assume $\enot\metaX$ and derive a contradiction, then we can conclude $\metaX$: we eliminate the negation.
\factoidbox{\begin{pf}
\open
	\hypo[m]{a}{\enot\metaX}
	\ellipsesline
