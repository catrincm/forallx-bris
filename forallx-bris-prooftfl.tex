...
\end{ebullet}
Note that there is no limit on the number of TFL sentences that can be mentioned before the symbol `$\entails$' (as long as it is a finite number of sentences). Indeed, we can even consider the limiting case where there is no sentence (this should ring familiar from \S\ref{sec:valtt}):
$$\entails \metaY$$
...
Even though we don't have a recursive definition of a proof in TFL, we can sketch how a recursive proof of the soundness of TFL would go. Imagine a base class of one-line proofs, one for each of our
%eleven
basic
