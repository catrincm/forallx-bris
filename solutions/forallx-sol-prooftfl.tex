...
\renewenvironment{pf} %TB: make proofs smaller, since space is a premium here
...
\end{pf}
\vfill
\noindent\myanswer{$\eand$E on line 4 can't be applied to line~1, since it is not of the form $\metaX \eand \metaY$.  `$A$' could be obtained by $\eand$E, but from line~2.}
...
\close
\have{con}{(G \eor H)}\pbc{ncon-bot2}
\end{pf}
}
...
\close
\have{con}{N \eor \enot N}\pbc{ncon-bot2}
\end{pf}}
...
\\\myanswer{If $\metaX \proves \metaY$, then $(\metaX \eand \metaZ )\proves \metaY$. After all, if $\metaX \proves \metaY$, then there is some proof with assumption $\metaX$ that ends with $\metaY$, and no undischarged assumptions other than $\metaX$. Now, if we start a proof with assumption $(\metaX \eand \metaZ )$, we can obtain $\metaX$ by $\eand$E. We can now copy and paste the original proof of $\metaY$ from $\metaX$, adding 1 to every line number and line number citation. The result will be a proof of $\metaY$ from assumption $\metaX$.}

\myanswer{However, we cannot prove much from $(\metaX \eor \metaZ )$. After all, it might be impossible to prove $\metaY$ from $\metaZ $.}
...
\\\myanswer{Consider the sentence $\metaX \eiff \metaY$. Suppose we can show that this is a theorem. So we can prove it, with no assumptions, in $m$ lines, say. Then if we assume $\metaX$ and copy and paste the proof of $\metaX \eiff \metaY$ (changing the line numbering), we will have a deduction of this shape:
\begin{pf}
\hypo{a}{\metaX}
\have[m][1]{m}{\metaX \eiff \metaY}
\have{b}{\metaY}\be{m, a}
\end{pf}
\\This will show that $\metaX \proves \metaY$. In exactly the same way, we can show that $\metaY \proves \metaX$. So if we can show that $\metaX \eiff \metaY$ is a theorem, we can show that $\metaX$ and $\metaY$ are provably equivalent.}

\myanswer{Conversely, suppose we can show that $\metaX$ and $\metaY$ are provably equivalent. Then we can prove $\metaY$ from the assumption of $\metaX$ in $m$ lines, say, and prove $\metaX$ from the assumption of $\metaY$ in $n$ lines, say. Copying and pasting these proofs together (changing the line numbering where appropriate), we obtain:
\begin{pf}
\open
	\hypo{a}{\metaX}
	\have[m]{b1}{\metaY}
...
\open
	...
	\have[m+n]{a1}{\metaX}
\close
\have{ab}{\metaX \eiff \metaY}\bi{a-b1, b-a1}
\end{pf}
\\Thus showing that $\metaX \eiff \metaY$ is a theorem.}

\myanswer{There was nothing special about $\metaX$ and $\metaY$ in this. So what this shows is that the problem of showing that two sentences are provably equivalent is, essentially, the same problem as showing that a certain kind of sentence (a biconditional) is a theorem.}
...
\\\myanswer{Third rule:
\begin{pf}
		\have[m]{nab}{\enot \metaX \eand \enot \metaY}
		\have[k]{na}{\enot \metaX}\ae{nab}
		\have{nb}{\enot \metaY}\ae{nab}
		\open
			\hypo{aob}{\metaX \eor \metaY}
			\open
				\hypo{a}{\metaX}
		...
		\close
		\have{con}{\enot (\metaX \eor \metaY)}\ni{aob-con3}
	\end{pf}}

\
\\\myanswer{Fourth rule:
	\begin{pf}
		\have[m]{nab}{\enot (\metaX \eor \metaY)}
		\open
			\hypo[k]{a}{\metaX}
			\have{aob}{\metaX \eor \metaY}\oi{a}
			\have{nab1}{\ered}\ri{aob, nab}
		\close
		\have{na}{\enot \metaX}\ni{a-nab1}
		\open
			\hypo{b}{\metaY}
			\have{aob2}{\metaX \eor \metaY}\oi{b}
			\have{nab2}{\ered}\ri{aob2, nab}
		\close
		\have{nb}{\enot \metaY}\ni{b-nab2}
		\have{con}{\enot \metaX \eand \enot \metaY}\ai{na, nb}
	\end{pf}
}
