...
This process may seem very tedious and, of course, very often we can determine a correct symbolization of an English sentence without sticking painstakingly to this step by step process. However, by sticking to the process we make sure not to move too quickly. So if you are unsure of how to symbolize a sentence, the process provides a safety net. Yet before starting with the symbolization process one should determine the precise sentential structure of the sentence under consideration. Consider a slight variant of Sentence \ref{sent:negcon1}:
\begin{itemize}
...
\end{samplesent}
Sentence \ref{sent:brownwins1} is true; sentence \ref{sent:brownwins2} is false, but both have false antecedents and false consequents. So the truth value of the whole sentence is not uniquely determined by the truth value of the constituent sentences.
...
Let us use the following representation key:
	...
	\end{ekey}
Sentence \ref{sent:not4} can now be symbolized by $R$. Moving on to sentence \ref{sent:not5}: saying that the information is irretrievable means that it is not the case that the information is retrievable. So even though sentence \ref{sent:not5} does not contain the word `not', we will symbolize it as `$\enot R$'.

Sentence \ref{sent:not5b} can be paraphrased as `It is not the case that the information is irretrievable.' Which can again be paraphrased as `It is not the case that it is not the case that the information is retrievable'. So we might symbolize this English sentence with the TFL sentence `$\enot\enot R$'. In other words, in English we can also express negation using prefixes such as `\emph{ir}' or `\emph{un}'. There are still further ways of expressing negation, for example, sometimes this may be done using  the prefix `\emph{dis}' as in honest/dishonest.

But some care is needed when handling negations. For example, one might think that sentences \ref{sent:not6} and \ref{sent:not7} negate each other and that if we symbolize sentence \ref{sent:not6} with $G$, then we should symbolize sentence  \ref{sent:not7} as $\enot G$.
...
Sometimes even in symbolizing sentences that very much look like they are negations of each other one needs to be careful. Consider:
	...
	\end{samplesent}
To some it may feel like Jane can be neither happy nor unhappy, and this may even be assumed in some argument. Jane is without emotions and in a state of blank indifference: she does not feel happy nor does she feel unhappy. If we were to symbolize \ref{sent:not8} by `$H$' and then \ref{sent:not9} by `$\enot H$', then we would rule out the possibility of Jane being neither happy nor unhappy. If we try to remain faithful to the idea that Jane can be neither happy nor unhappy then we need to symbolize \ref{sent:not9} using a new atomic sentence of TFL.
...
%Notice that we make no attempt to symbolize the word `also' in sentence \ref{sent:and3}. Words like `both' and `also' function to draw our attention to the fact that two things are being conjoined. Maybe they affect the emphasis of a sentence, but we will not (and cannot) symbolize such things in TFL.
...
In Chapter \ref{sec:tt} we already point out that here are two different reading available for the English word `or': an \emph{inclusive} one and an \emph{exclusive} one. On the inclusive reading of `or' we take the Sentence \ref{sent:or2} to be true, if either Fatima or Omar play videogames, or both play videogames. In contrast, on the exclusive reading we take \ref{sent:or2} to be true if either Fatima or Omar play videogames, but false if both play video games. It is the inclusive reading of `or' that we symbolize using the TFL-connective $\eor$.

For Sentence \ref{sent:or2} the inclusive reading is available and using the above symbolization key it can be symbolized as $F\eor O$. For Sentence \ref{sent:or1} it seems that the salient understanding of the sentence points to the exclusive reading: presumably Fatima cannot play videogames and watch movies at the same time. In fact the reason why we tend to understand Sentence \ref{sent:or1} in that way is, arguably, precisely because we think that Fatima cannot do two things at once, and not because of our understanding of `or'. As a consequence when, in the context of an argument, we symbolize the Sentence \ref{sent:or1} it is still preferable to symbolize it using the inclusive `or' as `$F\eor M$' and to add the additional premise that she does not do both, that is `$\enot(F\eand M)$'.
...
and use the following symbolization key:
	...
	\end{ekey}
Sentence \ref{sent:if3} and \ref{sent:if4} are just a rephrasing of \ref{sent:if1}. So we will again symbolise them as $(P\eif F)$.
...
\ref{sent:if2} is also a conditional. But, intuitively, while \ref{sent:if1}--\ref{sent:if3} are true \ref{sent:if4} is false, as, e.g., 6 is an even number but cannot be divided by 4. This suggest that \ref{sent:if4} needs to be symbolized differently than \ref{sent:if1}--\ref{sent:if3}. Indeed, `only if' inverses the order of the conditional: whereas \ref{sent:if1}--\ref{sent:if3} are symbolized as $(P\eif F)$, \ref{sent:if4} needs to be symbolized as $(F\eif P)$. Reflecting on the truth table of the conditional, $F\eif P$ says, assuming our symbolization key, that if $F$ is true, that if it is true that the number is even, then $P$ must be true for the conditional to be true, i.e., the number must be divisible by 4. That's of course not always the case, so the truth conditions for \ref{sent:if2} fits with our intuitive judgement.


% In \ref{sent:if1}--\ref{sent:if4} we took as an antecedent the part of sentence that immediately succeeded the word `if'. It might then be tempting to do the same here and symbolize this as $(P\eif F)$. That would be a mistake. Your knowledge of geography tells you that sentence \ref{sent:if1} is unproblematically true: there is no way for Jean to be in Paris that doesn't involve Jean being in France. But sentence \ref{sent:if2} is not so straightforward: were  Jean in Dieppe, Lyons, or Toulouse, Jean would be in France without being in Paris, thereby rendering sentence \ref{sent:if2} false. Since geography alone dictates the truth of sentence \ref{sent:if1}, whereas travel plans (say) are needed to know the truth of sentence \ref{sent:if2}, they must mean different things.
In fact, sentence \ref{sent:if2} can be paraphrased as `If Jean is in France, then Jean is in Paris'. So we can symbolize it by $(F \eif P)$: the other way around to \ref{sent:if1}.
	\factoidbox{
	 If a sentence can be paraphrased as \begin{itemize}
...
We will use the following symbolization key:
	...
	\end{ekey}
Sentence \ref{sent:iff1}, for reasons discussed above, can be symbolized by `$D \eif M$'. in contrast Sentence \ref{sent:iff2} can be paraphrased as, `If Laika is a mammal then Laika is a dog'. So it can be symbolized by `$M \eif D$'.

Sentence \ref{sent:iff3} says something stronger than both \ref{sent:iff1} and \ref{sent:iff2}. It can be paraphrased as `Laika is a dog if Laika is a mammal, and Laika is a dog only if Laika is a mammal'. This is just the conjunction of sentences \ref{sent:iff1} and \ref{sent:iff2}. So we can symbolize it as `$((D \eif M) \eand (M \eif D))$'. We call this a \define{biconditional}, because it entails the conditional in both directions. This leads to the following symbolization guideline:
...
\end{samplesent}
Sentence~\ref{sent:scamb2} is certainly false, since \emph{Avengers: Endgame} is over three hours long. Whether you think~\ref{sent:scamb1} is true depends on if you think it is boring or not. We can use the symbolization key:
\begin{ekey}
	\item[B] \emph{Avengers: Endgame} is boring.
	\item[L] \emph{Avengers: Endgame} is long.
\end{ekey}
Sentence~\ref{sent:scamb1} can now be symbolized as `$\enot(L \eand B)$', whereas sentence~\ref{sent:scamb2} would be `$\enot L \eand B$'. In the first case, the `\eand' is in the scope of `\enot', in the second case `\enot' is in the scope of `\eand'.
...
\end{ekey}
The symbolization of sentence~\ref{sent:scamb3} is `$(S \eand D) \eor T$' and that of sentence~\ref{sent:scamb4} is `$S \eand (D \eor T)$'. In the first, \eand is in the scope of \eor, and in the second \eor is in the scope of \eand.
