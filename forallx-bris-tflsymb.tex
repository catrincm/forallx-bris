...
Consider 
\begin{samplesent}
	\item If Harry had cake then either Joe or Carl did not. 
\end{samplesent}
We can break the sentence down as follows:
\begin{center}
	\begin{forest}
		[If Harry had cake then either Joe or Carl did not\\If \ldots then \ldots\\$\ldots\eif\ldots$
		[Harry had cake\\$H$]
		[Either Joe or Carl did not have cake\\\ldots or \ldots\\$\textcolor{lightgray}{(}\ldots\eor\ldots\textcolor{lightgray}{)}$
		[Joe did not have cake\\It is not the case that \ldots\\$\enot\ldots$[Joe had cake\\$J$]]
		[Carl did not have cake\\It is not the case that \ldots\\$\enot\ldots$[Carl had cake\\$C$]]
		]]
	\end{forest}
\end{center}
obtaining a symbolisation $$H\eif(\enot J\eor\enot C)$$
with a symbolisation key:\begin{ekey}
	\item[H] Harry had cake
	\item[J] Joe had cake
	\item[C] Carl had cake
\end{ekey}
...
We might try to symbolise it as $$B\eand W$$
	...
	\begin{center}
		\begin{tabular}{cc|c}
			Fluffy is black&Fluffy is white&Fluffy is a black and white cat\\
...
\end{samplesent}
	...
	Are these conjunctions? The word `but' sets up a contrast between the first part of the sentence and the second part. Nevertheless, it still tells us two things: that Adam is athletic, and that Barbara is not athletic. 
...
A similar point can be made about `although':
	\begin{samplesent}
		\item\label{sent:and6}Although Adam is athletic, Barbara is not.
		\item\label{sent:and8} Adam is athletic although Barbara is not. 
%	\item\label{sent:and7}Adam is athletic but Barbara is not.
\end{samplesent}
This raises the same sort of issue. The word `although' indicates a contrast, but that is not something TFL can represent. As far as truth conditions go, the sentence again tells us that Adam is athletic and that Barbara is not athletic. We will symbolise these sentences as $A\eand \enot B$.
