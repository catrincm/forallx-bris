...
\end{earg}
 ...
 We will symbolize the argument in TFL as follows:
...
\section{Atomic sentences}\todo{That's not how they should go about it!}

Symbolisations will be relative to a symbolisation key. For example:
...
\section{Symbolizing Complex Sentences}\label{sec:SymbolisingTFL - simple connectives complex form}
We introduced our connectives in \cref{sec:Connectives} giving their rough English meaning. This gives us some tools to know how to symbolise English sentences. 
As a general instruction:
	\begin{itemize}
		...
		\item If a sentence can be paraphrased as `if \ldots$_\metaX$, then \ldots$_\metaY$'\\ it can be symbolised as $\metaX\eif \metaY$.
	\end{itemize}
	
	Our task when given a sentence: can you find something to fill out the $\ldots$ to understand it in one of these ways. 
	By symbolising it with one of our connectives, we are specifying that the truth value of the overall sentence should be governed by the truth rule for that connective. So the truth value of the overall sentence should depend on the truth values of the sentences you pick to fill out $\ldots$ in the way governed by that connective. If it does not, then you need to think about a different paraphrasing or simply use an atomic sentence. 
%	The things filling out the $\ldots$ must be complete (declarative) sentences, and the truth value of the overall sentence ultimately should be determined in accordance with the truth rule for the chosen connective depending on the truth values of the 

By symbolising it with one of these connectives, the truth conditions for the sentence is determined in accordance with the truth rule. 
Symbolising it this way is appropriate when these are appropriate truth conditions; but these standard English paraphrasing is good guidance for when this is the case. 

Typically one needs to apply this paraphrasing trick a number of times to result in the symbolisation for the sentence. 
...
Our first task: identify its main logical form. For this sentence we can find something to fill out $\ldots$ where the sentence is paraphrased as 
\begin{samplesent}
	\item[] It is not the case that \ldots
\end{samplesent}
In particular: 
\begin{samplesent}
	\item[] It is not the case that \underline{you will get both soup and salad}.
\end{samplesent}
`You won't get both soup and salad' is true if and only if `you will get both soup and salad' is false. It is governed by the truth rule for $\enot$ depending on the truth of `you will get both soup and salad'. 

We will keep track of the stages of our symbolisation in a tree form, which bears a close relationship to the formation tree we've seen already.
\begin{center}
	\begin{forest}
		[You won't get both soup and salad.\\It is not the case that \ldots\\$\enot\ldots$
		[You will get both soup and salad\\?]]
	\end{forest}
\end{center}
This should be read as: we take our initial sentence `You won't get both soup and salad.' and we paraphrase it as `it is not the case that \ldots', with what fills out the $\ldots$ being placed underneath. We then know that we'll give a symbolisation of the overall sentence as $\enot \ldots$, where however we symbolise `You will get both soup and salad' fills out the $\ldots$. 

So, how to symbolise `You will get both soup and salad'? We can paraphrase it as the form \begin{samplesent}
	\item[]\ldots and $\ldots$
\end{samplesent}
In particular: 
\begin{samplesent}
	\item[] \underline{you will get soup} and \underline{you will get salad}
\end{samplesent}
With the sentence `You will get both soup and salad' being true if and only if both `you will get soup' and `you will get salad' are true. It is appropriately governed by $\eand$. 
\begin{center}
	\begin{forest}
		[You will get both soup and salad\\\ldots and \ldots\\$\almostinvisibleopenbracket\ldots {\eand} \ldots\almostinvisibleclosebracket$
		[You will get soup\\?]
		[You will get salad\\?]
		]
	\end{forest}
\end{center}
Both `You will get soup' and `You will get salad' cannot be paraphrased in any of our standard forms so we have to use an atomic TFL-sentence. As we haven't already introduced any atomic TFL-sentences, we need to introduce new ones. We can chose from $A$-$W$ or with numerical subscripts. Let's choose. 
\begin{ekey}
	\item[S_1] You will get soup
	\item[S_2] You will get salad
\end{ekey}

We can then build our symbolisation tree:
\begin{center}
	\begin{forest}
		[You won't get both soup and salad.\\It is not the case that \ldots\\$\enot\ldots$
[You will get both soup and salad\\\ldots and \ldots\\$\almostinvisibleopenbracket\ldots {\eand} \ldots\almostinvisibleclosebracket$
...
[You will get salad\\$S_2$]
	...
	\end{forest}
\end{center}
Now to obtain the symbolisation of our overall sentence we just need to work back up the tree, replacing \ldots with the determined symbolisation. 
\begin{center}
	\begin{forest}
		[You won't get both soup and salad.\\It is not the case that \ldots\\$\mainconnective{\enot}\textcolor{blue}{(S_1\eand S_2)}$
		[You will get both soup and salad\\\ldots and \ldots\\$\almostinvisibleopenbracket\textcolor{blue}{S_1} \mainconnective{\eand} \textcolor{blue}{S_2} \almostinvisibleclosebracket$
		[You will get soup\\$S_1$]
		[You will get salad\\$S_2$]
		]
		]
...
Here's a description of this strategy:
\begin{highlighted}
	\begin{enumerate}
		\item\label{bpr} Consider whether the sentence can be paraphrased as sentences in one of our standard forms, and governed by the associated truth rule. 
		\begin{center}
			\begin{tabular}{l l}
...
Consider:
\begin{samplesent}
	\item If John has a pet, then he has a cat or a dog. 
\end{samplesent}

We apply this strategy to break the sentence down:
\begin{center}
	\begin{forest}
		[If John has a pet then he has a cat or a dog\\If \ldots then \ldots\\$\ldots\eif\ldots$
		[John has a pet\\$P$]
		[John has a cat or a dog\\\ldots or \ldots\\$\almostinvisibleopenbracket\ldots\eor\ldots\almostinvisibleclosebracket$
		[John has a cat\\$C$]
		[John has a dog\\$D$]
		]]
	\end{forest}
\end{center}
Which, when we put back together as:
\begin{center}
	\begin{forest}
		[If John has a pet then he has a cat or a dog\\$\textcolor{blue}{P}\mainconnective{\eif}\textcolor{blue}{(C\eor D)}$
		[John has a pet\\$P$]
		[John has a cat or a dog\\$\almostinvisibleopenbracket\textcolor{blue}{C}\mainconnective{\eor}\textcolor{blue}{D}\almostinvisibleclosebracket$
		[John has a cat\\$C$]
		[John has a dog\\$D$]
		]]
	\end{forest}
\end{center}
So we obtain our symbolisation: $$P\eif (C\eor D)$$
with a symbolisation key:\begin{ekey}
	\item[P] John has a pet
	\item[C] John has a cat
	\item[D] John has a dog
\end{ekey}


Consider 
\begin{samplesent}
	\item If Harry had cake then either Lotta or Karl did not. 
\end{samplesent}
We can break the sentence down as follows:
\begin{center}
	\begin{forest}
		[If Harry had cake then either Lotta or Karl did not\\If \ldots then \ldots\\$\ldots\eif\ldots$
		[Harry had cake\\$H$]
		[Either Lotta or Karl did not have cake\\\ldots or \ldots\\$\almostinvisibleopenbracket\ldots\eor\ldots\almostinvisibleclosebracket$
		[Lotta did not have cake\\It is not the case that \ldots\\$\enot\ldots$[Lotta had cake\\$L$]]
		[Karl did not have cake\\It is not the case that \ldots\\$\enot\ldots$[Karl had cake\\$K$]]
		]]
	\end{forest}
\end{center}
obtaining a symbolisation $$H\eif(\enot L\eor\enot K)$$
with a symbolisation key:\begin{ekey}
	\item[H] Harry had cake
	\item[L] Lotta had cake
	\item[K] Karl had cake
\end{ekey}
...
%\chapter{When is it appropriate to symbolise using the connectives}



\section{Does the symbolisation give the right truth-conditions?}\label{sec:SymbTruthConds}

We have already had to apply some analysis to determine how to split a sentence up. 

Consider:
\begin{samplesent}
	\item Lotta and Karl had cake
\end{samplesent}
We will expand this out to:
\begin{samplesent}
	\item[] \underline{Lotta had cake} and \underline{Karl had cake}
\end{samplesent}
\begin{center}
	\begin{forest}
		[Lotta and Karl had cake\\\ldots and \ldots\\$\ldots \eand \ldots$
		[Lotta had cake\\$L$][Karl had cake\\$K$]]
	\end{forest}
\end{center}
We judge that it is correct that `Lotta and Karl had cake' is determined in accordance the rule for $\eand$:
\begin{center}
	\begin{tabular}{cc|c}
	Lotta had cake&Karl had cake&Lotta and Karl had cake\\
	$L$&$K$&$L\eand K$\Bstrut\\\hline\Tstrut
	T&T&T\\
	T&F&F\\
	F&T&F\\
	F&F&F
...
This was appropriate. But is it always appropriate? 

Consider
\begin{samplesent}
	\item Fluffy is black and white.
\end{samplesent}
This has `and' in it, but it is not appropriately paraphrased as:
\begin{samplesent}
	\item \underline{Fluffy is black} and \underline{Fluffy is white}
\end{samplesent}
We might try to symbolise it as $$B\eand W$$

	\begin{center}
		\begin{forest}
			[Fluffy is black and white\\\ldots and \ldots\\$\ldots \eand \ldots$
			[Fluffy is black\\$B$][Fluffy is white\\$W$]]
		\end{forest}
	\end{center}
	
	This clearly does not capture the intended truth conditions of `Fluffy is black and white'. 
	
	\begin{center}
		\begin{tabular}{cc|c}
			Fluffy is black&Fluffy is white&Fluffy is black and white \\
			$B$&$W$&$B\eand W$\Bstrut\\\hline\Tstrut
			T&T&T\\
			T&F&F\\
			F&T&F\\
			F&F&F
...
Similarly, consider:
	\begin{samplesent}
		\item Adina likes fish and chips.
	\end{samplesent}
	One might try to break it down as: 
	\begin{center}
		\begin{forest}
			[Adina likes fish and chips\\\ldots and \ldots\\$\ldots \eand \ldots$
			[Adina likes fish\\$F$][Adina likes chips\\$C$]]
		\end{forest}
	\end{center}
	which is governed by:
	\begin{center}
		\begin{tabular}{cc|c}
			Adina likes fish&Adina likes chips&Adina likes fish and chips\\
			$F$&$C$&$F\eand C$\Bstrut\\\hline\Tstrut
			T&T&T\\
			T&F&F\\
			F&T&F\\
			F&F&F
		\end{tabular}
	\end{center}
but the problem is that probably the author of this sentence meant that she likes eating fish-and-chips, rather than giving two separate facts, that she likes fish and that she likes chips. 

Sometimes it is a matter of judgement, though. What about:
\begin{samplesent}
	\item Priya likes nuts and cheese.
\end{samplesent}
What does the author of the sentence mean?
\begin{samplesent}
		\item Priya likes nuts and cheese (together)
		\item {Priya likes nuts} and {Priya likes cheese}
\end{samplesent}
Only the latter is to be sy
If we symbolise it as $N\eand C$ it will be given by the truth conditions: 
\begin{center}
	\begin{tabular}{cc|c}
		Priya likes nuts&Priya likes cheese&Priya likes nuts and cheese\\
		$N$&$C$&$N\eand C$\Bstrut\\\hline\Tstrut
		T&T&T\\
		T&F&F\\
		F&T&F\\
		F&F&F
...
\begin{highlighted}When symbolizing sentences, make sure the paraphrasing matches the truth conditions\end{highlighted}

\section{How much paraphrasing is appropriate?}

Consider the following sentence:
\begin{samplesent}
	\item\label{sent:not5} The information is irretrievable.
\end{samplesent}
Even though the sentence does not explicitly include `not', it does have `ir-' which indicates a negation.
We will paraphrase it as \begin{samplesent}
	\item It is not the case that \underline{the information is retrievable}
\end{samplesent}

\begin{center}
	\begin{forest}
		[The information is irretrievable\\It is not the case that \ldots
		[The information is retrievable\\$R$]]
	\end{forest}
\end{center}
So we symbolise it as `$\enot R$' with
\begin{ekey}
	\item[R] The information is retrievable.
\end{ekey}

In English we can also express negation using prefixes such as `\emph{ir}' or `\emph{un}'. There are still further ways of expressing negation, for example, sometimes this may be done using  the prefix `\emph{dis}' as in honest/dishonest. In these cases, we should expect to paraphrase it as `it is not the case that \ldots' and use $\enot$ in the symbolisation. 

Consider:
\begin{samplesent}
	\item\label{sent:good} That action is good.
	\item\label{sent:bad} That action is bad.
\end{samplesent}
Let's symbolise \cref{sent:good} with an atomic sentence, giving a symbolisation key with:
\begin{ekey}
	\item[G] That action is good.
\end{ekey}
Should we paraphrase \cref{sent:bad} as 
\begin{samplesent}
	\item It is not the case that \underline{that action is good}
\end{samplesent}
symbolising it as $\enot G$. 

There are two reasons we will not do this. 

For one, while we may all agree that stealing is not a good thing to do, we may think that it is perhaps not exactly bad, if someone who is on the verge of starvation steals some food. In other words, there may be things that are neither good nor bad. However,  in TFL if `$G$' is false, then `$\enot G$' is true and we would not have the option so say that something, e.g.~stealing, is neither good nor bad. If we symbolize \cref{sent:bad} using a new sentence letter, say `$B$', then we can allow for that option. 

For another, there is no syntactic marker such as a prefix, a word like `not', etc.~that suggests that the sentences \cref{sent:good} and \cref{sent:bad} are negations of each other. There is nothing in the structure of the two sentences that suggests that one negates the other.  But recall that one principal goal for symbolization is to check whether a given argument is valid and consider the argument:
\begin{earg}
	\prem Stealing is good or donating for charity is good.
	\prem Stealing is bad.
	\conc Donating for charity is good.
\end{earg}
While this is a good argument and perhaps a conceptually valid argument it is not valid in the strict sense because it not valid in virtue of its form: it requires understanding `bad' as `not good'. So, if we were to symbolize \cref{sent:bad} as `$\neg G$ we would stipulate some logical structure that the English sentence does not display and would wrongly deem the argument to be valid.

\begin{highlighted}When symbolizing sentences, only introduce logical structure that is displayed by the English sentence\end{highlighted}

If you wish to make the information that in the specific circumstances `bad' means that something is not good and vice versa available in TFL, you can add this as an additional premises when symbolizing the relevant argument in TFL. That, is if we think that \cref{sent:bad} is the negation of \cref{sent:good} we should add the following premise (using truth tables check that the premise is true, if and only if, `$\enot G$' is true whenever `$B$' is true): $$(\enot G\eif B)\eand (B\eif \enot G).$$ If this premise is added to the above argument, then the argument is valid and rightly so!
...
Often English sentences come with different ways of joining sentences than our standard `and', `or', `not' or `if'. 
...
%	\item \mainconnective{Either} Alice \mainconnective{or} Beth will come to the party \mainconnective{\emph{(and not both)}}.  
	\item Alice won't come to the party \mainconnective{unless} Beth comes. 
...
These are all sentences whose truth values depend on whether Alice and Beth come to the party. The highlighted connectives are components of English which join together two simpler sentences so that the truth value of the overall sentence just depends on the truth values of the simpler sentences which are combined. 
...
In plenty of languages, for example English, there are connectives that are not truth-functional. We here describe just two (Exercise: find further non-truth-functional connectives of English):
...
\chapter{Expanding our Symbolisation Guide}\label{sec:symbcon}
We have our standard symbolisation guide: 
	\begin{itemize}
	\item If a sentence can be paraphrased as `it is not the case that \ldots'\\ it can be symbolised as $\enot\metaX$.
	\item If a sentence can be paraphrased as `\ldots and \ldots'\\ it can be symbolised as $\metaX\eand \metaY$.
	\item If a sentence can be paraphrased as `\ldots or \ldots'\\ it can be symbolised as $\metaX\eor \metaY$.
...
But often sentences of English aren't neatly given in one of these forms. Consider these sentences:
\begin{compactlist}
	\item \mainconnective{Although} Alice will come to the party\mainconnective{,} Beth will not.  
	\item Alice will come to the party, \mainconnective{but} Beth will not.  
	\item \mainconnective{Either} Alice \mainconnective{or} Beth will come to the party \mainconnective{\emph{(and not both)}}.  
	\item \mainconnective{Unless} Alice will come to the party\mainconnective{,} Beth will not.  
	\item Beth won't come to the party \mainconnective{unless} Alice comes.
	\item \mainconnective{Neither} Alice \mainconnective{nor} Beth will come to the party.  
	\item Alice will come to the party \mainconnective{only if} Beth will too.
	\item Alice will come to the party \mainconnective{if} Beth will too.
	\item Alice will come to the party \mainconnective{if and only if} Beth will. 
...
In this chapter we will discuss how they should be symbolised in TFL. 


The feature of TFL is that it can capture any \emph{truth functional} connective (recall \cref{sec:TruthFunctionality}), possibly by a more complex sentence. 

We will expand our symbolisation guide to give instructions about how to treat a variety of English sentences whose logical form isn't presented in a nicely canonical way. The completed symbolisation guide can be found in \cref{appendix:quickref:symbolization}. 

%Let's first have a look which conjunction words and expressions can be directly symbolized by one of the TFL-connectives.

\section{Ways of expressing conjunction}
\label{sec:ConnectiveConjunction}

The symbolization guideline for conjunction we introduced Section \cref{sec:SymbolisingTFL - simple connectives complex form} was:.
...
\begin{samplesent}
	\item Adam and Barbara are both athletic. 
\end{samplesent}

We will appropriately paraphrase it as 
\begin{samplesent}
	\item \underline{Adam is athletic} and \underline{Barbara is athletic}.
\end{samplesent}
In our symbolisation tree format:
\begin{center}
	\begin{forest}
		[Adam and Barbara are both athletic\\\ldots and \ldots\\$\ldots\eand\ldots$
		[Adam is athletic\\$A$]
		[Barbara is athletic\\$B$]
		]
	\end{forest}
\end{center}
Giving a symbolisation $$A\eand B$$ 
...
This appropriately captures the truth conditions of the original sentence. It is true just if Adam is athletic and Barbara is athletic, i.e., if both $A$ and $B$ are true. 

%
%
%How about 
%\begin{samplesent}
%	\item Although Adam is athletic, Barbara is not. 
%\end{samplesent}
%
%
%
...
%
%\todo{This discussion needs moving to the earlier paraphrasing}
...
Now consider:
	\begin{samplesent}
%	\item\label{sent:and6}Although Adam is athletic, Barbara is not.
	\item\label{sent:and7}Adam is athletic but Barbara is not.
\end{samplesent}

The word `but' sets up a contrast between the first part of the sentence and the second part. Nevertheless, it still tells us two things: that Adam is athletic, and that Barbara is not athletic. 
	
	We will treat it as just another way of expressing a conjunction `and': 
	\begin{center}
		\begin{forest}
			[Adam is athletic but Barbara is not.\\\ldots and \ldots\\$\ldots\eand\ldots$
			[Adam is athletic\\$A$]
			[Barbara is not athletic\\It is not the case that \ldots\\$\enot\ldots$
			[Barbara is athletic\\$B$]]
			]
		\end{forest}
	\end{center}
	So we give $$A\eand\enot B$$
	
	There is a difference in tone between these two sentences, but no difference in truth conditions. 
	\begin{samplesent}
		\item Adam is athletic but Barbara is not.
		\item Adam is athletic and Barbara is not athletic.
	\end{samplesent}
They are both true just when Adam is athletic and Barbara is not athletic: 
	\begin{center}
	\begin{tabular}{cc|def}
		$A$&$B$&$A$&$\eand$&$\enot B$\Bstrut\\\hline\Tstrut
		T&T&T&\TTbf{F}&F\\
		T&F&T&\TTbf{T}&T\\
		F&T&F&\TTbf{F}&F\\
		F&F&F&\TTbf{F}&T
...
We have lost some nuance in this symbolisation, but TFL does not (and cannot) preserve that nuance. 
The word `but' often carries an additional suggestion of contrast or unexpectedness between the two clauses. This is not part of the literal truth-conditions of the sentence, but arises from what is called a conversational implicature: something suggested or conveyed by the way a sentence is used in conversation, rather than by its strict logical content.
If there was no surprise in the difference between the facts, then we would still say that the sentence combined with `but' is infelicitous or misleading but is nonetheless true.

A similar point can be made about `although':
	\begin{samplesent}
		\item\label{sent:and6}Although Adam is athletic, Barbara is not.
		\item\label{sent:and8} Barbara is not athletic, although Adam is. 
%	\item\label{sent:and7}Adam is athletic but Barbara is not.
\end{samplesent}
This raises the same sort of issue. The word `although' indicates a contrast, but that is not something TFL can represent. As far as truth conditions go, the sentence again is true just if Adam is athletic and Barbara is not athletic. We will symbolise these sentences as $A\eand \enot B$.
...
In \cref{sec:SymbolisingTFL - simple connectives complex form}, we proposed that:
\begin{highlighted}
...
Sometimes in English we use `either\ldots or\ldots' in a way which indicates that it should be read exclusively, consider, for example:
\begin{samplesent}
...
\end{samplesent}
Here it's not so clear that an exclusive disjunction is being indicated. Often `either' is introduced in the English to play something like the role of brackets. It helps indicate what is being held together. 
...
\begin{highlighted}
If a sentence can be paraphrased in English as `\emph{either \ldots or \ldots (and not both)}', \\it can be symbolised as $(\metaX\eor\metaY)\eand\enot(\metaX\eand\metaY)$.
	\end{highlighted}
...
\begin{samplesent}
	\item Neither Fatima nor Omar will do their homework.
...
Here is a way to paraphrase it and break it down into our standard connectives:
\begin{center}
	\begin{forest}
		[Neither Fatima nor Omar will do their homework\\It is not the case that \ldots\\$\enot\ldots$
		[Fatima or Omar will do their homework\\\ldots or \ldots \\$\ldots\eor\ldots$
		[Fatima will do her homework\\$F$][Omar will do his homework\\$O$]]]
	\end{forest}
\end{center}
Giving the symbolisation: $$\enot(F\eor O)$$

There is a completely different but also entirely adequate way to paraphrase it and break it down: 
\begin{center}
	\begin{forest}
		[Neither Fatima nor Omar will do their homework\\\ldots and \ldots\\$\ldots\eand \ldots$
		[Fatima will not do her homework\\It is not the case that \ldots\\$\enot \ldots$
		[Fatima will do her homework\\$F$]]
		[Omar will not do his homework\\It is not the case that \ldots\\$\enot \ldots$
		[Omar will do his homework\\$O$]]
		]
	\end{forest}
\end{center}
Giving the symbolisation $$\enot F\eand\enot O$$
...
\begin{highlighted}
	If a sentence can be paraphrased in English as `\emph{neither \ldots nor \ldots}',\\ it can be symbolised as $\enot(\metaX\eor\metaY)$ or as $\enot \metaX\eand\enot \metaY$.\end{highlighted}

\section{Conditionals}
Let us again recall the symbolization guideline that we introduced in \Cref{sec:SymbolisingTFL - simple connectives complex form}:
	\begin{highlighted}
...
\begin{highlighted}
	\textbf{Symbolisation Guide}
	
	\smallskip 
	
	\begin{tabular}{c|l}
		\textbf{Paraphrased as} & \textbf{Symbolised as}\\\hline[.5em]
		...
		Unless $\ldots_{\metaX}$, $\ldots_{\metaY}$ & $\metaX \eor \metaY$ or $\enot X\eif Y$ or $\enot Y\eif X$\\
	\end{tabular}
\end{highlighted}
...
However, there are of course many more conjunction-words in English and some of them can be adequately symbolized in TFL. However, there are also many conjunction words that cannot be adequately symbolized in TFL. We discussed some examples in \cref{sec:TruthFunctionality}.
...
\begin{practiceproblems}
	\solutions
	\problempart The following sentences are ambiguous. Give symbolization keys for each and symbolize the different readings. Show that they are not logically equivalent. 
	\begin{problemslist}
