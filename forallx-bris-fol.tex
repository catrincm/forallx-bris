...
The expressive power of TFL is not rich enough to explain why these English arguments are valid. Arguments in TFL are valid in virtue of the meanings of the connectives $\enot, \eif,\eand, \eor$. These arguments, though are valid in virtue of something else. 
...
In First Order Logic, FOL, we can symbolise these different components. We will use lower-case letters $a,b,c\ldots$ for names (except $x,y,z$ which are used for variables as we will later see), and upper case letters such as $A,B,C,\ldots$ for predicates (except $X,Y,Z$, which are used for metavariables). We can also add numbered subscripts if needed, for example using $d_{27}$ as a name, or $H_{386}$ as a predicate. 
...
We could then give a symbolisation key specifying: 
\begin{ekey}
\item[a] Alice
\item[Lx] \gap{x} is a logician
\end{ekey}
...
Let's see some other example sentences which have this same form. Each of these sentences could be symbolised as $La$, though the symbolisation key would have to change in each of these instances.
\begin{earg}
...
\item[a] Rocky
\item[Lx] \gap{x} is strong
\end{ekey}
\item[\ref{folbiden}] \begin{ekey}
\item[a] Joe Biden
\item[Lx] \gap{x} is a Democrat
\end{ekey}
\item[\ref{folpalin}] \begin{ekey}
\item[a] Michael Palin
\item[Lx] \gap{x} is a member of Monty Python
\end{ekey}
\end{enumerate}
...
\end{earg}
as $La$ using the symbolisation key:
\begin{ekey}
\item[a] The Tower of London
\item[Lx] \gap{x} is in England
\end{ekey}
...
\end{earg}
This doesn't say of any specific individual that they wear a funny hat. To express this, we introduce the $\forall$ symbol. This is called the \emph{universal quantifier}.
...
as $$\forall x Fx$$ If we wanted to symbolise ``Alice wears a funny hat'' we would use $Fa$. To symbolise ``Everyone wears a funny hat'', we paraphrase it as ``For everyone $_x$: $x$ wears a funny hat.'' and then symbolise it as $\forall x Fx$. 
...
But how the quantifier should be read depends on the \define{domain}. The domain is the collection of things that we are talking about.

We can use FOL to talk about things other than people. For example, we might talk about dogs or landmarks. 

Then $\forall x$ shouldn't be read as ``for everyone'' but more generally as ``for all objects in the domain $_x$''. That is, the quantifiers \emph{range over} the objects in the domain. 
...
\end{ekey}
Then $\forall x Ex$ symbolises ``All dogs are energetic''. More schematically, we'd then read $\forall x$ as ``For every dog $_x$ : \ldots". 
...
We can make this explicit by spelling out these details in the domain. 
