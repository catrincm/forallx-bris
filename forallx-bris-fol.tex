...
%We now give the inductive definition of what it is to be a formula of FOL.
	...
	As in TFL, we can display the construction of formulas using our formation trees (\cref{sec:TFLSentences}). Like in TFL, we will write outer brackets in invisible ink, which here is displayed in gray. Such outer brackets can be dropped when the formula is written by itself but must be included once it is embedded in a further formula. 
		\begin{center}
		\begin{forest}
		[$\almostinvisibleopenbracket\enot(Dx\eand Db)\mainconnective{\eif} Lxy\almostinvisibleclosebracket$
		[$\mainconnective{\enot}(Dx\eand Db)$
		[$\almostinvisibleopenbracket Dx \mainconnective{\eand} Db\almostinvisibleclosebracket$
		[$Dx$]
		[$Db$]
		]
		]
		...
		\end{forest}
		\end{center}
		This is exactly as in the case of TFL, the only difference is that the ``leaves'' of the tree have more structure to them: they're predicates applied to names or variables rather than simply the single atomic sentences that we had in TFL.

FOL also allows us to add quantifiers with variables in the front of a formula we have. We can, for example, give:
\begin{center}
	\begin{forest}
...
\begin{center}
	\begin{forest}
		[$\mainconnective{\enot}\forall x\enot Hx$
		[$\mainconnective{\forall x}\enot Hx$
		[$\mainconnective{\enot}Hx$
		[$Hx$]]
		]
		]
	\end{forest}
...
We can also add existential quantifiers, and mix up the variables in use, for example: 
		\begin{center}
		\begin{forest}
			[$\mainconnective{\exists z}(Hz\eand\forall y (Ryz\eand \exists x Qx))$
			[$\almostinvisibleopenbracket Hz\mainconnective{\eand}\forall y (Ryz\eand \exists x Qx)\almostinvisibleclosebracket$
			[$\mainconnective{\forall y }(Ryz\eand\exists x Qx)$
			[$\almostinvisibleopenbracket Ryz\mainconnective{\eand} \exists x Qx\almostinvisibleclosebracket$
			[$Ryz$]
...
In the formation tree of the formula, the scope of a connective or quantifier is the sub-tree where it is the main logical operator of the formula at the top. Only that subtree is relevant to determining the truth value of that formula under each variable assignment.

We give an example identifying the scope of each quantifier in this sentence:
\begin{center}
	\begin{forest}
		[$\mainconnective{\enot}\exists z(\exists y(\forall x Rxy\eand Ryz)\eand \forall y Rzy)$
		[{$\customcoloredbox[teal!20]{\exists z}(\exists y(\forall x Rxy\eand Ryz)\eand \forall y Rzy)$},tikz={\node [draw,inner sep=7pt,teal,fit to=tree,thick,rounded corners]{};}
			[$(\exists y(\forall x Rxy\eand Ryz)\mainconnective{\eand}\exists y Rzy)$
				[{$\customcoloredbox[blue!10]{\exists y}(\forall x Rxy\eand Ryz)$},tikz={\node [draw,blue,inner sep=3pt,fit to=tree,thick,rounded corners]{};}
					[$(\forall x Rxy\mainconnective{\eand} Ryz)$
						[{$\customcoloredbox[green!10]{\forall x}Rxy$},tikz={\node [draw,green,inner sep=0,fit to=tree,thick,rounded corners]{};}
							[$Rxy$]
						]
						[$Ryz$]
					]
				]
				[{$\customcoloredbox[red!10]{\forall y} Rzy$},tikz={\node [draw,red,inner sep=0,fit to=tree,thick,rounded corners]{};}
					[$Rzy$]
				]
			]
		]
...
We can also present this linearly:
