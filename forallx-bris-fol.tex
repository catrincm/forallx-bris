...
Instead you might try: well, lets just let $L$ stand for `logician' and $H$ stand for `wears a funny hat'. Then we can just use $L\eif H$. The problem is that these aren't declarative sentences. They're not the sort of thing to be true or false. That is what a move to FOL will let us capture, though. ``If someone is a logician, they wear a funny hat.'' We will use variables to play the role of `they' which aren't talking about individuals but whose reference may vary. We then want to say of all people, if they are a logician then they wear a funny hat.  \begin{itemize}
	\item for all $x$, if $x$ is a logician, then $x$ wears a funny hat.
\end{itemize} We will then end up symbolising it as $$\forall x(Lx\eif Hx)$$ where $L$ and $H$ are now predicates expressing properties of individuals: being a logician, or wearing a funny hat. 
