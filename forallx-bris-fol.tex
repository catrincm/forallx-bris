...
You might feel the pull to symbolise premise 1 as $$L\eif H.$$
With the given symbolisation key, this is to say ``if Alice is a logician, she wears a funny hat.'' That's enough to get to the conclusion, and it's a consequence of premise 1, but it's not what premise 1 says. It's not an adequate symbolisation of premise 1. 

Instead you might try: well, lets just let $L$ stand for `logician' and $H$ stand for `wears a funny hat'. Then we can just use $L\eif H$. The problem is that these aren't declarative sentences. They're not the sort of thing to be true or false. They only form declarative sentences once their applied to an individual, such as Alice. 
...
$$\forall x(Lx\eif Hx),\; La\;\therefore\; Ha$$
...
\begin{center}
	\begin{forest}
		[Joe is happy and Katie is sad\\Joe is happy \mainconnectivecolor{and} Katie is sad\\$\ldots {\eand} \ldots$
		[Joe is happy\\$Hj$]
		[Katie is sad\\$Sk$]
		]
	\end{forest}
