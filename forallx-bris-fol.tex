...
These are symbolised in FOL as $Hx$ or $\exists y Lxy$. They are not \emph{sentences} of FOL, but they are well-formed \emph{formulas}. Formulas are the sorts of things that form sentences when you put quantifiers in front of them. Sentences are special kinds of formulas, those which have no so-called free-variables. No additional specification of who $x$ is is needed in $\forall x Hx$. It is a sentence. 
...
We can also add existential quantifiers, and mix up the variables in use, for example: 
		...
		\end{forest}
	\end{center}
	Again, if no binary connectives are used, the quantifiers just get stacked on top of one another without any brackets being written.
		\begin{center}
		\begin{forest}
