...
However, just like with TFL, we have to be careful. 
To symbolise:
\begin{samplesent}
	\item\label{sent:skilledsurgeon} Adina is a skilled surgeon
\end{samplesent}
Should one simply symbolise it as
$$Ka$$
using
\begin{ekey}
	\item[Kx] \gap{x} is a skilled surgeon
	\item[a] Adina
\end{ekey}
Or should one split it into
$$La\eand Sa$$
using
\begin{ekey}
	\item[Lx]\gap{x} is skilled
	\item[Sx] \gap{x} is a surgeon
	\item[a] Adina
\end{ekey}
This symbolisation would naturally lead to a symbolisation of this argument which would make it valid:
\begin{quote}
	Adina is a skilled surgeon. She also plays tennis. Therefore, Adina is a skilled tennis player.
\end{quote}
by symbolising it as $$La\eand Sa,\; Ta\;\therefore\; La\eand Ta$$
...
\end{samplesent}
Here `it' is not referring to a particular object, instead the object that it refers to can vary. This sentence is saying of every thing, if it is in my pocket then it is a coin. 
\begin{samplesent}
...
\begin{highlighted}
	...
	\begin{itemize}
		\item For all $x$: $\ldots$
	\end{itemize}
	We also use it for:
	\begin{itemize}
		\item For everything, $x$: $\ldots$
	\end{itemize}
\end{highlighted}
...
\begin{samplesent}
	\item[] For all $x$: if \underline{$x$ is like \emph{this}}, then \underline{$x$ is like \emph{that}}. 
\end{samplesent}
...
\begin{samplesent}
	\item[] For all $x$: if \underline{$x$ is a dog} then \underline{$x$ is energetic}.
\end{samplesent}
...
\end{samplesent}
We see that it's main form is saying something about everyone. So we're expecting to use $\forall$. $\forall x$ is understood as everything is like blah. What do we want to say of everything? We want to say, if it is like \emph{this} and \emph{that}. So we need to decide what to use to play the role of `\emph{this}' and `\emph{that}'. 
...
In this case: 
\begin{samplesent}
\item[]	For all $x$: if \underline{$x$ is an enthusiastic student}, then \underline{$x$ is not tired}. 
\end{samplesent}
...
\begin{center}
	\begin{forest}
		...
		For all $x$: if \ldots then \ldots\\
		$\forall x(\ldots\eif\ldots)$
		[$x$ is an enthusiastic student\\$\almostinvisibleopenbracket Ex\eand Sx\almostinvisibleclosebracket$]
		[$x$ is not tired\\$\enot Tx$]
		]
	\end{forest}
\end{center}
so provide the symbolisation $$\forall x((Ex\eand Sx)\eif\enot Tx)$$
Recalling our bracketing rules, we can also drop some brackets:
$$\forall x(Ex\eand Sx\eif \enot Tx)$$
...
\begin{center}
	...
	\end{forest}
\end{center}
Giving us $$\forall x((Ex\eand Sx)\eif (\enot Tx\eand\enot Bx))$$
Recalling our bracketing rules, we can also drop some brackets:
$$\forall x(Ex\eand Sx\eif \enot Tx\eand\enot Bx)$$
...
\begin{highlighted}
	...
	\begin{itemize}
		\item There exists at least one $x$: $\ldots$
	\end{itemize}
	Or, shorter:
		\begin{itemize}
		\item There exists $x$: $\ldots$
	\end{itemize}
	We also use it for:
	\begin{itemize}
		\item For something, $x$: $\ldots$
...
\end{center}
giving 
$$\exists x((Ex\eand Sx)\eand\enot Tx)$$
or $$\exists x(Ex\eand Sx\eand\enot Tx)$$
...
Here the $\exists x$ is at the beginning of the sentence, but it is not the main logical operator. The main logical operator is $\eand$. 
...
\begin{samplesent}
	...
	\item[] For all $x$, if \underline{$x$ is an animal} then \underline{Charlie likes $x$}. 
\end{samplesent}
So we give:
\begin{center}
	\begin{forest}
...
Yes:
\begin{samplesent}
	\item[] There exists $x$: \underline{$x$ is a vet} and \underline{every animal likes $x$}.
\end{samplesent}
...
\begin{center}
	...
	\end{forest}
\end{center}
`$x$ is a vet' can be symbolised as $Vx$. How should we symbolise `every animal likes $x$'? 
%\begin{samplesent}
...
\end{samplesent}
Now, we have already got variable $x$ in play -- it's the vet -- so we should choose a different variable, such as $y$. 
\begin{samplesent}
...
\begin{center}
	\begin{forest}
		[every animal likes $x$\\{For all $y$, if \ldots then \ldots}\\$\forall y(\ldots\eif\ldots)$
		[$y$ is an animal\\$Ay$]
		[$x$ likes $y$\\$Lyx$]
		]
	\end{forest}
...
So we give: $$\exists x(Vx\eand\forall y(Ay\eif Lyx))$$
...
\begin{samplesent}
	\item [] There exists something which is an animal and which $x$ likes.
\end{samplesent}
But again, $x$ is our vet; so we need to use a different variable, lets use $y$. 
\begin{samplesent}
...
Again we can manage to break it down step by step:
%\begin{center}
%	\begin{forest}
...
\begin{samplesent}
	\item Every vet who likes some animal which is not calm likes every calm animal.
\end{samplesent}

\begin{center}
	\begin{forest}
		[Every vet who likes some animal likes every calm animal.\\
		For all $x$:   if \ldots then \ldots\\
		$\forall x(\ldots\eif \ldots)$
		[$x$ is vet who likes some animal\\\ldots and \ldots\\$\ldots\eand\ldots$
		[$x$ is a vet\\$Vx$]
		[$x$ likes some animal\\there exists $y$:\ldots and \ldots\\$\exists y(\ldots\eand \ldots)$
		[$y$ is an animal\\$Ay$]
		[$x$ likes $y$\\$Lxy$]]
		]
		[$x$ likes every calm animal\\For all $y$ if \ldots then \ldots\\$\forall y(\ldots\eif\ldots)$
		[$y$ is a calm animal\\$Cy\eand Ay$]
		[$x$ likes $y$\\$Lxy$]
		]
		]
	\end{forest}
\end{center}
giving: $$\forall x(Vx\eand \exists y (Ay\eand Lxy)\eif \forall y(Cy\eand Ay\eif Lxy))$$
When evaluating this, the $y$ that appears on the left hand side, in the scope of the $\exists y$ quantifier, is evaluated entirely separately from the $y$ in the scope of the $\forall y$ quantifier. 
...
Let us now return to more carefully see the syntax of the language of FOL. How to read a sentence? Where do brackets go? 
...
%	\begin{tabular}{ll}
%		&How to read:\\\Tstrut
%	$\forall x\ldots$ & For all $x$: $\ldots$\\
%	$\exists x\ldots$ & There exists $x$: $\ldots$\\
%\end{tabular}
%\end{center}
...
We have now informally introduced the basic building blocks of sentences of FOL. We will now carefully introduce what it is to be a sentence of FOL. This is important to, for example, know precisely what is allowed in writing expressions of FOL and where brackets are to be added or not. 
...
\begin{definition}
	The vocabulary of FOL is as follows: 
\begin{description}
\item[Predicates] $A,B,C,\ldots,W$,\\ or with subscripts, as needed: $A_1, W_2,A_{25},J_{375},\ldots$.\footnote{Each predicate will have a number of \emph{places} associated with it. We should thus really introduce:\begin{description}
\item [Zero-Place Predicates = Atomic sentences of TFL]$A^0,B^0,\ldots,W^0$,
with subscripts, as needed: $A_1^0, W_2^0,A_{25}^0,J_{375}^0,\ldots$
\item [One-Place Predicates]$A^1,B^1,\ldots,W^1$,
with subscripts, as needed: $A_1^1, W_2^1,A_{25}^1,J_{375}^1,\ldots$
\item [Two-Place Predicates]$A^2,B^2,\ldots,W^2$,
with subscripts, as needed: $A_1^2, W_2^2,A_{25}^2,J_{375}^2,\ldots$
\item [Three-Place Predicates]$A^3,B^3,\ldots,W^3$,
with subscripts, as needed: $A_1^3, W_2^3,A_{25}^3,J_{375}^3,\ldots$
\item etc. We drop the superscripts for ease.
\end{description}}
\item[Names] $a,b,c,\ldots, s, t$, \\
or with subscripts $a_1, b_{224}, h_7, m_{32},\ldots$
\item[Variables] $x,y,z$, \\
or with subscripts $x_1, y_1, z_1, x_2,\ldots$. \\
$u,v,w$ may also be used.
...
\end{definition}
\begin{definition}
	An \define{expression of FOL} is any string of symbols of FOL. Take any of the symbols of FOL and write them down, in any order, and you have an expression.
\end{definition}
...
\section{Formulas}
In \cref{sec:TFLSentences}, we went straight from the presentation of the vocabulary of TFL to the definition of a sentence of TFL. In FOL, we will have to go via an intermediary stage, that is, we first need to introduce \define{formulas} of FOL. 

During our symbolisation strategy we have stages like symbolising ``$x$ is happy'' or ``$x$ likes someone''. These are not the sorts of things that are true or false outright. They are not sentences of English. To determine if they are true or false, not only do you need to know what the world is like (be given a model), you also need a specification of who $x$ is. 

These are symbolised in FOL as $H\yellowbox{x}$ or $\exists y L\yellowbox{x}y$, and the variable $x$ is unbound. To know whether it is true, you need to know who $x$ is. These are not \emph{sentences} of FOL, but they are well-formed \emph{formulas}. Formulas are the sorts of things that form sentences when you put quantifiers in front of them. Sentences are special kinds of formulas, those which have no so-called free-variables. No additional specification of who $x$ is is needed in $\forall x\, Hx$. It is a sentence. 

What counts as a formula of FOL is given by an inductive definition similar to that which we gave for TFL. 
...
\begin{quote}
	{ If $\metaPredicate$ is an $n$-place predicate and $\metaterm_1,\ldots\metaterm_n$ are either variables or names, then $\metaPredicate\metaterm_1\ldots\metaterm_n$ is an \define{atomic formula}.\footnotemark}
...
%We now give the inductive definition of what it is to be a formula of FOL.
	\begin{definition}\label{FOLformula}A \define{formula of FOL} is defined inductively as follows:
	\begin{enumerate}
	...
	As in TFL, we can construct more complex formulas from simple ones. Like in TFL, we will write outer brackets in invisible ink, which here is displayed in gray. Such outer brackets can be dropped when the formula is written by itself but must be included once it is embedded in a further formula. 
	
\begin{center}
	\begin{forest}
	[$\almostinvisibleopenbracket\ldots \eif \ldots\almostinvisibleclosebracket$
		[$\enot \ldots$
...
\end{center}
	...
	We can also display the construction of formulas by with braces, where we also indicate the main connective of the braced part of the sentence with a red dot. 
	
		\begin{center}
		\(\tikzmarknode{lpar}{\almostinvisibleopenbracket}%
		\tikzmarknode{not}{\enot}%
		\tikzmarknode{inner_lpar}{(}%
		\tikzmarknode{Dx}{Dx}%
		\tikzmarknode{and}{\eand}%
		\tikzmarknode{Db}{Db}%
		\tikzmarknode{inner_rpar}{)}%
		\tikzmarknode{if}{\eif}%
		\tikzmarknode{Lxy}{Lxy}%
		\tikzmarknode{rpar}{\almostinvisibleclosebracket}%
		\rule[-4\scopebracedepthunit]{0pt}{0pt}\)
		\end{center}
		\begin{tikzpicture}[remember picture,overlay]
			\scopebrace{Dx}
			\scopebrace{Db}
			\scopebrace{Lxy}
			\scopefitbracewithcoordinate{inner_lpar}{inner_rpar}{and}[2]
			\scopefitbracewithcoordinate{not}{inner_rpar}{not}[3]
			\scopefitbracewithcoordinate{lpar}{rpar}{if}[4]
		\end{tikzpicture}
		This is exactly as in the case of TFL, the only difference is that the smallest components of the formula have more structure to them: they are predicates applied to names or variables rather than simply the single atomic sentences that we had in TFL.


FOL also allows us to add quantifiers with variables in the front of a formula we have. We can thus construct $$\forall x(\enot(Dx\eand Db)\eif Lxy)$$ in this process:
\begin{center}
	\begin{forest}
	[$\forall x\ldots$
	[$\almostinvisibleopenbracket\ldots \eif \ldots\almostinvisibleclosebracket$
	...
	\end{forest}
\end{center}
Or indicating this with scope braces:
		\[		\tikzmarknode{allx}{\forall x}%
		\tikzmarknode{lpar}{(}%
		\tikzmarknode{not}{\enot}%
		\tikzmarknode{inner_lpar}{(}%
		\tikzmarknode{Dx}{Dx}%
		...
		\begin{tikzpicture}[remember picture,overlay]
			...
			\scopefitbracewithcoordinate{not}{inner_rpar}{not}[3]
			\scopefitbracewithcoordinate{lpar}{rpar}{if}[4]
			\scopefitbracewithcoordinate{allx}{rpar}{allx}[5]
		\end{tikzpicture}
		

Note that the quantifier phrase, $\forall x$, does not itself add any new brackets. Brackets are only introduced with the binary connectives: $\eand,\eor,\eif$. 

If our sentence only includes quantifiers and negations and none of these binary connectives, then there are no brackets: 
...
\begin{center}
	...
	\end{forest}
\end{center}
Which is also usefully indicated with scope brackets: 
\begin{center}
	\(\tikzmarknode{outer_not}{\enot}%
	\tikzmarknode{allx}{\forall x}%
	\tikzmarknode{inner_not}{\enot}%
	\tikzmarknode{Hx}{Hx}%
	\rule[-4\scopebracedepthunit]{0pt}{0pt}\)
\end{center}
\begin{tikzpicture}[remember picture,overlay]
	\scopebrace{Hx}
	\scopefitbracewithcoordinate{inner_not}{Hx}{inner_not}[2]
	\scopefitbracewithcoordinate{allx}{Hx}{allx}[3]
	\scopefitbracewithcoordinate{outer_not}{Hx}{outer_not}[4]
\end{tikzpicture}
This is a sentence symbolising `Not everything is not happy'. 

We can also add existential quantifiers, and mix up the variables in use, for example: 
	\begin{center}
	\begin{forest}
		[$\exists z\ldots$
			[$\almostinvisibleopenbracket\ldots\eand\ldots\almostinvisibleclosebracket$
...
Or indicating this with scope braces:
\begin{center}
	\(\tikzmarknode{existsz}{\exists z}%
	\tikzmarknode{lpar}{(}%
	\tikzmarknode{Hz}{Hz}%
	\tikzmarknode{and}{\eand}%
	\tikzmarknode{ally}{\forall y}%
	\tikzmarknode{inner_lpar}{(}%
	\tikzmarknode{Ryz}{Ryz}%
	\tikzmarknode{inner_and}{\eand}%
	\tikzmarknode{existsx}{\exists x}%
	\tikzmarknode{Qx}{Qx}%
	\tikzmarknode{inner_rpar}{)}%
	\tikzmarknode{rpar}{)}%
	\rule[-6\scopebracedepthunit]{0pt}{0pt}\)
\end{center}
\begin{tikzpicture}[remember picture,overlay]
	\scopebrace{Hz}
	\scopebrace{Ryz}
	\scopebrace{Qx}
	\scopefitbracewithcoordinate{existsx}{Qx}{existsx}[2]
	\scopefitbracewithcoordinate{inner_lpar}{inner_rpar}{inner_and}[3]
	\scopefitbracewithcoordinate{ally}{inner_rpar}{ally}[4]
	\scopefitbracewithcoordinate{lpar}{rpar}{and}[5]
	\scopefitbracewithcoordinate{existsz}{rpar}{existsz}[6]
\end{tikzpicture}
If no binary connectives are used, the quantifiers just get stacked on top of one another without no brackets.
	\begin{center}
	\begin{forest}
		[$\exists z\ldots$
			[$\forall y\ldots$
...
\begin{center}
	...
	\tikzmarknode{Qx}{Qx}%
	\tikzmarknode{rpar}{)}%
	\rule[-5\scopebracedepthunit]{0pt}{0pt}\)
\end{center}
\begin{tikzpicture}[remember picture,overlay]
	\scopebrace{Ryz}
	\scopebrace{Qx}
	\scopefitbracewithcoordinate{existsx}{Qx}{existsx}[2]
	\scopefitbracewithcoordinate{lpar}{rpar}{and}[3]
	\scopefitbracewithcoordinate{ally}{rpar}{ally}[4]
	\scopefitbracewithcoordinate{existsz}{rpar}{existsz}[5]
\end{tikzpicture}
...
% In fact, we can now give a formal definition of scope which incorporates the definition of the scope of a quantifier. Here we follow the case of TFL, though we note that a logical operator can be either a connective or a quantifier:
	\begin{definition}
The \define{main logical operator} of a formula is the last logical operator ($\enot$, $\eand$, $\eor$, $\eif$, $\exists \metav$ or $\forall \metav$) that was introduced in the construction of the formula.

\bigskip

		The \define{scope} of an occurrence of a logical operator in a sentence is the sub-formula for which that operator is the main logical operator.
	\end{definition}
...
\[
\enot \overbrace{\exists z\bigl(\underbrace{\exists y(\overbrace{\forall x Rxy}^{\textcolor{green}{\text{scope of }\forall x}}\eand Ryz)}_{\textcolor{blue}{\text{scope of }\exists y}}\eand \underbrace{\forall y Rzy}_{\textcolor{red}{\text{scope of }\forall y}}\bigr)}^{\textcolor{teal}{\text{scope of }\exists z}}
\]

Since there is no interaction between the scope of $\exists y$ and $\forall y$ the same variable $y$ can be used in both places without confusion or danger, the evaluation is done entirely separately. 
...
\section{Sentences}
Consider this formula, which we used to symbolise the English sentence `some vet likes every animal'. $$\exists x (Vx\eand\forall y(Ay\eif Lxy))$$
Indicating the scope of the quantifiers we get: 
\[
\overbrace{
  \customcoloredbox[red!10]{\exists x}
  \bigl(
    Vx
    \eand
    \underbrace{
      \customcoloredbox[blue!10]{\forall y}
      \bigl(
        Ay
        \eif
        Lxy
      \bigr)
    }_{\textcolor{blue}{\text{scope of }\forall y}}
  \bigr)
}^{\textcolor{red}{\text{scope of }\exists x}}
\]
Looking at this, we can see that every occurrence of a variable occurs within the scope of a quantifier which binds that variable: 
\[
\overbrace{
  \customcoloredbox[red!10]{\exists x}
  \bigl(
    V
    \customcoloredbox[red!10][narrow]{x}
    \eand
    \underbrace{
      \customcoloredbox[blue!10]{\forall y}
      \bigl(
        A
        \customcoloredbox[blue!10][narrow]{y}
        \eif
        L
        \customcoloredbox[red!10][narrow]{x}
        \customcoloredbox[blue!10][narrow]{y}
...
Compare that to this formula:
$$\forall y(Ay\eif L\yellowbox{x}y)$$
This symbolises something of pseudo-English: `Every animal is liked by $x$'. But who does `$x$' refer to? 
There is an occurrence of a variable which is not in the scope of a quantifier which binds it. Let's see some more examples:
\begin{gather}
H\yellowbox{x}\eand \enot P\yellowbox{x}\\ 
\exists y(Ly\yellowbox{x}\eand \enot L\yellowbox{x}y)\\
\underbrace{\forall x Hx}_{\text{scope of }\forall x}\eif \exists y R\customcoloredbox[yellow][narrow]{x}y
\end{gather}
In each case, there are some variable occurrences which are not in the scope of a quantifier binding them: the ones highlighted in yellow. We will call this being free. 
	\begin{highlighted}
		An occurrence of a variable \metav is \define{bound} if it is within the scope of either $\forall\metav$ or $\exists\metav$.

		\bigskip

		An occurrence of a variable \metav is \define{free} if it is not within the scope of either $\forall\metav$ or $\exists\metav$.
	\end{highlighted}
...
Consider this example and note that the highlighted occurrence of the variable occurs free. Even though there is a quantifier over $x$ in the sentence, it is not within the scope of that quantifier.
\begin{equation}
\underbrace{\forall x Hx}_{\text{scope of }\forall x}\eif \exists y R\customcoloredbox[yellow][narrow]{x}y
\end{equation}

If a formula has a free variable, the formula doesn't get a fixed truth value in a model. To know whether it is true, you need to specify who $x$ is. This won't be fixed by a model because it is important for the semantics that who $x$ refers to can vary. What this means is that it won't get a truth value on the model without the additional information given by a variable assignment specifying who $x$ is. 
...
%\begin{definition}An occurrance of a variable being \define{free} is defined as follows:
%	\begin{itemize}
...
%	$$\overbrace{\forall x(Ex \eor Dy)}^{\text{scope of $\forall x$}} \eif \exists z(E\customcoloredbox[yellow][narrow]{x} \eif Lz\customcoloredbox[yellow][narrow]{x} )$$
...
It is sentences which are given truth values on a model. For formulas which still have free variables, they are only given truth values on a model with additional information specified by a variable assignment. 
...
We will adopt the same notational conventions governing brackets that we did for TFL (see \cref{sec:TFLBrackets}):  we may omit the outermost brackets of a formula.
