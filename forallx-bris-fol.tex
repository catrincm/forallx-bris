...
In this case: 
\begin{samplesent}
\item[]	For all $x$: if $x$ is an enthusiastic student, then $x$ is not tired. 
\end{samplesent}
...
When using the existential quantifier, $\exists$, we typically find it with $\eand$, unlike the universal quantifier, $\forall$, which typically comes paired with $\eif$. In WHERE we will show why this is right, for now, we simply motivate it with paraphrasing. 
...
\begin{center}
	\begin{forest}
		[$x$ likes every animal\\$\forall y(\ldots\eif\ldots)$
		[$y$ is an animal\\$Ay$]
		[$x$ likes $y$\\$Lxy$]
		]
	\end{forest}
...
\begin{center}
	\begin{forest}
		...
		$\forall x(\ldots\eif \ldots)$
		[$x$ is a vet\\$Vx$]
		[$x$ likes some animal\\$\exists y(\ldots\eand\ldots)$
		[$y$ is an animal\\$Ay$]
		[$x$ likes $y$\\$Lxy$]
		]
		]
...
\begin{center}
	\begin{forest}
		...
		$\forall x(\ldots\eif \ldots)$
		[$x$ is an animal\\$Ax$]
		[$x$ is liked by some vet\\$\exists y(\ldots\eand\ldots)$
		[$y$ is a vet\\$Vy$]
		[$x$ likes $y$\\$Lxy$]
		]
		]
...
\begin{center}
	\begin{forest}
		...
		[$x$ likes all calm animals\\$\forall y(\ldots\eif\ldots)$
