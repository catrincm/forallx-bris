...
\factoidbox{A model 
has three constituent parts. It:	\begin{itemize}
		\item specifies a non-empty set as the domain $\domain$;
		\item specifies the denotation of the names and predicates as objects and relations on the domain
		\begin{itemize}
...
A model thus provides us with all the necessary information for interpreting the predicates and names, as well as, information about the range of the quantifiers. Importantly, we are not guaranteed that the denotation of an $n$-place predicate is nonempty, i.e., it is possible that $\emptyset$ is assigned to a predicate. In general, a convenient way of specifying a model is to supplement a given symbolization key (cf.~ Part~\ref{ch.FOL}) with the relevant model of the names and predicates.
...
Suppose we want to consider just a single two-place predicate, $Rxy$. 
And we wish to consider the following model:
\begin{interp}
	\item[\domain]$\Set{1,2,3,4}$
	\item[\denote{R}]$\Set{
		\ntuple{1, 1}
...
Then we can represent it just by drawing an arrow between two objects, and stipulate that $Rxy$ is to hold of $x$ and $y$ just in case there is an arrow running from $x$ to $y$ in our diagram. As an example, we might offer:
\begin{center}
\begin{tikzpicture}
\node (atom1) at (0,2) {1};
\node (atom2) at (2,2) {2};
...
\end{tikzpicture}
\end{center}
With this context, we can then instead describe this same model as
\begin{interp}
	\item[\domain]$\Set{x\given x \text{ is a number in the above diagram}}$
	\item[\denote{R}]$\Set{\sequence{x,y}\given \text{ there is an arrow from $x$ to $y$ in the above diagram}}$
\end{interp}

What is important about a model is that it fully specifies what this is true of. This is only appropriate when it is abundantly clear. 
...
But we defined what a sentence was by first specifying what a formula is. Formulas like $Ex$ aren't the sorts of things that are true or false i models. Only sentences are true or false. But if we provide extra information we can determine the truth of $Ex$: we need to specify what object $x$ stands for. This is done by using a variable assignment: 
...
\factoidbox{A \define{term} is either a name or a variable. 
	...
	\begin{itemize}
		\item If $t$ is a name, then $\denote{t}^\alpha$ is just $\denote{t}$, the object of the domain assigned to the FOL-name $t$ by the model.
...
With this final bit of terminology out of way we can state the truth conditions for atomic formuals. Let \meta{R} be an $n$-place predicate and $\metaterm_1,\,\metaterm_2\ldots\metaterm_{n}$ terms. Then
	...
	\factoidbox{
		$\meta{R}\metaterm_1\metaterm_2\ldots\metaterm_{n}$ is true in a model under a variable assignment $\alpha$ \textbf{iff}\\
...
We saw in \S\ref{s:FOLSentences} that FOL formulas can be built up from simpler ones using the truth-functional connectives that were familiar from TFL. The rules governing these truth-functional connectives are \emph{exactly} the same as they were when we considered TFL. Here they are:
	\factoidbox{
	$\enot \metaX$ is true in a model under a variable assignment \textbf{iff} \\$\metaX$ is false in that model under that variable assignment

		\
		\\$(\metaX \eand \metaY)$ is true in a model under a variable assignment \textbf{iff}\\ both $\metaX$ and $\metaY$ is true in that model under that variable assignment

		\
		\\
		$(\metaX \eor \metaY)$ is true in a model under a variable assignment \textbf{iff}\\ either $\metaX$ is true or $\metaY$ is true in that model under that variable assignment

		\
		\\$(\metaX \eif \metaY)$ is true in a model under a variable assignment \textbf{iff}\\ either $\metaX$ is false or $\metaY$ is true in that model under that variable assignment

		%\
		%\\$\metaX \eiff \metaY$ is true in a model under a variable assignment \textbf{iff} \\$\metaX$ has the same truth value as $\metaY$ in that model under that variable assignment
	}
...
More generally, if we have a quantifier like $\exists x$ or $\forall x$ we ignore whatever our original variable assignment told us about $x$ but consider suitable $x$-variants. So when all our variables are bound by quantifiers, all the original components of our variable assignment are ignored. To summarise: Sentences are simply true or false i models, variable assignments don't matter.

 Now for our formal definition:
\factoidbox{
$\forall \metav\metaX$ is true in the model under a variable assignment $\alpha$\\
\textbf{iff} $\metaX$ is true in the model under the variable assignment $\varvariant{\alpha}{\metav}{o}$ for every $o\in \domain$.

\bigskip

$\exists \metav\metaX$ is true in the model under a variable assignment $\alpha$\\
\textbf{iff} $\metaX$ is true in the model under the variable assignment $\varvariant{\alpha}{\metav}{o}$ for some $o\in \domain$.
}
