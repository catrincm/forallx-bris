...
\chapter[FOL semantics part 1]{FOL semantics Part 1 - Monadic sentences}
\label{ch:FOL semantics informal - monadic}
We have discussed the syntax of the language of first-order language and, using our intuitive understanding of the vocabulary of the language, proposed a guideline for symbolizing English sentences in the language of FOL. We now turn to the semantics, that is, we now want to spell out truth-conditions for sentences of the language. 
...
If we have symbolised 
\begin{samplesent}
	\item Alice likes some philosopher.
\end{samplesent}
as $\exists x(Lax\eand Px)$
To evaluate whether it is true, one needs to work out who Alice is, who she likes, and whether they are a philosopher. 
...
Is `$Ca$' true on this model? To answer this, we simply ask if the object the model assigns to `$a$', in this case the person/image {Alice}, is a member of the set that the model assigns to `$C$', in this case $\{\text{Alice, Cathy}\}$. That is, `$Ca$' is true on $\model$ iff $$\denote{a}_\model\in\denote{C}_\model.$$ On this model, that is the question of whether $\text{{Alice}}\in\Set{\text{{Alice, Cathy}}}$. Since she is, it is true.



Is $Ha$ true on this model? It is true on $\model$ iff $$\denote{a}_\model\in\denote{H}_\model.$$ That is, iff 
$\text{{Alice}}\in\Set{\text{{Bob}}}$
Since $\text{{Alice}}\notin\Set{\text{{Bob}}}$, $Ha$ is false on $\model$. 
...
\[\begin{array}{c|def}
	&Ha&\eif &Ca\\
	&Ha&&Ca\\\hline
	\model &\false&\TTbf{\true}&\cdot\\\hline
\end{array}\]
...
\[\begin{array}{c|def}
	&Ha&\eand &Ca\\
	&Ha&&Ca\\\hline
	\model &\false&\TTbf{\false}&\cdot\\\hline
\end{array}\]
...
\begin{itemize}
	\item $\forall x\metaX$ is true on $\model$ iff for every $o\in\domain_\model$, $\metaX$ is true on $\model$ under the variable assignment $x\mapsto o$. 
\end{itemize}
...
\begin{itemize}
	\item $\exists x\metaX$ is true on $\model$ iff for at least one $o\in\domain_\model$, $\metaX$ is true on $\model$ under the variable assignment $x\mapsto o$. 
\end{itemize}
...
\[\begin{array}{c|deeeeef}
	...
	&&	\text{Bo}&&\true&\false&\false\\\varassigncline{3-8}
	&&	\text{Ca}&&\false&\false&.\\\varassigncline{3-8}
	&&	\text{De}&&\false&\false&.\\\varassigncline{3-8}
\end{array}\]
...
\[\begin{array}{c|deeeeeef}
	...
	%	&&& \text{Al}&&\false&\true&.\\\varassigncline{4-9} 
	&&&	\text{Bo}&&\true&\false&\false\\\varassigncline{4-9} 
	%	&&&	\text{Ca}&&\false&\true&.\\\varassigncline{4-9} 
	%	&&&	\text{De}&&\false&\true&.\\\varassigncline{4-9} 
\end{array}\]
...
\[\begin{array}{c|deeeeeeeeef}	&\forall x &Hx&{\eand} &	{\forall} &{x} &(&Hx&\eif &Cx&)\\\hline
	\model &.&&\false&\TTbf{\false} &	\\\varassigncline{6-11} 
	&&&&	&\text{Bo}&&\true&\false&\false\\\varassigncline{6-11} 
\end{array}\]
...
iff there is no model in which all of $\metaX_1, \metaX_2, \ldots, \metaX_n$ are true and in which \metaZ is false. }
%Derivatively,
%	$$\entails\metaX$$
...
\chapter[Symbolising `None']{More symbolisation -- common quantifier phrases like `none'}
...
\begin{practiceproblems}
	...
	\begin{ekey}
		...
		\item[\atom{L}{xy}] \gap{x} is larger than \gap{y}
		\item[r] Rave
		\item[h] Shane
		\item[d] Daisy
	\end{ekey}
	...
	\begin{problemslist}
		...
		\item[] \myanswer{$\forall x (\atom{D}{x} \eif \exists y(\atom{D}{y} \eand \atom{L}{xy}))$\\
		...
		\item[] \myanswer{$\forall x (\forall y (\atom{D}{y} \eif \atom{L}{xy}) \eif \enot \atom{S}{x})$\\
	...
	\begin{ekey}
		...
		\item[\atom{B}{xy}] \gap{x} is better than \gap{y}.
	\end{ekey}
	\begin{problemslist}
...
Since $\sequence{\text{Alice, Bob}}\notin\Set{\sequence{\text{Alice, Denny}},\sequence{\text{Alice, Cathy}}}$, `$Aab$' is false on $\model$. 
...
So, $\exists x Aax$ is true. There is some choice of an object for $x$ which makes $Aax$ true: there someone for whom there is an arrow from Alice to them. For example, $x\mapsto$Cathy makes $Aax$ true on $\model$; so $\exists x Aax$ is true on $\model$. 
...
\[\begin{array}{c|deeeeeeeeeeeeeeef}
	...
	&&&&&&& \text{Al}&&\false&\true&.&\\\varassigncline{8-13}
	&&&&&&& \text{Bo}&&\true&\false&\false\\\varassigncline{8-13}
	&&&&&&& \text{Ca}&&\false&\true&.&\\\varassigncline{8-13}
	&&&&&&& \text{De}&&\true&\true&\true\\\varassigncline{8-13}
	\varassigncline{3-14} 
	&&\text{Bo}&&\false&\false&.\\\varassigncline{3-14} 
	&&\text{Ca}&&\true&\false&\false\\
	&&&&&&& \text{Al}&&\false&\true&.&\\\varassigncline{8-13}
	&&&&&&& \text{Bo}&&\true&\false&\false\\\varassigncline{8-13}
	&&&&&&& \text{Ca}&&\false&\true&.&\\\varassigncline{8-13}
...
\begin{itemize}
	\item $Ca$ is true on $\model$ iff $$\denote{a}_\model\in\denote{C}_\model.$$ 
	\item $Lab$ is true on $\model$ iff $$\sequence{\denote{a}_\model,\denote{b}_\model}\in\denote{L}_\model.$$
\end{itemize}

But we also need to describe a clause for atomic formulas which contain variables. 
\begin{itemize}
	\item $Cx$ is true on $\model$ under variable assignment $\alpha$ iff $$\alpha(x)\in\denote{C}_\model$$
	\item
