...
In \ref{s:TruthFOL}, we introduced the idea of a model but we did not include relations. Now we can extend it to give the proper definition of a  \define{model}. Sentences of FOL will be true or false \emph{on a model}.
...
For our third example, we'll mix things up a bit. In \S\ref{s:model}, we described how we can present some models using diagrams. For example:
\begin{center}
\begin{tikzpicture}
\node (atom1) at (0,2) {1};
\node (atom2) at (2,2) {2};
