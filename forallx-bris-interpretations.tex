...
Summing up, two place predicates such as `\gap{x}is bigger than\gap{y}' or `\gap{x}loves\gap{y}' apply to ordered pairs, that is, pairs of object in a specific order. For example, `\gap{x}is bigger than\gap{y}' applies to the ordered pair $\langle\text{Tom, Jerry}\rangle$. The \define{ordered pair} $\langle\text{Tom, Jerry}\rangle$ is different to the set $\{\text{Tom, Jerry}\}$, but also different to the ordered pair $\langle\text{Jerry, Tom}\rangle$. $\langle\text{Tom, Jerry}\rangle$ tells us that Tom is the first object and Jerry is the second object in contrast $\langle\text{Jerry, Tom}\rangle$ where it is the other way around and the set $\{\text{Tom, Jerry}\}$ for which the order does not matter (recall that $\{\text{Tom, Jerry}\}=\{\text{Jerry, Tom}\}$). 
...
\factoidbox{The role of an interpretation is threefold. An interpretation 
	\begin{ebullet}
		\item specifies a non-empty set as the domain $D$;
		...
		\end{itemize}
		\end{ebullet}
	The interpretation of a FOL-expression $\eta$ is denoted by $\|\eta\|$:
		\begin{ebullet}
		\item if $c$ is a FOL-name, $\|c\|$ denotes the object of the domain assigned to `$c$' by the interpretation and thus $\|c\|\in D$.
		\item if $P$ is the $n$-place predicate, then we write $\|P\|$ to denote the $n$-ary relation assigned to $P$ by the interpretation.
	\end{ebullet}}
...
\factoidbox{A \define{variable assignment} assigns to each variable an object the variable stands for.}
...
\factoidbox{A \define{term} is either a name or a variable. If $t$ is a term then the \define{denotation of the term} in the interpretation under an variable assignment $\alpha$, $\|t\|^\alpha$ is the object pick out by the interpretation, if $t$ is a name and $\alpha(t)$ otherwise:
\begin{align*}\|t\|^\alpha=\begin{cases}\|t\|,&\text{ if $t$ is name;}\\\alpha(t),&\text{  if $t$ is a variable.}\end{cases}\end{align*} }
...
With this final bit of terminology out of way we can state the truth conditions for atomic formuals. Let \meta{R} be an $n$-place predicate and $\metaterm_1,\,\metaterm_2\ldots\metaterm_{n}$ terms. Then
	...
	\factoidbox{
		...
		$$\langle\|t_1\|^\alpha,\ldots,\|t_n\|^\alpha\rangle\in\|\meta{R}\|$$
	}
...
% To check whether We need to find some number $n$ such that every number $m$: $\langle n,m\rangle\in\|R\|$, i.e., there is an arrow from $n$ to every number $m$. Let's chose the number 3 for $x$ ($x\mapsto 3$). Let's call this particular variable assignment $\alpha$. $\alpha$ also assigns objects to all other variables of the language (we just don't care which ones) Is $\forall y Rxy$ true under the variable assignment $\alpha$? Intuitively, the answer is yes as there is an arrow from 3 to 1, 2, 3, and 4, that is, all objects of the domain.
% 
...
\textbf{iff} $\metaX$ is true in the interpretation under the variable assignment $\varvariant{\alpha}{\metav}{o}$ for every $o\in D$.

\bigskip

$\exists \metav\metaX$ is true in the interpretation under a variable assignment $\alpha$\\
\textbf{iff} $\metaX$ is true in the interpretation under the variable assignment $\varvariant{\alpha}{\metav}{o}$ for some $o\in D$.
}
...
To show that this is invalid, we must make the premise true and the conclusion false. The conclusion is a conditional, so to make it false, the antecedent must be true and the consequent must be false. Clearly, our domain must contain two objects. Let's try:
	\begin{ekey}
		...
		\item[Gx] \{o\,|\,o\text{ hated communism}\} (=$\{\emph{Ludwig von Mises}\}$)
