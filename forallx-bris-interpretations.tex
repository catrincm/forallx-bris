...
%
%A universal sentence, $\forall x\metaX$ is true on a model when $\metaX$ is true under every choice of an object from the domain as the referent of $x$:
\[\begin{array}{c|def}
...
In \cref{sec:Truth FOL monadic}, we introduced the idea of a model but we did not include relations. Now we can extend it to give the proper definition of a  \define{model}. Sentences of FOL will be true or false \emph{on a model}.
...
\chapter{Truth on a model, more carefully}
Sentences of FOL are true or false on a model. 
...
\begin{itemize}
	...
	\item
	$Lxy$ is true on a model $\model$ under a variable assignment $\alpha$ iff $$\sequence{\alpha(x),\alpha(y)}\in\denote{L}_\model$$
\item
$Lxb$ is true on a model $\model$ under a variable assignment $\alpha$ iff $$\sequence{\alpha(x),\denote{b}_\model}\in\denote{L}_\model$$
\end{itemize}
...
%
%\section{Models}\label{sec:model}\todo{call it ``model''?}
%We defined a \define{valuation} in TFL as any assignment of truth and falsity to atomic sentences. In FOL, we are going to define an \define{model}. Sentences of FOL will be true or false \emph{on a model}.
%\factoidbox{A model $\model$ provides \begin{itemize}
%		\item specifies a non-empty set as the domain, $\domain$;
%		\item specifies an interpretation of the FOL-names and FOL-predicates. 
%		\begin{itemize}
