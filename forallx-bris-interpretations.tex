...
%The truth of atomic \emph{sentences} should be fairly straightforward. The sentence $Pa$ should be true just in case $Px$ is true of $a$. Given our go-to model, this is true iff Aristotle is a philosopher. Aristotle is a philosopher. So the sentence is true. Equally, $Pb$ is false on our go-to model.
...
This is just another presentation of the truth rules we gave for the connectives in TFL; it just does so in a slightly different way. Some examples will probably help to illustrate the idea. On our go-to model:
	\begin{compactlist}
...
%Consider `$\exists x Sx$'.
%\begin{quote}
%	`$\exists x Sx$' true in a model under the variable assignment $\alpha$ \textbf{iff} there exists an object $o$ of the domain, i.e.~$o\in \domain$, such that `$Sx$' is true in the model under the variable assignment $\varvariant{\alpha}{x}{o}$.
%\end{quote}
...
\begin{center}
	\begin{tabular}{c| deeeeeeef}
		...
		\textit{Alice} &
		& \textcolor{gray}{T} & \TTbf{T} & & \textcolor{gray}{F} & T & \textcolor{gray}{T} & & \\
		\textit{Bob} &
		& \textcolor{gray}{T} & \TTbf{T} & & \textcolor{gray}{T} & T & \textcolor{gray}{F} & & \\
		\textit{Cathy} &
		& \textcolor{gray}{F} & \TTbf{T} & & \textcolor{gray}{F} & T & \textcolor{gray}{T} & & \\
		\textit{Denny} &
		& \textcolor{gray}{F} & \TTbf{T} & & \textcolor{gray}{F} & F & \textcolor{gray}{F} & & \\
	\end{tabular}
\end{center}
%\begin{center}
%\begin{tabular}{l|ccccc}
%$x$&$Sx$&$Hx$&$Cx$&$Hx\eor Cx$&$Sx\eif (Hx\eor Cx)$\Bstrut\\\hline\Tstrut
%$\text{\emph{Alice}}$&T&F&T&T&T\\
%$ \text{\emph{Bob}}$&T&T&F&T&T\\
%$ \text{\emph{Cathy}}$&F&F&T&T&T\\
%$ \text{\emph{Denny}}$&F&F&F&F&T
%\end{tabular}
%\end{center}
...
\begin{center}
	\begin{tabular}{c|def}
		$x$ & $Sx$ & $\eif$ & $\exists y Rxy$ \Bstrut\\\hline\Tstrut
		1 & T & ? & ?\\
		2 & F & T & \\
		3 & T & ? & ?\\
		4 & F & T & \\
...
Consider a variable assignment where $x\mapsto 1$. We can find an object for $y$ such that `$Rxy$' is true under that variable assignment, namely, $y\mapsto 2$. Since there is an arrow from 1 to 2, `$Rxy$' is true in the variable assignment $\varvariant{\varvariant{\alpha}{x}{1}}{y}{2}$. Thus `$\exists y Rxy$' is true under the variable assignment $\varvariant{\alpha}{x}{1}$. For $x\mapsto 3$ we can also find an assignment for $y$ such that `$Rxy$' is true (which one?) and thus  `$\exists y Rxy$' is also true under the variable assignment $\varvariant{\alpha}{x}{3}$.
%\begin{center}
%\begin{tabular}{cc|c}
%&&$Rxy$\Bstrut\\\hline\Tstrut
...
\begin{center}
	...
	\end{tabular}
\end{center}
So $\forall x(Sx\eif \exists y Rxy)$ is true. Informally we might say this as: for every number that has a square around it has an arrow going out of it.
...
\section{Working out the truth value}
...
\begin{itemize}
	\item On $M_1$ it is false: $\exists x Cx$ is true so $\enot\exists x Cx$ is false. Also $\forall y(Sy\eor Cy)$ is false ($y\mapsto 4$). So the disjunction is false. 
\item On $M_2$ it is true because $\forall y(Sy\eor Cy)$ is true: $1$ is $C$ and $2$ is $S$. 
\end{itemize}
...
\begin{array}{c|d*{20}{e}f}
	...
	M_1 & ?& ? &  &  &  &  &  &  &  &  &  &  &  &  & & &? &?\\ \varassigncline{4-16}\varassigncline{20-22}
	...
	M_2 & ?& ? &  &  &  &  &  &  &  &  &  &  &  &  & & &? &?\\ \varassigncline{4-16}\varassigncline{20-22}
