...
\chapter{Quick reference}\label{appendix:quickref}
%\pagestyle{plain}
...
\label{appendix:quickref:CharacteristicTTs}
%\begin{center}
...
\section{Symbolisation strategy}\label{appendix:quickref:symbolization}

\begin{enumerate}
	\item Can it be paraphrased in accordance with out symbolisation guide?
	Make sure that the `$\ldots$' are complete (declarative) sentences. 
...
\begin{tabular}{c|l}
	...
	$\ldots_{\metaX}$ if and only if $\ldots_{\metaY}$ & $(\metaX \eif \metaY)\eand(\metaY\eif \metaX)$ \\& \emph{(can be shortened to $\metaX\eiff\metaY$)}\\[.4em]
...
\newpage
\section{Basic deduction rules for TFL}\label{appendix:quickref:basic rules TFL}
...
\subsection*{Rules for connectives - introduction and elimination}
\begin{tabular}{|l|c|c|}\hline
	&Introduction&Elimination\\\hline\Tstrut
	$\eand$&	\begin{fitchproof}
		\have[m]{x}{\metaX}
		\have[n]{y}{\metaY}
	...
	&
	\begin{fitchproof}
		\have[m]{x}{\metaX\eif \metaY}
		\have[n]{y}{\metaX}
		\have[\ ]{xy}{\metaY} \ifE{x,y}
	\end{fitchproof}	\Bstrut\\\hline\Tstrut
	$\enot$&	\begin{fitchproof}
		\open
		\hypo[m]{x}{\metaX}
		\ellipsesline
...
\begin{multicols}{2}
	...
	\begin{fitchproof}
		...
		\have[m]{dna}{ \metaX}\\
		\have[ ]{a}{\enot\enot\metaX}\by{DNI}{dna}
	\end{fitchproof}
...
\begin{tabular}{|l|p{0.42\linewidth}|p{0.42\linewidth}|}
	\hline
	& Introduction & Elimination \\\hline
	
	$\forall$
	&
	\begin{fitchproof}
		\have[m]{a}{\metaX[c/\metav]}
		\have[\ ]{c}{\forall \metav\metaX} \forallI{a}
	\end{fitchproof}
	
	\begin{itemize}
		\item 	\meta{c} must not occur in any premise or undischarged assumption
	\end{itemize}
	&
	
	\begin{fitchproof}
		\have[m]{a}{\forall \metav\metaX}
		\have[\ ]{c}{\metaX[c/\metav]} \forallE{a}
	\end{fitchproof}
	\\\hline
	$\exists$&
	\begin{fitchproof}
		\have[m]{a}{\metaX[\metac/\metav]}
		\have[\ ]{c}{\exists\metav\metaX} \Ei{a}
	\end{fitchproof}
	&
	\begin{fitchproof}
	...
	\begin{itemize}
		\item \meta{c} must be new to the proof: it does not occur in any line before $i$;
		\item 	\meta{c} must not occur in \metaY.
	\end{itemize}\\\hline
\end{tabular}
