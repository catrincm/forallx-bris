...
\begin{samplesent}
\item\label{sent:else1} Pavel owes money to everyone
\end{samplesent}
Let the domain be people; this will allow us to symbolize `everyone' as a universal quantifier. Offering the symbolization key:
	\begin{ekey}
		...
		\item[p] Pavel
	\end{ekey}
we can symbolize \cref{sent:else1} by `$\forall x Opx$'. But this has a (perhaps) odd consequence. It requires that Pavel owes money to every member of the domain (whatever the domain may be). The domain certainly includes Pavel. So this entails that Pavel owes money to himself.

Perhaps we meant to say:
	\begin{samplesent}
		\item\label{sent:else1b} Pavel owes money to everyone \emph{else}
		\item\label{sent:else1c} Pavel owes money to everyone \emph{other than} Pavel
		\item\label{sent:else1d} Pavel owes money to everyone \emph{except} Pavel himself
	\end{samplesent}
...
\begin{samplesent}
\item\label{sent:superman} Clark Kent is Superman.
\end{samplesent}
...
\item[s] Superman
\end{ekey}
This will also be a symbolisations of paraphrases of \cref{sent:superman}:
\begin{samplesent}
\item\label{sent:superman1b} Clark Kent and Superman are the same person.
\item\label{sent:superman1c} Clark Kent is identical to Superman.
\end{samplesent}

Using `$\eid $' we will now be able to symbolise sentences \cref{sent:else1b}--\ref{sent:else1d}. All of these sentences can be  paraphrased as `Everyone who is not Pavel is owed money by Pavel'. Paraphrasing some more, we get: `For all x, if x is not Pavel, then x is owed money by Pavel'. Now that we are armed with our new identity symbol, we can symbolize this as `$\forall x (\enot\, x\eid p \eif Opx)$'.
...
\begin{samplesent}
\item\label{sent:else3} No one besides Pavel owes money to Hikaru.
\item\label{sent:else4} Only Pavel owes Hikaru money.
\end{samplesent}
Let `$h$' name Hikaru. \Cref{sent:else3} can be paraphrased as, `No one who is not Pavel owes money to Hikaru'. This can be symbolized by `$\enot\exists x(\enot\,x \eid p \eand Oxh)$'. Equally, \cref{sent:else3} can be paraphrased as `for all x, if x owes money to Hikaru, then x is Pavel'. It can then be symbolized as `$\forall x (Oxh \eif x \eid p)$'.

\Cref{sent:else4} can be treated similarly, but there is one subtlety here. Do either \cref{sent:else3} or \ref{sent:else4} entail that Pavel himself owes money to Hikaru?
...
\begin{samplesent}
\item\label{sent:atleast1} There is at least one apple
\item\label{sent:atleast2} There are at least two apples
\item\label{sent:atleast3} There are at least three apples
\end{samplesent}
We will use the symbolization key:
	\begin{ekey}
		\item[Ax] \gap{x} is an apple
	\end{ekey}
\Cref{sent:atleast1} does not require identity. It can be adequately symbolized by `$\exists x Ax$': There is an apple; perhaps many, but at least one.

It might be tempting to also symbolize \cref{sent:atleast2} without identity. Yet consider the sentence `$\exists x \exists y(Ax \eand Ay)$'. Roughly, this says that there is some apple $x$ in the domain and some apple $y$ in the domain. Since nothing precludes these from being one and the same apple, this would be true even if there were only one apple. In order to make sure that we are dealing with \emph{different} apples, we need an identity predicate. \Cref{sent:atleast2} needs to say that the two apples that exist are not identical, so it can be symbolized by `$\exists x \exists y((Ax \eand Ay) \eand \enot\, x\eid y)$'.

\Cref{sent:atleast3} requires talking about three different apples. Now we need three existential quantifiers, and we need to make sure that each will pick out something different: `$\exists x \exists y\exists z(((Ax \eand Ay) \eand Az) \eand ((\enot\, x\eid y \eand \enot\, y\eid z) \eand \enot\, x\eid z))$'.

\section{There are at most\ldots}
Now consider these sentences:
\begin{samplesent}
	\item\label{sent:atmost1} There is at most one apple
	\item\label{sent:atmost2} There are at most two apples
\end{samplesent}
\Cref{sent:atmost1} can be paraphrased as, `It is not the case that there are at least \emph{two} apples'. This is just the negation of \cref{sent:atleast2}:
$$\enot \exists x \exists y((Ax \eand Ay) \eand \enot\, x\eid y)$$
But \cref{sent:atmost1} can also be approached in another way. It means that if you pick out an object and it's an apple, and then you pick out an object and it's also an apple, you must have picked out the same object both times. With this in mind, it can be symbolized by
$$\forall x\forall y\bigl((Ax \eand Ay) \eif x\eid y\bigr)$$
The two sentences will turn out to be logically equivalent.

In a similar way, \cref{sent:atmost2} can be approached in two equivalent ways. It can be paraphrased as, `It is not the case that there are \emph{three} or more distinct apples', so we can offer:
...
\begin{samplesent}
\item\label{sent:exactly1} There is exactly one apple.
\item\label{sent:exactly2} There are exactly two apples.
\item\label{sent:exactly3} There are exactly three apples.
\end{samplesent}
\Cref{sent:exactly1} can be paraphrased as, `There is \emph{at least} one apple and there is \emph{at most} one apple'. This is just the conjunction of \cref{sent:atleast1} and \cref{sent:atmost1}. So we can offer:
$$\exists x Ax \eand \forall x\forall y\bigl((Ax \eand Ay) \eif x\eid y\bigr)$$
But it is perhaps more straightforward to paraphrase \cref{sent:exactly1} as, `There is a thing x which is an apple, and everything which is an apple is just x itself'. Thought of in this way, we offer:
$$\exists x\bigl(Ax \eand \forall y(Ay \eif x\eid  y)\bigr)$$
Similarly, \cref{sent:exactly2} may be paraphrased as, `There are \emph{at least} two apples, and there are \emph{at most} two apples'. Thus we could offer
\begin{multline*}
...
\begin{samplesent}
\item\label{sent:exactly2things} There are exactly two things
\item\label{sent:exactly2objects} There are exactly two objects
\end{samplesent}
...
\begin{samplesent}
	\item\label{sent:exept2} Every officer except Pavel owes money to Hikaru.
\end{samplesent}
Symbolize this sentence, using `$\atom{F}{x}$' for `\gap{x} is an officer'.  Are you confident that your symbolization is true if, and only if, \cref{sent:except2} is true?  What happens if every officer owes money to Hikaru, Pavel does not, but Pavel isn't an officer? \myanswer{Most people, including most linguists thinking about `except', read \cref{sent:except2} as entailing all three of the following:
\begin{samplesent}
...
Consider sentences like:
	\begin{samplesent}
		\item\label{sent:traitor1} Nick is the traitor.
		\item\label{sent:traitor2} The traitor went to Cambridge.
		\item\label{sent:traitor3} The traitor is the deputy
	\end{samplesent}
...
A second option would be to use a \emph{new} definite description operator, such as `$\maththe$'. The idea would be to symbolize `the F' as `$\maththe xFx$'; or to symbolize `the G' as `$\maththe xGx$', etc. Expression of the form $\maththe \meta{x} \metaX\meta{x}$ would then behave like names. If we followed this path, then using the following symbolization key:
	\begin{ekey}
		...
		\item[n] Nick
	\end{ekey}
We could symbolize \cref{sent:traitor1} with `$\maththe x Tx \eid  n$', \cref{sent:traitor2} with `$C\maththe xTx$', and \cref{sent:traitor3} with `$\maththe x Tx \eid  \maththe x Dx$'.
...
Using these sorts of techniques, we can now symbolize sentences \cref{sent:traitor1}--\ref{sent:traitor3} without using any new-fangled fancy operator, such as `$\maththe$'.

\Cref{sent:traitor1} is exactly like the examples we have just considered. So we would symbolize it by `$\exists x (Tx \eand \forall y(Ty \eif x \eid y) \eand x \eid n)$'.

\Cref{sent:traitor2} poses no problems either: `$\exists x (Tx \eand \forall y(Ty \eif x \eid y) \eand Cx)$'.

\Cref{sent:traitor3} is a little trickier, because it links two definite descriptions. But, deploying  Russell's analysis, it can be paraphrased by `there is exactly one traitor, x, and there is exactly one deputy, y, and x \eid  y'. So we can symbolize it by:
...
France has no king at present. Now, if we were to introduce a name, `$k$', to name the present King of France, then everything would go wrong: remember from \cref{sec:FOLBuildingBlocks} that a name must always pick out  some object in the domain, and whatever we choose as our domain, it will contain no present kings of France.

Russell's analysis neatly avoids this problem. Russell tells us to treat definite descriptions using predicates and quantifiers, instead of names. Since predicates can be empty (see \cref{sec:MoreMonadic}), this means that no difficulty now arises when the definite description is empty.
...
Indeed, Russell's analysis helpfully highlights two ways to go wrong in a claim involving a definite description. To adapt an example from Stephen Neale (1990),\footnote{Neale, \emph{Descriptions}, 1990, Cambridge: MIT Press.}  suppose Alex claims:
	\begin{samplesent}
		\item\label{sent:kingdate} I am dating the present king of France.
	\end{samplesent}
Using the following symbolization key:
	\begin{ekey}
		\item[a] Alex
	...
	\end{ekey}
\Cref{sent:kingdate} would be symbolized by `$\exists x (\forall y(Ky \eiff  x \eid y) \eand Dax)$'. Now, this can be false in (at least) two ways, corresponding to these two different sentences:
	\begin{samplesent}
		\item\label{sent:outernegation} There is no one who is both the present King of France and  such that he and Alex are dating.
		\item\label{sent:innernegation} There is a unique present King of France, but Alex is not dating him.
	\end{samplesent}
\Cref{sent:outernegation} might be paraphrased by `It is not the case that: the present King of France and Alex are dating'. It will then be symbolized by `$\enot \exists x\bigl((Kx \eand \forall y(Ky \eif  x \eid y)) \eand Dax \bigr)$'. We might call this \emph{outer} negation, since the negation governs the entire sentence. Note that it will be true if there is no present King of France.

\Cref{sent:innernegation} can be symbolized by `$\exists x ((Kx \eand \forall y(Ky \eif x \eid y)) \eand \enot Dax)$'. We might call this \emph{inner} negation, since the negation occurs within the scope of the definite description. Note that its truth requires that there is a present King of France, albeit one who is not dating Alex.
...
Keith Donnellan raised a second sort of worry, which (very roughly) can be brought out by thinking about a case of mistaken identity.\footnote{Keith Donnellan, `Reference and Definite Descriptions', 1966, \emph{Philosophical Review 77}, pp.\ 281--304.} Two men stand in the corner: a very tall man drinking what looks like a gin martini; and a very short man drinking what looks like a pint of water. Seeing them, Malika says:
	\begin{samplesent}
		\item\label{sent:gindrinker} The gin-drinker is very tall!
	\end{samplesent}
Russell's analysis will have us render Malika's sentence as:
	\begin{compactlist}
		\item[\ref{sent:gindrinker}$'$.] There is exactly one gin-drinker [in the corner], and whoever is a gin-drinker [in the corner] is very tall.
	\end{compactlist}
...
We simply do this by adding a new kind of atomic sentence: \begin{highlighted}If $\meta{a}$ and $\meta{b}$ are names, then $\meta{a}\eid \meta{b}$ is an atomic sentence.\end{highlighted}
This is added to the other clauses of what it is to be a sentence as they were given in \cref{sec:FOLSentences}
...
%In \cref{ch:semantics} we described when sentences of FOL were true or false. Whereas for TFL sentences were true or false \emph{on a valuation}, in FOL sentences were true or false \emph{on a model}. For FOL with identity we will also use models, for example: 	\begin{ekey}
...
%	\end{ekey}
%
%$\eid $ will stand for identity, so in our give model $a\eid b$ is false. If we extended this model with
%\begin{ekey}
%\item[c] Aristotle
%\end{ekey} then a sentence $a\eid c$ is true.

Now that we have added the identity symbol to FOL, we simply need to expand our notion of truth from \cref{ch:semantics} to also account for atomic sentences like $a\eid b$. Our clause that we add is:
\begin{highlighted}
$\meta{a}\eid\meta{b}$ is true \textbf{iff} $\meta{a}$ and $\meta{b}$ name the same object in that model.
\end{highlighted}
So on our go-to model from \cref{ch:semantics},
\begin{ekey}
		\item[\text{domain}] all people born before 2000\textsc{ce}
		\item[a] Aristotle
		\item[b] Beyonc\'e
...
\end{ekey} It turns out that The Morning Star is \emph{the same object as} The Evening Star: they are names for Venus. So here we have two names for the same object. We thus have $e\eid m$ is true on this model.
...
\item[c] Carys
\end{ekey}
Remember, to check if $\exists x\metaX(\ldots x\ldots x\ldots)$ is true we first add a new name, let's use $d$, and we see if there is some way of extending the domain so that $\metaX(\ldots d\ldots d\ldots)$ is true. So, let's see if $\exists x \,x\eid a$ is true: we add a new name $d$ and consider the extended model with
\begin{ekey}
\item[d] Alfred
\end{ekey}
Then $d\eid a$ is true on this model. So there is some way of interpreting `$d$' where $d\eid a$ is true; and thus $\exists x \, x\eid a$ is true.

On this model we can also see that $\forall x(x\eid a\eor x\eid b\eor x\eid c)$ is true. Why? We add a new name `$d$'. There are three ways we can extend our original model:\begin{enumerate}
\item
\begin{ekey}
\item[d] Alfred
\end{ekey}
...
\end{ekey}
\end{enumerate}
On the first of these, $d\eid a$ is true, on the second $d\eid b$ is true, and on the third $d\eid c$ is true. So on each of these models, $d\eid a\eor d\eid b\eor d\eid c$ is true, and thus $\forall x(x\eid a\eor x\eid b\eor x\eid c)$.



%Identity is a special predicate of FOL. We write it a bit differently than other two-place predicates: `$x\eid y$' instead of `$Ixy$' (for example). More important, though, its model is fixed, once and for all.
%


%Where \meta{a} and \meta{b} are any names,
%	\factoidbox{
%		$\meta{a} \eid  \meta{b}$ is true in a model \textbf{iff}\\
%		 \meta{a} and \meta{b} name the very same object in that model
%	}
%So in our go-to model, `$a \eid  b$' is false, since Aristotle is distinct from Beyonc\'e.
...
\begin{fitchproof}
	...
	\close
	\have{abba}{a \eid  b \eif b \eid a}\ifI{ab-ba}
...
\begin{fitchproof}
	\open
		...
		\have{ab}{a \eid  b}\andE{abc}
		\have{bc}{b \eid  c}\andE{abc}
		\have{ac}{a \eid  c}\by{\eid E}{ab, bc}
	\close
	\have{con}{(a \eid b \eand b \eid c) \eif a \eid  c}\ifI{abc-ac}
