...
\problempart Explain why:
	\begin{ebullet}
		\item   `$\exists x \forall y(Ay \eiff x\eid  y)$' is a good symbolization of `there is exactly one apple'.
		\item `$\exists x \exists y \bigl[\enot\, x\eid y \eand \forall z(Az \eiff (x\eid  z \eor y \eid z))\bigr]$' is a good symbolization of `there are exactly two apples'.
	\end{ebullet}
...
\begin{earg}
\item $Pa \eor Qb, Qb \eif b\eid c, \enot Pa \proves Qc$
\item $m\eid n \eor n\eid o, An \proves Am \eor Ao$
\item $\forall x\ x\eid m, Rma\proves \exists x Rxx$
\item $\forall x\forall y(Rxy \eif x\eid y)\proves Rab \eif Rba$
...
\item $\forall x(x\eid n \eiff Mx), \forall x(Ox \eor \enot Mx)\proves On$
\item $\exists x Dx, \forall x(x\eid p \eiff Dx)\proves Dp$
\item $\exists x\bigl[(Kx \eand \forall y(Ky \eif x\eid y)) \eand Bx\bigr], Kd\proves Bd$
...
\begin{ebullet}
\item $\exists x \bigl([Fx \eand \forall y (Fy \eif x \eid y)] \eand x \eid  n\bigr)$
\item $Fn \eand \forall y (Fy \eif n\eid  y)$
\end{ebullet}
...
\problempart
In \S\ref{ch.identity}, we claimed that the following are logically equivalent symbolizations of the English sentence `there is exactly one F':
\begin{ebullet}
\item $\exists x Fx \eand \forall x \forall y \bigl[(Fx \eand Fy) \eif x \eid  y\bigr]$
