...
This, however, gives us $(A\eif\enot B)$ rather than what we wrote, which was $A\eif\enot B$. 
...
\end{itemize}
An alternative way of thinking about it: the brackets should always be there, but we are allowed to write them in invisible ink. 
...
\begin{center}
	\begin{forest}
		...
		[$\mainconnective{\enot} (A\eand B)$[$A\mainconnective{\eand} B$[$A$][$B$]]]
		[$C$]
		]
	\end{forest}
\end{center}
...
\begin{definition}The \define{scope} of a connective in a sentence is the sub-sentence for which that connective is the main connective.
\end{definition}

In the formation tree of a sentence, the scope of a connective is the subtree whose top node is the formula in which that connective is the main connective. Only that subtree is relevant to determining the truth value of that formula under a valuation.
...
We can also indicate these scopes linearly:
\[\overbrace{\almostinvisibleopenbracket \underbrace{(P \eand Q)}_{\textcolor{blue}{\text{scope of }\eand}}\eor \underbrace{(\overbrace{\enot R}_{\textcolor{teal}{\text{scope of }\enot}}\eif S)}^{\textcolor{red}{\text{scope of }\eif}}\almostinvisibleclosebracket}^{\textcolor{yellow!80!black}{\text{scope of }\eor}}
\]

In this sentence, the scope of $\eor$ is the whole sentence, as it is the main connective of that whole sentence: it was the connective which was introduced last.  The scope of $\eand$ is $(P \eand Q)$, the scope of $\eif$ is $(\enot R \eif S)$, and the scope of $\enot$ is $\enot R$. 
...
$\eand$ and $\eor$ will be said to bind more strongly than $\eif$. This is like in arithmetic where $\times$ binds stronger than $+$. In arithmetic, you learn to treat $2+3\times 4$ as $2+(3\times 4)$. 
