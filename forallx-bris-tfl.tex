...
In this part the lecture notes we start developing our theory of validity and logical consequence, that is, we start introducing the logical language. In \cref{sec:Valid} we already introduced the idea that an argument is valid if and only if the truth of the premises guarantees the truth of the conclusion in virtue of their logical form. It is this idea that suggest to spell out an account of validity in a formal language, that is, to conceive of the logical language as a formal language. This will enable us to single out arguments that are valid in virtue of their form and eventually make sense of the notion of an interpretation we used in our definition of validity in \cref{sec:Valid}. We can then give a rigorous formal definition of validity of arguments in the formal language we shall devise. This language will be the language of Truth-functional logic (TFL).
...
When one introduces a language there are (at least) two task: the first is to specify the vocabulary of the language and equip the language with a grammar, that is, one has to specify how well-formed sentences of the language look like. This aspect of the language is called its \define{syntax}. The second task is to specify the \define{semantics} of the language. The semantics specifies how we are to understand the expressions of the language, what the sentences of the language mean etc. \cref{ch:TFL} develops both the syntax and the semantics of the language of TFL. Once this has been established we can consider how TFL may be useful for thinking about arguments in English. This will lead to the idea of symbolizing arguments in TFL and will be picked up in \cref{ch:TFLsymb}.

%In \cref{ch:TFL}--\ref{ch:NDTFL}, we are going to develop a formal system which allows us to symbolize many arguments in such a way as to show that they are valid in virtue of their form. That language will be \emph{truth-functional logic}, or TFL. It will have sentences like $$(A\eand (B\eif\enot C)),$$ which we will read ``$A$ and if $B$, then it is not the case that $C$''.
%\chapter{Building blocks of TFL}

%In natural languages like English or German, there are many different parts of the vocabulary 
\section{Vocabulary of TFL}
The vocabulary of TFL is very simple. There are three parts to the vocabulary: atomic sentences, connectives, and brackets. 

\subsection{Atomic Sentences}
\label{sec:atomic sentences}
...
\label{sec:TFLConnectives}

The second category of the vocabulary of TFL is the connectives. They can be used to form new sentences out of old ones.
...
description={A logical operator in TFL used to combine \glspl{atomic sentence} into larger sentences}
	...
	\begin{tabular}{l l ll}
		...
		\textbf{symbol}&\textbf{official name}&\textbf{rough meaning}&\textbf{read it}\\
		\hline
		\enot&negation&`It is not the case that$\ldots$'&not\\
		\eand&conjunction&`$\ldots$\ and $\ldots$'&and \\
		\eor&disjunction&`$\ldots$\ or $\ldots$'&or \\
		\eif&conditional&`If $\ldots$\ then $\ldots$'&arrow\\
		%\eiff&biconditional&`$\ldots$ if and only if $\ldots$'\\
		
	\end{tabular}
\end{table}

You may use the simple names `not', `and', `or', and `arrow' when reading the symbols. 

%These are not the only connectives of English of interest. Others are, e.g., `unless', `neither \dots{} nor \dots', and `because'. We will see that the first two can be expressed by the connectives we will discuss, while the last cannot. `Because', in contrast to the others, is not \emph{truth functional}. In \cref{??} we shall explain what this means precisely.

If we were to substitute declarative sentences for `\dots' in the `meaning' column of the table above, we obtain new English sentences. The language of truth-functional logic works in the same way: the connectives `\enot',`\eand',`\eor' and `\eif' combine other sentences. For example, the atomic sentences `$R$' and `$M$' combine with `$\eand$' to form the sentence `$R\eand M$'. If think of `$R$' and `$M$' as representing the English sentences
\begin{ekey}
		\item[R] It is raining outside
		\item[M] Jenny is miserable
	\end{ekey}
`$R\eand M$' can be read as:
\begin{itemize}
\item It is raining \mainconnective{and} Jenny is miserable.
\end{itemize}
Similarly, on this understanding we get the following readings:

\begin{center}
	\begin{tabular}{rl}
	$\enot R$&\mainconnective{It is not the case that} it is raining outside. \\
	&(It is not raining outside).\\\Tstrut
		$R\eand M$& It is raining outside \mainconnective{and} Jenny is miserable.\\\Tstrut
	$R\eor M$&It is raining outside \mainconnective{or} Jenny is miserable.\\\Tstrut
	 $R\eif M$&\mainconnective{If} it is raining outside, \mainconnective{then} Jenny is miserable.
\end{tabular}
\end{center}
...
\subsection{Brackets}
Brackets are the remaining component of the vocabulary of TFL. They will be used to provide the grammar. They are vital when we start looking at complex sentences of TFL. For example `$A\eand(B\eor C)$'. 
But, to start to understand the connectives we will ignore this for now and just focus on one connective applied to atomic sentences. 


\chapter{The connectives}\label{sec:Connectives}
Whilst this provides a gloss on our new symbols of TFL, their role in TFL in terms of their contribution to the truth conditions of a sentence is explicitly specified. 
%In \cref{ch:syntfl} we introduced the idea that an argument is valid if there is no interpretation on which all premises of the argument are true but the conclusion false. Accordingly, if we wish to make a start on making this idea more precise, we need to say when a sentence of TFL is true according to an interpretation and when it is false. The target of this chapter is to give precise rules to this effect. 
The important feature of truth functional logic is that the truth value of a sentence such as `$A\eand B$' is determined entirely by the truth value of `$A$' and the truth value of `$B$'. The truth value of $\enot A$ is determined entirely by the truth value of $A$. 
...
\section{Negation}
The `$\enot$'-connective is called negation. It should roughly be understood as `it is not the case' or, perhaps, simply `not'. 

If think of `$A$' as representing the English sentence `Alice is happy':
\begin{ekey}
	\item[A] Alice is happy
\end{ekey}
Then `$\enot A$' will be used to symbolise the English sentence:
\begin{samplesent}
	\item {It is not the case that} Alice is happy
\end{samplesent}
Is this sentence true or false? It depends on whether Alice is happy. She either is or isn't happy, that is `Alice is happy' might be true or false:
\begin{center}
	\begin{tabular}{cl}
		$A$&\\\hline\Tstrut
		T& Alice is happy\\
		F& Alice is not happy
	\end{tabular}
\end{center}

In which cases is `$\enot A$' true? Just when `$A$' is false: 
\begin{center}
	\begin{tabular}{c|c}
		$A$&$\enot A$\\\hline\Tstrut
		T&F\\
		F&T
	\end{tabular}
\end{center}

The negation symbol of TFL, $\enot$, is defined to have this affect on the truth value of the sentence. Whether $\enot A$ is then a good symbolisation of a given sentence then depends on whether it is appropriately governed by this truth table.
...
\newglossaryentry{conjunction}{
	name=conjunction,
	description={The connective \eand, used to represent words and phrases that function like the English word ``and''; or a sentence formed using this connective}
}
...
If think of `$A$' and `$B$' as representing the English sentences
\begin{ekey}
	\item[A] Alice is happy
	\item[B] Beth is happy
\end{ekey}
`$A\eand B$' can be read as:
\begin{samplesent}
	\item Alice is happy \emph{and} Beth is happy.
\end{samplesent}

Is it true or false? It depends on whether Alice is happy and whether Beth is happy. 
Here are the four possibilities:
\begin{center}
	\begin{tabular}{rccl}
	&$A$&$B$\\\hline\Tstrut
	$v_1$&T&T&Alice and Beth are both happy\\
	$v_2$&T&F&Alice is happy but Beth is not\\
	$v_3$&F&T&Alice is not happy but Beth is\\
	$v_4$&F&F&Neither Alice nor Beth are happy
\end{tabular}
\end{center}

In which cases is `$A\eand B$' true? Only $v_1$. That is, only if `$A$' is true and `$B$' is true. This is:
\begin{center}
	\begin{tabular}{cc|c}
	$A$&$B$&$A\eand B$\\\hline
	T&T&T\\
...
\section{Disjunction}\label{sec:connectives-intro:disjunction}
...
If think of `$G$' and `$F$' as representing the English sentences
\begin{ekey}
	\item[G] She understands German
	\item[F] She understands French
\end{ekey}
`$G\eor F$' can be read as:
\begin{samplesent}
	\item She understands German or she understands French.
\end{samplesent}
Or, more concisely:
\begin{samplesent}
	\item She understands German or French.
\end{samplesent}

Again whether this is true will depend on $G$ and $F$. Here are the four possibilities:
\begin{center}
	\begin{tabular}{rccl}
		&$G$&$F$\\\hline\Tstrut
		$v_1$&T&T&She understands German and French\\
		$v_2$&T&F&She understands German but not French\\
		$v_3$&F&T&She understands French but not German\\
		$v_4$&F&F&She understands neither German nor French\\
	\end{tabular}
\end{center}
Clearly `She understands German or French' is false in $v_4$ where she understands neither; and true in $v_2$ and $v_3$ where she understands one of these languages:
\begin{center}
	\begin{tabular}{rcc|c}
		&$G$&$F$&$G\eor F$\\\hline\Tstrut
		$v_1$&T&T&\\
		$v_2$&T&F&T\\
		$v_3$&F&T&T\\
		$v_4$&F&F&F
	\end{tabular}
\end{center}
But what about $v_1$, where she understands both? 
...
\begin{samplesent}
	\item\label{sent:inclor} She can understand German or French (or both).
	\item\label{sent:exclor} She can understand German or French (but not both).
\end{samplesent}
Sometimes English is ambiguous between the two readings. 

In logic there can be no ambiguity. We stipulate that $\eor$ stands for the \emph{inclusive or}, giving it the truth value true in $v_1$:
\begin{center}
	\begin{tabular}{rcc|c}
		&$G$&$F$&$G\eor F$\\\hline
		$v_1$&T&T&T\\
	...
	\end{tabular}
\end{center}
That is `$G\eor F$' is true if and only if `$G$' is true or `$F$' is true (or both). The ``or both'' will be dropped throughout, and we will understand the English `or' to be given an inclusive reading unless explicitly discussed. 

If one wishes to capture the exclusive use of English, this will be perfectly possible in TFL, but for that, we'll have to use a more complex sentence. We will essentially make explicit the whispered ``but not both''. This will end up being symbolised as:
$$(G\eor F)\eand\enot (G\eand F)$$
She can understand German or French, and it's not the case that she can understand both German and French.  
...
Consider
\begin{samplesent}
	\item\label{sent:cond: beer then eighteen} If she is drinking beer, then she is over eighteen.
	\item\label{sent:cond: eighteen then beer} If she is over eighteen, then she is drinking beer.
\end{samplesent}
These are very different. We might have atomic sentences:
\begin{ekey}
	\item[B] She is drinking beer.
	\item[E] She is over eighteen.
\end{ekey}
We will then represent \cref{sent:cond: beer then eighteen} as `$B\eif E$' and \cref{sent:cond: eighteen then beer} as `$E\eif B$'. 


Since it matters which order they come in, we is valuable to give a name to indicate which part goes where:
\begin{definition}
...
In what cases is `$B\eif E$' true or false? Here are the four cases:
\begin{center}
	\begin{tabular}{rccl}
		&$B$&$E$\\\hline
		$v_1$&T&T&She is drinking beer and is over eighteen\\
		$v_2$&T&F&She is drinking beer and is not over eighteen\\
		$v_3$&F&T&She is not drinking beer and she is over eighteen\\
		$v_4$&F&F&She is not drinking beer and she is not over eighteen\\
	\end{tabular}
\end{center}
In $v_2$ it is false, and in $v_1$ it is true. 
\begin{center}
	\begin{tabular}{rcc|c}
		&$B$&$E$&$B\eif E$\\\hline
		$v_1$&T&T&T\\
	...
	\end{tabular}
\end{center}
What about $v_3$ and $v_4$? 
We stipulate that it is true in $v_3$ and $v_4$: if the antecedent of the conditional is false, the conditional is true.
\begin{center}
	\begin{tabular}{rcc|c}
		&$B$&$E$&$B\eif E$\\\hline
		$v_1$&T&T&T\\
...
%This material conditional tells us nothing about what would be the case if the antecedent were true; if she were to be drinking a beer she would be over eighteen. 

For our purposes $\eif$-connective will be understood in terms of truth rules given in this section.
That is, we define `$\eif$' by these truth rules:
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	That is, $\metaX\eif\metaY$ is true if and only if it is not the case that $\metaX$ is true and $\metaY$ is false; or, equivalently, if $\metaX$ is false or $\metaY$ is true.

The conditional which is defined by this truth rule is sometimes marked by calling it the \define{material conditional}. It is the conditional that we will use in the unit. 

This material conditional says nothing about what the world \emph{would be like} were the antecedent to be true. For the material conditional, if the antecedent doesn't apply, in this case, she is not drinking beer, then the conditional is simply trivially true independently of what would happen were she to be drinking a beer. 

There is a rich philosophical and logical investigation into conditionals, but for this unit, we will stick with the material conditional.
The material conditional is the best that can be done to capture the conditional in a truth-functional way, where the truth can be determined simply from the truth values of its constituent parts. 
 See \url{https://plato.stanford.edu/entries/conditionals} and \cref{sec:IndicativeSubjunctive} for further discussion

As we highlighted, the order matters. So lets consider `$E\eif B$': if she is over eighteen then she is drinking beer. How does it differ from $B\eif E$? We can apply this rule, but we have to be careful which order to apply it in. To help make sure we apply it in the right order, it might be helpful to copy down the values for $E$ and $B$ before applying the rule: 
\begin{center}
	\begin{tabular}{rcc|c|def}
		&$B$&$E$&$B\eif E$&$E$&$\eif$&$B$\\\hline
		$v_1$&T&T&T&T&&T\\
		$v_2$&T&F&F&F&&T\\
		$v_3$&F&T&T&T&&F\\
		$v_4$&F&F&T&F&&T
	\end{tabular}
\end{center}
and we can then apply the rule. We will write the answer underneath the `$\eif$'. As we have extra truth values hanging around in the table, we have highlighted the column of truth values which give the truth value for the sentence `$E\eif B$'.
\begin{center}
	\begin{tabular}{rcc|c|def}
		&$B$&$E$&$B\eif E$&$E$&$\eif$&$B$\\\hline
		$v_1$&T&T&T&T&\TTbf{T}&T\\
		$v_2$&T&F&F&F&\TTbf{T}&T\\
		$v_3$&F&T&T&T&\TTbf{F}&F\\
		$v_4$&F&F&T&F&\TTbf{T}&T
	\end{tabular}
\end{center}
`$E\eif B$' is false when she is over eighteen and not drinking beer. But otherwise it is true. 




\section{Truth rules}\label{sec:Truth rules}
These are the rules associated with the connectives. They are motivated by the English uses, but are stipulated to be governed by these truth rules. 
\begin{highlighted}\label{def:Truthint}
	% 	Let $v$ be a valuation. 
	%	\begin{enumerate}
		%		\item An atomic sentence $X$ is true relative to $v$, if and only if $v$ assigns the value T to $X$.
		%		\item A sentence $\enot\metaX$, $(\metaX\eand\metaY)$, $(\metaX\eor\metaY)$, $(\metaX\eif\metaY)$, whether it is true or false relative to $v$ can be determined by applying the appropriate truth rule depending on the truth of its constituant parts
...
If you want to capture an exclusive disjunction, you can use a more complex sentence of TFL. In fact, anything where the truth values of the sentence depends entirely on the truth of the component sentences is something that can be captured in truth-functional logic, possibly by a more complex sentence. Non-truth-functional notions, however, go beyond the resources of TFL. See \cref{sec:TruthFunctionality}.
...
%\begin{samplesent}
%	\item\label{sent:neg-f} Bristol is not in France.
%	\item\label{sent:neg-f} Bristol is not in England.
%\end{samplesent}
...
%\begin{samplesent}
%	\item\label{sent:conj} She can speak German and she can speak French.
%\end{samplesent}
...
%Consider:
%\begin{samplesent}
%	\item\label{sent:conj} She can speak German or she can speak French.
%\end{samplesent}
...
%It seems that in English there are two kinds of disjunctions: an \define{inclusive} and an \define{exclusive} one. For the inclusive \emph{or}, we might whisper a ``or both'' after it; whereas for the exclusive \emph{or}, we'd want to whisper a ``but not both'':
%\begin{samplesent}
%	\item\label{sent:inclor} She can speak German or
%	she can speak French (or both).
%	\item\label{sent:exclor} She can speak German or
...
%\begin{samplesent}
%	\item\label{sent:bartender} If she is drinking a beer, then she is over eighteen.
%\end{samplesent}
...
%\begin{samplesent}
%	\item\label{sent:kangaroo} If Kangaroos had no tails, they would topple over.
%\end{samplesent}
%%We will discuss conditional-sentences of this kind in \cref{sec:IndicativeSubjunctive} and will look at some problems arising due to understanding $\eif$ in terms of material conditional in \cref{sec:ParadoxesOfMaterialConditional}.
%%
%For our purposes $\eif$-connective will be understood in terms of truth rules given in this section.
...
\begin{practiceproblems}
		\problempart 
	Write down (from memory) the truth rules for each of the connectives. 
...
\section{Validity}
\subsection{Example}
We are given a cards on the table which has a number on one side and a colour on the other side. You know a rule: `If the card is blue, it has an even number on it'. You can see the card is red. What can you conclude from this? You try to reason as follows: 
\begin{earg}
\prem If the card is blue, it has an even number on it. 
\prem The card is not blue.
\conc The card does not have an even number.
\end{earg}
Is this a good pattern of reasoning? Is it a valid argument? 

We will symbolise this argument as: $$B\eif E,\; \enot B\; \therefore\; \enot E$$

An argument is valid if the premises guarantee the conclusion. There is no way for the premises to be true and the conclusion false. 
Whether the sentences in this argument are true or false depend on whether $B$ is true or false and whether $E$ is true or false. Here are the four possibilities: 
\begin{center}
	\begin{tabular}{rccl}
		&$B$&$E$\\\hline
		$v_1$&T&T&The card is blue and is even\\
		$v_2$&T&F&The card is blue and is not even\\
		$v_3$&F&T&The card is not blue and is even\\
		$v_4$&F&F&The card is not blue and is not even\\
	\end{tabular}
\end{center}
They are called ``valuations''. 

\begin{definition}
		A \define{valuation} is any assignment of truth values (true or false) to the atomic sentences.
	\end{definition}
...
What is special about the logic we are introducing here, called Truth Functional Logic, is that to know the truth value of any sentence, one just needs to specify the truth value of the atomic sentences, which is exactly what is given by a valuation. 

Now, to check validity, we will evaluate whether the premises and conclusions are true or false in each of these valuations.

\begin{center}
	\begin{tabular}{rcc|c|c|cr}
		&&&premise 1&premise 2&conclusion\\
		&$B$&$E$&$B\eif E$&$\enot B$&$\enot E$\\\hline
		$v_1$&T&T&T&F&F\\
		$v_2$&T&F&F&F&T\\
		$v_3$&F&T&\textbf{T}&\textbf{T}&\textbf{F}&$\leftsquigarrow$ counterexample\\
		$v_4$&F&F&T&T&T\\
	\end{tabular}
\end{center}

We look through these lines and see if we can find one of them where the premises are all true and the conclusion is false. We can: $v_3$!

The card might be red but still have an even number on it. Nothing in the information you are given (i.e., the premises) rules out the possibility. Your drawing the conclusion the the card was odd was a mistake of reasoning. 

\section{Definition of Validity}

\begin{definition}\label{def:TFLvalid}A TFL-argument is \define{valid} if and only if there is no valuation on which all the premises are true but the conclusion is false. 
	
	\medskip 
	That is, $X_1,\ldots,X_n\therefore Y$ is \define{valid} if and only if there is no valuation $v$ such that $X_1,\ldots,X_n$ are all true relative to $v$, but $Y$ is false relative to $v$.\end{definition}
	
	
	
	
\begin{highlighted}
To check {validity} with truth tables, draw a truth table for all the sentences simultaneously and look for a row like:
...
We will also say:
\begin{definition}
\metaY is a \define{logical consequence} of $\metaX_1,\ldots,\metaX_n$ if and only if there is no valuation on which $X_1,\ldots,X_n$ are true and $Y$ false. 
\end{definition}
That is, \metaY is a {logical consequence} of $\metaX_1,\ldots,\metaX_n$ if and only if $X_1,\ldots,X_n\therefore Y$ is valid. 
...
Consider this argument: 
\begin{earg}
	\prem The cook or the gardener did it.
	\prem The cook did not do it.
	\conc The gardener did it.
\end{earg}

We will follow the same approach:
\begin{enumerate}
	\item Symbolise the premises and conclusion.
	\item List every valuation and calculate the truth value of the premises and conclusion on each valuation. 
	\item Determine whether there is one possibility where the premises are all true and the conclusion is false. 
\end{enumerate}

Let:
\begin{ekey}
	\item[C]The cook did it.
	\item[G] The gardener did it.
\end{ekey}

So the argument is:
\[
	C \eor G, \;\enot C \therefore G
\]

\begin{center}
	\begin{tabular}{rcc|c|c|c}
		&&&premise 1&premise 2&conclusion\\
		&$C$&$G$&$C\eor G$&$\enot C$&$G$\\\hline
		$v_1$&T&T&T&F&T\\
		$v_2$&T&F&T&F&F\\
		$v_3$&F&T&\textbf{T}&\textbf{T}&\textbf{T}\\
		$v_4$&F&F&F&T&F\\
	\end{tabular}
\end{center}

For this argument, there is no valuation where both the premises are true and the conclusion is false. It is only $v_3$ where the premises are both true, and on $v_3$ the conclusion is also true. So the argument is valid. 

\subsection{More valuations}

Consider this argument:
\begin{earg}
	\prem If Alex went to the party, then Beth went to the party.
	\prem If Beth went to the party, then Chris went to the party.
	\prem Alex went to the party.
	\conc Chris went to the party.
\end{earg}

Using the symbolisation key:
\begin{ekey}
	\item[A] Alex went to the party.
	\item[B] Beth went to the party.
	\item[C] Chris went to the party.
\end{ekey}

The argument is symbolised as:
\[
	A \eif B,\; B \eif C,\; A \;\therefore\; C
\]

Now we need to again list all the valuations and determine the truth of each of the premises and the conclusion on each. Now we have three atomic sentences to deal with. 
There are eight valuations:
...
We can now use our truth rules to determine the truth of each sentence on each valuation. To aid this calculation, we first copy across the truth values for $A$, $B$ and $C$ to make sure we apply the truth rule for $\eif$ to the right values in the right order. We then highlight the column for the main connective, which provides the truth value for the overall sentence at each valuation, indicating  that the others were just calculation columns. 
\begin{center}
	\begin{tabular}{rccc|def|def|d|d}
		&&&&\multicolumn{3}{c|}{premise 1}
		&\multicolumn{3}{c|}{premise 2}
		&\multicolumn{1}{c||}{premise 3}
		&\multicolumn{1}{c}{conclusion}\\
		&$A$&$B$&$C$
		&$A$&$\eif$&$B$
		&$B$&$\eif$&$C$
		&$A$
...
We check whether there is a valuation where the premises are all true and the conclusion is false. $v_1$ is the only valuation where the premises are all true, and on this valuation, the conclusion is also true. It is therefore a valid argument.




\section{Determining all the valuations}\label{sec:ValsAll}
...
$A$ & $B$ &\qquad& $A$ & $B$ & $C$ \\
\cline{1-2}\cline{4-6}
\multirow{2}{*}{T} & \multirow{2}{*}{T} & \neearrow & T & T & T \\
                   &                    & \seearrow & T & T & F \\[2pt]

\multirow{2}{*}{T} & \multirow{2}{*}{F} & \neearrow& T & F & T \\
                   &                    & \seearrow& T & F & F \\[2pt]

\multirow{2}{*}{F} & \multirow{2}{*}{T} & \neearrow& F & T & T \\
                   &                    & \seearrow& F & T & F \\[2pt]

\multirow{2}{*}{F} & \multirow{2}{*}{F} & \neearrow & F & F & T \\
                   &                    & \seearrow& F & F & F
\end{tabular}
\end{center}
...
What if we add a further atomic sentence, say $P_{23}$? We want to consider the valuations for atomic sentences $A,B,C,P_{23}$.
For each valuation for $A,B,C$ there is a version of it where $P_{23}$ is true, and another where $P_{23}$ is false. We thus duplicate each line of the table so far and add these two possible truth values for $P_{23}$ in the two variants. 

\begin{center}
\begin{tabular}{cccccccc}
$A$ & $B$ & $C$ & \qquad & $A$ & $B$ & $C$ & $P_{23}$ \\
\cline{1-3}\cline{5-8}
\multirow{2}{*}{T} & \multirow{2}{*}{T} & \multirow{2}{*}{T} & \neearrow & T & T & T & T \\
                   &                    &                    & \seearrow & T & T & T & F \\[2pt]

\multirow{2}{*}{T} & \multirow{2}{*}{T} & \multirow{2}{*}{F} & \neearrow & T & T & F & T \\
                   &                    &                    & \seearrow & T & T & F & F \\[2pt]

\multirow{2}{*}{T} & \multirow{2}{*}{F} & \multirow{2}{*}{T} & \neearrow & T & F & T & T \\
                   &                    &                    & \seearrow & T & F & T & F \\[2pt]

\multirow{2}{*}{T} & \multirow{2}{*}{F} & \multirow{2}{*}{F} & \neearrow & T & F & F & T \\
                   &                    &                    & \seearrow & T & F & F & F \\[2pt]

\multirow{2}{*}{F} & \multirow{2}{*}{T} & \multirow{2}{*}{T} & \neearrow & F & T & T & T \\
                   &                    &                    & \seearrow & F & T & T & F \\[2pt]

\multirow{2}{*}{F} & \multirow{2}{*}{T} & \multirow{2}{*}{F} & \neearrow & F & T & F & T \\
                   &                    &                    & \seearrow & F & T & F & F \\[2pt]

\multirow{2}{*}{F} & \multirow{2}{*}{F} & \multirow{2}{*}{T} & \neearrow & F & F & T & T \\
                   &                    &                    & \seearrow & F & F & T & F \\[2pt]
...
The list of all possible valuations gets very big very fast. It doubles for each new atomic sentence. 

There are $2^n$ ways to assign truth values to $n$ atomic sentences. Using truth tables to determine validity thus becomes quite difficult and it is one reason we consider an alternative method: that of proofs, which we will see in \cref{ch:NDTFL}.
...
\section{Careful application of the definition}
\subsection{Check all the valuations}
Consider this argument:
\begin{earg}
	\prem If Ava studied, then Ava passed.
	\prem Ava passed.
	\conc Ava studied.
\end{earg}
...
This argument is symbolised as:
\[
	S \eif P,\; P \therefore S
\]

\begin{center}
	\begin{tabular}{rcc|c|c|c}
		&&&premise 1&premise 2&conclusion\\
		&$S$&$P$&$S \eif P$&$P$&$S$\\\hline
		$v_1$&T&T&\textbf{T}&\textbf{T}&\textbf{T}\\
		$v_2$&T&F&F&F&T\\
		$v_3$&F&T&\textbf{T}&\textbf{T}&\textbf{F}\\
		$v_4$&F&F&T&F&F\\
	\end{tabular}
\end{center}

This argument is invalid. There is a valuation where the premises are true and the conclusion is false: $v_3$.
It does not matter that there is another valuation where the premises and the conclusion are all true, namely $v_1$.

Compare it to:
\begin{center}
	\begin{tabular}{rccc|c|c|c|c}
		&&&&premise 1&premise 2&premise 3&conclusion\\
		&$A$&$B$&$C$&$A \eif B$&$B \eif C$&$A\eor B$&$C$\\\hline
		$v_1$&T&T&T&\textbf{T}&\textbf{T}&\textbf{T}&\textbf{T}\\
		$v_2$&T&T&F&T&F&T&F\\
		$v_3$&T&F&T&F&T&T&T\\
		$v_4$&T&F&F&F&T&T&F\\
		$v_5$&F&T&T&\textbf{T}&\textbf{T}&\textbf{T}&\textbf{T}\\
		$v_6$&F&T&F&T&F&T&F\\
		$v_7$&F&F&T&T&T&F&T\\
		$v_8$&F&F&F&T&T&F&F\\
	\end{tabular}
\end{center}
Again we have two valuations where the premises are all true, $v_1$ and $v_5$, but here the conclusion is true on both of them. 
...
\begin{center}
	...
	\end{tabular}
\end{center}
This is a valid argument! There is no valuation where the premises are all true, and thus, no valuation where the premises are true and the conclusion is false. 

Recall, validity is only a judgement about the connection between the premises and the conclusion. It does not imply that the conclusion is true, only that it follows from the premises. If the premises have flaws, then a valid argument isn't of much help, except to help us identify that they have flaws. This is a common pattern of reasoning: identify some proposals, and see where they lead. If what they lead to is implausible, and your argument was valid, there must be a mistake with the premises. 


\section{Beyond Logical Validity}


Some valuations might not correspond to conceptual possibilities. 
Consider this argument: 
\begin{earg}
\prem Jonas is a puppy.
\conc Jonas is dog.
\end{earg}

\begin{center}
	\begin{tabular}{rcc|c|c}
		&&&premise&conclusion\\
		&$P$&$D$&$P$&$D$\\\hline
		$v_1$&T&T&T&F\\
		$v_2$&T&F&T&T\\
...
So, in $v_2$, the premise is true and the conclusion is false. Therefore, the argument is (logically) invalid.

$v_2$, however, is not a conceptually possible valuation. Given what `puppy' and `dog' mean, $v_2$ is not possible. 
Sometimes the notion of `conceptual validity' is introduced, which considers not every valuation but only those which are compatible with the meanings of the terms at stake. In this unit, we will instead say it is invalid but encourage you to include an additional implicit premise: $P\eif D$. 
...
\chapter{Use and mention}\label{sec:UseMention}
We have now talked \emph{about} sentences of TFL and we are just about to do so in much more detail. We should pause to explain an important, and very general, point.
...
$$A, B, C, P, Q, A_1, B_4, A_{25}, J_{375},\ldots$$
...
\section{Metavariables}\label{sec:Metavariables}
However, we do not just want to talk about \emph{specific} expressions of TFL. We also want to be able to talk about \emph{any arbitrary} sentence of TFL. Indeed, we had to do this in \cref{sec:TFLSentences}, when we presented the recursive definition of a sentence of TFL. We used uppercase script letters to do this, namely:
$$\metaX, \metaY, \metaZ,\metaX_1,\metaY_1,\metaZ_1\ldots$$
These symbols do not belong to TFL. Rather, they are part of our (augmented) metalanguage that we use to talk about \emph{any} expression of TFL. 

To explain why we need them, recall the second clause of the recursive definition of a sentence of TFL:
\begin{itemize}
...
\begin{highlighted}
	`$\metaX$',  is a symbol (called a \define{metavariable}) in augmented English, which we use to talk about expressions of TFL. 	`$A$' is a particular sentence letter of TFL.\end{highlighted}
...
One of our main purposes for using TFL is to study arguments. In English, the premises of an argument are often expressed by individual sentences, and the conclusion by a further sentence. Since we can symbolize English sentences, we can symbolize English arguments using TFL.
...
\chapter{Complex sentences of TFL}\label{sec:TFLSentences}
\section{Forming complex sentences}
We have introduced our new connectives and specified how the truth of a sentence like $\enot A$, $A\eand B$, $A\eor B$ and $A\eif B$ depends on the truth of $A$ and the truth of $B$. 

These cover many arguments, but some involve sentences with more complex logical form, with multiple connectives. For example, they might involve the following sentences. 
\begin{samplesent}
	\item It is not the case that both Alice and Beth are happy.
	\item \label{sent:exclAB}Alice or Beth is happy, and not both.
	\item Either Alice and Beth are both happy, or Charlie is happy. 
	\item If Alice is happy, then Beth is not happy.
\end{samplesent}

In order to use our logic to evaluate them, we will have to symbolise them in our logical system, TFL. 
We thus need to consider what complex sentences of TFL look like and then how to symbolise these sentences. 
...
We gave truth-rules for each of our connectives telling us how the truth value of a sentence depends on the truth-values of its component parts. This can be iterated to apply to more complex sentences. This will allow us to determine the truth value of complex sentences such as `$(A\eor B)\eand\enot(A\eand B)$' from knowing the truth value of $A$ and of $B$. To do this, it needs to be clear which order to apply the truth rules and this is the value of brackets. 

Brackets play a similar role in mathematics to tell you which order to apply your operations. 
\begin{samplesent}
	\item\label{sent:mathamb1} $9 + (3 \times 4) \qquad(=9+12=21)$
	\item\label{sent:mathamb2} $(9+3) \times 4 \qquad(=12\times 4=48)$
\end{samplesent}

Importantly, the language of TFL is designed to exclude any form of ambiguity. For example, $$A\eand B\eor C$$ will not be a sentence of TFL, brackets must be included. The syntactic/grammatical rules of the language of TFL will be such that only expressions that have a \emph{unique} reading can be sentences of TFL. To make this precise we now provide a formal definition of what it is to be a sentence in TFL.

This will specify what counts as a sentence, and given a sentence, will help us determine in which order it was constructed and thus which order to apply our truth rules. 

We will describe a procedure for constructing sentences of TFL:

\begin{definition}\label{def:TFLsentences}
	
%	The {sentence letters} of TFL are the capital letters 
%	\begin{center}
%		$A$, $B$, $C$,\ldots $W$,
%	\end{center}
%	Plus these capital letters subscripted by a number, $$A_{1}, A_2, C_{37}, F_{102},\ldots$$
%	Excluding `$X$', `$Y$', `$Z$', which are reserved for metavariables.
	
	
	
 Sentences of TFL are given by the following rules:
	\begin{enumerate}
		\item The \define{atomic sentences} of TFL are the capital letters
			\begin{center}
					$A$, $B$, $C$,\ldots $W$,
			\end{center}
			Plus these subscripted by a number, $A_{1}, A_2, C_{37}, F_{102},\ldots$
		\item If \metaX is a sentence of TFL, then $\enot\metaX$ is a sentence of TFL.
		\item If \metaX and \metaY are sentences of TFL, then $(\metaX\eand\metaY)$ is a sentence of TFL.
		\item If \metaX and \metaY are sentences of TFL, then $(\metaX\eor\metaY)$ is a sentence of TFL.
		\item If \metaX and \metaY are sentences of TFL, then $(\metaX\eif\metaY)$ is a sentence of TFL.
		%\item If \metaX and \metaY are sentences of TFL, then $(\metaX\eiff\metaY)$ is a sentence of TFL.
		\item 	Nothing else is a sentence of TFL.
	\end{enumerate}
...
This tells us that $$\enot A$$ is a sentence: $A$ is a sentence by our base clause, then adding a negation in front of it obtains a new sentence. Further negations could still be added, obtaining $$\enot\enot A$$

It also tells us that $$(A\eand B)$$ is a sentence: $A$ is a sentence by our base clause, and so is $B$, then putting a `$\eand$' between them and adding brackets on the outside we get a further sentence. 

We adopt a useful convention which allows us to write $A\eand B$ instead of the more cumbersome $(A\eand B)$; and $A\eif \enot B$ instead of $(A\eif \enot B)$:
\begin{itemize}
	\item One is allowed to drop the outermost brackets of a sentence. 
\end{itemize}

Thus we allow ourselves to write $A\eand B$ instead of the strict sentence $(A\eand B)$. However, we must remember to put the brackets back in, when we want to embed the sentence into a more complicated sentence!

An alternative way of thinking about it: the brackets should always be there, but we are allowed to write them in invisible ink.\footnote{Thanks to Kevin Blackwell for this metaphor. }
We will sometimes display this invisible ink in gray.
$$\almostinvisibleopenbracket A\eand B\almostinvisibleclosebracket$$
 When the sentence is written by itself, it can be dropped, but when it is included in another sentence, those brackets now need to be written. 

So when we write $$\enot(A\eand B)$$ those brackets must no longer be invisible. 
\begin{center}
	\begin{forest}
		[${\enot}\ldots$
		[$\almostinvisibleopenbracket\ldots{\eand} \ldots\almostinvisibleclosebracket$ [$A$][$B$]]
		]
	\end{forest}
\end{center}

We might indicate the construction of this sentence visually through the use of braces: 
$$\enot\,{\underbrace{(A\eand B)}}$$

This determining of the order of construction is important when knowing how to read it and how to apply the truth rules. 
We can link to English again:
\begin{center}
	\begin{forest}
		[${\enot}\ldots$\\It is not the case that \ldots
		[$\almostinvisibleopenbracket\ldots{\eand} \ldots\almostinvisibleclosebracket$\\\ldots and \ldots [$A$\\Alice is happy][$B$\\Beth is happy]]
		]
	\end{forest}
\end{center}

We will use the sentence $\enot (A\eand B)$ to be the symbolisation of the English sentence:
\begin{samplesent}
	\item 	It is not the case that both Alice and Beth are happy.
\end{samplesent}
...
Now consider 
$$(A\eor B)\eand \enot(A\eand B)$$
This sentence is constructed in stages: 
\begin{center}
	\begin{forest}
		[$\almostinvisibleopenbracket\ldots\eand\ldots\almostinvisibleclosebracket$
		[$\almostinvisibleopenbracket\ldots{\eor} \ldots\almostinvisibleclosebracket$[$A$][$B$]]
		[${\enot}\ldots$
		[$\almostinvisibleopenbracket\ldots{\eand} \ldots\almostinvisibleclosebracket$ [$A$][$B$]]
		]
		]
	\end{forest}
\end{center}
Displayed with braces as:
$$\underbrace{(A\eor B)}\eand \underbrace{\enot\underbrace{(A\eand B)}}$$
It can be read as:
\begin{center}
	\begin{forest}
		[$\almostinvisibleopenbracket\ldots\eand\ldots\almostinvisibleclosebracket$\\\ldots and \ldots
		[$\almostinvisibleopenbracket\ldots{\eor} \ldots\almostinvisibleclosebracket$\\\ldots or \ldots[$A$\\Alice is happy][$B$\\Beth is happy]]
		[${\enot}\ldots$\\It is not the case that \ldots
		[$\almostinvisibleopenbracket\ldots{\eand} \ldots\almostinvisibleclosebracket$\\\ldots and \ldots [$A$\\Alice is happy][$B$\\Beth is happy]]
		]
		]
	\end{forest}
\end{center}
This will be an appropriate symbolisation for: 
\begin{samplesent}
\item	Alice or Beth are happy, and it is not the case that both Alice and Beth are happy.
\end{samplesent}
Or, more concisely put:
\begin{samplesent}
	\item Alice or Beth are happy, and not both. 
\end{samplesent}


\subsection{Another example}
Consider $$(A\eand B)\eor C$$

It is formed as:
\begin{center}
\begin{forest}
[$\almostinvisibleopenbracket \ldots\eor\ldots\almostinvisibleclosebracket$
[$\almostinvisibleopenbracket \ldots\eand \ldots\almostinvisibleclosebracket$
[$A$][$B$]]
[$C$]
]
\end{forest}
\end{center}
We can also indicate with braces $$\underbrace{(A\eand B)}\eor {C}$$
We can link it with English:
\begin{center}
\begin{forest}
[$\almostinvisibleopenbracket \ldots\eor\ldots\almostinvisibleclosebracket$\\\ldots or \ldots
[$\almostinvisibleopenbracket \ldots\eand \ldots\almostinvisibleclosebracket$\\\ldots and \ldots
[$A$\\Alice is happy][$B$\\Beth is happy]]
[$C$\\Charlie is happy]
]
\end{forest}
\end{center}
When giving English sentences we have to give some mechanisms to indicate which order to read it. Commas, or pauses when speaking, or words like `both' holding components of the sentence together. This is a sentence which might naturally be symbolised as $(A\eand B)\eor C$:
\begin{samplesent}
	\item  \label{sent:andor} Alice and Beth are both happy, or Charlie is happy
\end{samplesent}
	English, however, can be ambiguous. Indeed that is part of the value of offering such symbolisations: no ambiguity is possible. Consider: 
		\begin{samplesent}
		\item  Alice and Beth or Charlie is happy
	\end{samplesent}
	Or, more plainly, 
			\begin{samplesent}
		\item  Alice is happy and Beth is happy or Charlie is happy
	\end{samplesent}
	Here it is not at all clear whether it should be symbolised as $(A\eand B)\eor C$ or as $A\eand(B\eor C)$. Moreover, this isn't a purely stylistic choice: they mean different things and are true in different circumstances. They are not logically equivalent. The sentence is simply ambiguous. In logic, there is no ambiguity possible. You will not be allowed to write $A \eand B\eor C$. Instead, you have to either choose $(A\eand B)\eor C$ or $A\eand(B\eor C)$. 
	 

\section{Defining Main Connective and Scope}

Given any sentence of TFL, there is a unique way to read the sentence. 
In each sentence there is a singe main connective. 
\begin{definition}The \define{main connective} of a sentence is the last connective that was introduced in the construction of the sentence.\end{definition}
\newglossaryentry{main connective}{
	name = main connective,
	description = {the connective introduced last in the construction of a \gls{sentence of TFL}}
}
In $(A\eand B)\eor C$, the main connective is $\eor$. In $A\eand(B\eor C)$, the main connective is $\eand$. 

Another important notion is that of scope. 
...
\newglossaryentry{scope}{
	name = scope,
	description = {the formula or formulae to which a connective is applied in the construction of a \gls{sentence of TFL}}
}

To add to our notion of main connective, we introduce the notion of \define{scope}.

\begin{definition}The \define{scope} of an occurrence of a connective in a sentence is the sub-sentence for which that connective is the main connective.
\end{definition}

For example, consider $$(A\eor B)\eand\enot(A\eand B)$$

We can add braces to indicate how to read it. We also indicate the scope of each connective with the red dot on the brace. 

\[
\mathopen{\tikzmarknode{wholeopenbracket}{\almostinvisibleopenbracket}}
\mathopen{(}
	\mathord{\tikzmarknode{a}{A}}
	\mathbin{\tikzmarknode{or}{\eor}}
	\mathord{\tikzmarknode{b}{B}}
\mathclose{)}
\mathbin{\tikzmarknode{andouter}{\eand}}
\mathord{\tikzmarknode{not}{\enot}}
\mathopen{(}
	\mathord{\tikzmarknode{a2}{A}}
	\mathbin{\tikzmarknode{andinner}{\eand}}
	\mathord{\tikzmarknode{b2}{B}}
\mathclose{)}
\mathclose{\tikzmarknode{wholeclosebracket}{\almostinvisibleclosebracket}}
\]

\begin{tikzpicture}[remember picture,overlay]
	% A or B
	\scopefitbracewithcoordinate{a}{b}{or}[1]

	% A and B
	\scopefitbracewithcoordinate{a2}{b2}{andinner}[1]

	% not (A and B)
	\scopefitbracewithcoordinate{not}{b2}{not}[2]

	% whole formula
	\scopefitbracewithcoordinate{wholeopenbracket}{wholeclosebracket}{andouter}[3]
\end{tikzpicture}
\bigskip 


We can more explicitly list the scope of each connective:

\begin{itemize}
	\item The scope of the central $\eand$ is the whole sentence: \\$(A \eor B)\eand \enot(A \eand B)$.\\
	The main connective of the whole sentence is $\eand$: \\$(A \eor B)\mainconnective{\eand} \enot(A \eand B)$
	
	\item The scope of $\eor$ is $\almostinvisibleopenbracket A \eor B \almostinvisibleclosebracket$.\\
	The main connective of $\almostinvisibleopenbracket A \eor B \almostinvisibleclosebracket$ is $\eor$.
	
	\item The scope of $\enot$ is $ \enot(A \eand B) $.\\
	The main connective of $ \enot(A \eand B) $ is $\enot$.
	
	\item The scope of the right-hand $\eand$ is $\almostinvisibleopenbracket A \eand B \almostinvisibleclosebracket$.\\
	The main connective of $\almostinvisibleopenbracket A \eand B \almostinvisibleclosebracket$ is $\eand$.
	
	

\end{itemize}

\bigskip % just to make sure the scope braces for the next example are on the right page...

For another example, consider: $$(P\eand Q)\eor \enot( R\eif \enot S)$$

We can visually indicate scopes and main connectives as:

\[
\mathopen{\tikzmarknode{wholeopenbracket}{\almostinvisibleopenbracket}}
\mathopen{(}
	\mathord{\tikzmarknode{p}{P}}
	\mathbin{\tikzmarknode{and}{\eand}}
	\mathord{\tikzmarknode{q}{Q}}
\mathclose{)}
\mathbin{\tikzmarknode{or}{\eor}}
\mathord{\tikzmarknode{notouter}{\enot}}
\mathopen{(}
	\mathord{\tikzmarknode{r}{R}}
	\mathbin{\tikzmarknode{if}{\eif}}
	\mathord{\tikzmarknode{notinner}{\enot}}
	\mathord{\tikzmarknode{s}{S}}
\mathclose{)}
\mathclose{\tikzmarknode{wholeclosebracket}{\almostinvisibleclosebracket}}
\]\nopagebreak
\begin{tikzpicture}[remember picture,overlay]
	% P and Q
	\scopefitbracewithcoordinate{p}{q}{and}[1]

	% not S
	\scopefitbracewithcoordinate{notinner}{s}{notinner}[1]

	% R if not S
	\scopefitbracewithcoordinate{r}{s}{if}[2]

	% not (R if not S)
	\scopefitbracewithcoordinate{notouter}{s}{notouter}[3]

	% whole formula
	\scopefitbracewithcoordinate{wholeopenbracket}{wholeclosebracket}{or}[4]
\end{tikzpicture}
\bigskip 
...
\begin{itemize}
	\item The scope of $\eor$ is the whole sentence, $((P \eand Q)\eor \enot(R \eif \enot S))$. \\The main connective of this whole sentence is $\eor$.
We can more explicitly list the scope of each connective:

	\item The scope of $\eand$ is $(P \eand Q)$.\\
	The main connective of $(P \eand Q)$ is $\eand$.
	
	\item The scope of $\enot$ is $\enot S$.\\
	The main connective of $\enot S$ is $\enot$.
	
	\item The scope of $\eif$ is $(R \eif \enot S)$.\\
	The main connective of $(R \eif \enot S)$ is $\eif$.
	
	\item The scope of the outer $\enot$ is $\enot(R \eif \enot S)$.\\
	The main connective of $\enot(R \eif \enot S)$ is the outer $\enot$.
	
\end{itemize}

\section{Negations}
\subsection{No brackets}

When we were combining two sentences with one of our connectives, we are told to add brackets. Here were three of our rules for what counts as a sentence of TFL: 
\begin{itemize}
	\item [3.-5.] If \metaX and \metaY are sentences then so are $(\metaX\eand\metaY)$, $(\metaX\eor\metaY)$ and $(\metaX\eif\metaY)$. 
\end{itemize}
But the rule for negation did not include brackets: 
	\begin{itemize}
	\item [2.] If \metaX is a sentence, then so is $\enot\metaX$.
\end{itemize}

Using these rules we write: $$ A\eif\enot B$$ which is to be read as:
$$\almostinvisibleopenbracket\minibrace{A}\eif\minibrace{\enot B}\almostinvisibleclosebracket$$

You might be tempted to write:
	
	$$\almostinvisibleopenbracket A\eif(\enot B)\almostinvisibleclosebracket\qquad\leftsquigarrow \text{wrong!}$$
	
	That would be wrong. Negations do not introduce brackets.

\subsection{Identifying scope with negations}
An important example is $$\enot A\eor B$$ The main connective of the sentence is $\eor$. 
It is to be understood as $$\minibrace{\enot A}\mainconnective{\eor}\minibrace{B}$$


We can see this because it can be obtained from dropping the outer brackets from the sentence $$\almostinvisibleopenbracket\enot A\eor B\almostinvisibleclosebracket$$ 

To instead have $\enot$ as the main connective, one must include brackets: $$\enot(A\eor B).$$ These brackets cannot be dropped because they are inner brackets. 

One way you might want to think about it: $\enot$ binds more strongly than the other connectives. You should do it first, unless there's brackets telling you explicitly to first do something else. 

Similarly, in $$\enot (A\eand B)\eif C$$
The main connective is the $\eif$. 
$$\enot (A\eand B)\mainconnective{\eif} C$$

The scope of the $\enot$ is $\enot(A\eand B)$ rather than the whole sentence. 
...
\mathclose{)}
\mathbin{\tikzmarknode{if}{\eif}}
\mathord{\tikzmarknode{c}{C}}
\mathclose{\tikzmarknode{wholeclosebracket}{\almostinvisibleclosebracket}}
\]

\begin{tikzpicture}[remember picture,overlay]
	% A and B
	\scopefitbracewithcoordinate{a}{b}{and}[1]

	% not (A and B)
	\scopefitbracewithcoordinate{not}{b}{not}[2]
	
	% C
	\scopebrace{c}

	% whole formula
	\scopefitbracewithcoordinate{wholeopenbracket}{wholeclosebracket}{if}[3]
\end{tikzpicture}


\section{More bracketing rules}\label{TFLconventions}
\subsection{Binding strength}
We give another rule for brackets. 

In arithmetic, there is a convention telling you in which order to apply operations. You learn that you have to do a $\times$ calculation before the $+$, unless there are brackets instructing you otherwise. You learn to treat $2+3\times 4$ as $2+(3\times 4)$.

We can describe this as that $\times$ binds more strongly than $+$. We apply an exactly analogous rule in logic: 
$\eand$ and $\eor$ bind more strongly than $\eif$. You need to calculate their truth value before you calculate that with the $\eif$. 

So $$A\eand B\eif C$$ is to be understood as shorthand for $(A\eand B)\eif C$ or, even more carefully: $((A\eand B)\eif C)$. The brackets are allowed to be dropped, writing simply $A\eand B\eif C$. 

In $A\eand B\eif C$, the scope of $\eand$ is narrower than that of $\eif$. In determining the truth value, one first works out the truth value of $A\eand B$.  
\[
\mathopen{\tikzmarknode{wholeopenbracket}{\almostinvisibleopenbracket}}
\mathord{\tikzmarknode{a}{A}}
\mathbin{\tikzmarknode{and}{\eand}}
...
\begin{tikzpicture}[remember picture,overlay]
	...
	% whole formula
	\scopefitbracewithcoordinate{wholeopenbracket}{wholeclosebracket}{if}[2]
...
Similarly $$A\eand B \eif C\eor D$$ is shorthand for $(A\eand B)\eif (C\eor D).$
\[
\mathopen{\tikzmarknode{wholeopenbracket}{}}
\mathord{\tikzmarknode{a}{A}}
\mathbin{\tikzmarknode{and}{\eand}}
\mathord{\tikzmarknode{b}{B}}
\mathbin{\tikzmarknode{if}{\eif}}
\mathord{\tikzmarknode{c}{C}}
\mathbin{\tikzmarknode{or}{\eor}}
\mathord{\tikzmarknode{d}{D}}
\mathclose{\tikzmarknode{wholeclosebracket}{}}
\]

\begin{tikzpicture}[remember picture,overlay]
	% A and B
	\scopefitbracewithcoordinate{a}{b}{and}

	% whole formula
	\scopefitbracewithcoordinate{wholeopenbracket}{wholeclosebracket}{if}[2]
	
	\scopefitbracewithcoordinate{c}{d}{or}
\end{tikzpicture}
\smallskip  

There is no rule that tells us which of $\eand$ or $\eor$ is stronger. $A\eand B\eor C$ is dangerously ambiguous and must not be written. You have to include brackets, giving either $(A\eand B)\eor C$ or $A\eand (B\eor C)$. 

\subsection{Brackets in strings of conjunctions or disjunctions}
There is one final bracketing rule which we will use, although we will not properly talk about it until later: 
Brackets in strings of $\eand$ may be dropped. When you read a sentence with a string of $\eand$ and no brackets, one can read it as an abbreviation for the sentence adding brackets back towards the left. 

$P\eand Q\eand R$ abbreviates $(P\eand Q)\eand R$.
The reason that this is a sensible rule, is that it doesn't actually matter which order we compute this. $(P\eand Q)\eand R$ is logically equivalent to $P\eand(Q\eand R)$. (We will introduce the notion of logical equivalence in \cref{sec:logical equivalent}).

Similarly for $\eor$: 
$P\eor Q\eor R$ abbreviates $(P\eor Q)\eor R$. 

There is however no rule for mixing $\eand$ and $\eor$. 
$P\eand Q\eor R$ is not allowed to be written. 

There is also no analogous rule for $\eif$: You must not write $P\eif Q\eif R$. 
%We do this because $(P\eif Q)\eif R$ is not logically equivalent to $P\eif(Q\eif R)$. 


\subsection{Summarising our bracketing rules}\label{sec:TFLBrackets}
\begin{definition}
	\begin{itemize}
		\item Outer brackets may be dropped.
		\begin{itemize}
			\item []
		$P\eand Q$ is an abbreviation of $(P\eand Q)$.
		\end{itemize}
		\item $\eand$ and $\eor$ bind more strongly than $\eif$.
		\begin{itemize}
			\item [] $P\eand Q\eif R\eor S$  is an abbreviation of  $(P\eand Q)\eif (R\eor S)$
		\end{itemize}
		\item Brackets in strings of $\eand$ or $\eor$ may be dropped. \\A sentence with such brackets dropped is an abbreviation of one where they are added to the left.
		\begin{itemize}
			\item []$P\eand Q\eand R$  is an abbreviation of  $(P\eand Q)\eand R$.\item[]
		$P\eor Q\eor R$  is an abbreviation of  $(P\eor Q)\eor R$.
		\end{itemize}
	\end{itemize} 
\end{definition}
...
\begin{practiceproblems}
	...
	\problempart 
	Add brackets to form a sentence of TFL.
	\begin{center}
	...
	\problempart
	\label{pr:wiffTFL}
	For each of the following: (a) Is it a sentence of TFL, strictly speaking? (b) Is it a sentence of TFL, allowing for our relaxed bracketing conventions? (c) If the answer to (b) is yes, include braces indicating the scope of each connective.
	\begin{problemslist}
		...
		\item $((A \eif S) \eif W) \eand (J \eor A)$\hfill \myanswer{(a) no (b) yes}
	...
	\begin{problemslist}
		...
		\item Does $\enot(A\eor B)$ have the form $\metaX\eor \metaY$?
		\item Does $(A\eand B)\eor C$ have the form $\metaX\eor\metaY$?
		\item Does $(A\eand B)\eor C$ have the form $\metaX\eand\metaY$?
	\end{problemslist}
	\problempart
	\begin{problemslist}
...
%So far, we have only  focused on simple sentences of TFL; but more complex sentences can be obtained by adding additional connectives. 
% 
%%For example, we already have that $\enot A$ is a sentence of TFL, we can combine this with a new atomic sentence, $C$, to form a complex sentence of TFL: $\enot A\eand C$. 
...
%
%%We have introduced the basic building blocks of truth-functional logic, the atomic sentences, and the connectives, which allow us to conjoin different sentences to form new sentences. The third component of the vocabulary of TFL is brackets. These play a role like punctuation marks. 
%
%We will now discuss the rules of grammar of TFL. Two important roles of grammar are to distinguish wellformed sentences from nonsense and to avoid ambiguity. It is in the grammar that brackets are vital parts of TFL. 
%
...
%\begin{samplesent}
%\item\label{sent:engamb1} John's tired, and Sue's tall or Rob's short.
%\item\label{sent:engamb2} John's tired and Sue's tall, or Rob's short.
%\end{samplesent}
...
%
%You might know this use of brackets from mathematics. Consider:
%\begin{samplesent}
%\item\label{sent:mathamb} $9 + 3 \times 4$
%\end{samplesent}One could add brackets to this equation to obtain: 
%\begin{samplesent}
%\item\label{sent:mathamb1} $9 + (3 \times 4) \qquad(=9+12=21)$
%\item\label{sent:mathamb2} $(9+3) \times 4 \qquad(=12\times 4=48)$
%\end{samplesent}
...
%
%
%\section{Sentences of TFL}\label{sec:TFLSentences}
%\todo{This went through a major rewrite. Check it!!!}
%
%\begin{highlighted}\label{def:TFLsentences}
%	
%	The {sentence letters} of TFL are the capital letters 
%	\begin{center}
%		$A$, $B$, $C$,\ldots $W$,
...
%
%	
%	\medskip Sentences of TFL are given by the following rules:
%	\begin{enumerate}
%		\item Sentence letters are sentences of TFL; they are called the \define{atomic sentences}.
%		\item If \metaX is a sentence of TFL, then $\enot\metaX$ is a sentence of TFL.
%		\item If \metaX and \metaY are sentences of TFL, then $(\metaX\eand\metaY)$ is a sentence of TFL.
%		\item If \metaX and \metaY are sentences of TFL, then $(\metaX\eor\metaY)$ is a sentence of TFL.
%		\item If \metaX and \metaY are sentences of TFL, then $(\metaX\eif\metaY)$ is a sentence of TFL.
...
%
%\begin{enumerate}
%\item[1.] Tells us that atomic sentences are sentence of TFL.
%\end{enumerate}
%Recall that any uppercase Roman letters $A$--$W$, or with subscripts, e.g., $A_1, B_3, A_{100}, J_{375}$, are atomic sentences of TFL. Notice $X$, $Y$, and $Z$ are not atomic sentences. They are so-called \define{metavariables} and used as place holders for sentences of TFL (see more on metavairables in \cref{sec:UseMention}).
%
%
%
...
%\end{forest}
%\end{center}
%The different formations will be important for determining the truth values of the sentences on different valuations. $((A\eand B)\eor C)$ and $(A\eand (B\eor C))$ will differ in when they are true.
%
%
...
%\subsection{Outer brackets}
%
%Strictly speaking, $A\eand B$ is not a sentence of TFL. When we introduce a connective $\eand,\eor$ or $\eif$, strictly speaking, we must include brackets. Only $(A\eand B)$ is strictly speaking a sentence of TFL. 
...
%
%When working with TFL, however, it will make our lives easier if we are sometimes a little less than strict. So, here is a convenient convention.
%\begin{enumerate}
%\item We can remove \emph{outermost} brackets of a sentence. 
...
%\end{enumerate}
%Thus we allow ourselves to write $A\eand B$ instead of the sentence $(A\eand B)$. However, we must remember to put the brackets back in, when we want to embed the sentence into a more complicated sentence!
%
%So we can write $$A\eand(B\eor C)$$ dropping the outer brackets from the strictly speaking formation $$(A\eand (B\eor C))$$ But we cannot drop the brackets in the inner `$(B\eor C)$'. This would result in $$A\eand B\eor C$$ which would be dangerously ambiguous.
%
%An important example is $$\enot A\eor B$$
%Here, this can be understood as the outer brackets being dropped from $$(\enot A\eor B)$$ The main connective of $\enot A\eor B$ is $\eor$ not $\enot$. It has the following formation tree: 
%\begin{center}
%	\begin{forest}
%	[$\enot A\mainconnective{\eor}B$
%	[$\mainconnective{\enot} A$[$A$]]
%	[$B$]
%	]
%\end{forest}
%\end{center}
...
%\begin{center}
%	\begin{forest}
%		[$\mainconnective{\enot} (A\eor B)$
%		[$\almostinvisibleopenbracket A\mainconnective{\eor}B\almostinvisibleclosebracket $
%		[$A$][$B$]
%		]
%		]
%	\end{forest}
...
%\subsection{Binding strength}
%One further rule:
%$\eand$ and $\eor$ will be said to bind more strongly than $\eif$. This is like in arithmetic where $\times$ binds stronger than $+$. In $2+3\times 4$ you learn to treat it as $2+(3\times 4)$. 
%
%So $A\eand B\eif C$ is to be understood as shorthand for $(A\eand B)\eif C$, or, even more carefully: $((A\eand B)\eif C)$. Similarly $A\eand B \eif C\eor D$ is shorthand for $(A\eand B)\eif (C\eor D)$. 
%
%There is no rule that tells us which of $\eand$ or $\eor$ is stronger. So $A\eand B\eor C$ is dangerously ambiguous: $(A\eand B)\eor C$ or $A\eand (B\eor C)$. 
%%\subsection{Strings }
%
%\end{color}
%
...
In \cref{sec:Truth rules} we gave truth rules for the connectives of TFL. Now we will more carefully talk about applying them to complex sentences. 

We will make use of the truth rules along with our instructions about reading a sentence and which order it was formed. 


Whether $A\eand B$ is true depends on whether Alice is happy and whether Beth is happy.
There are four possible combinations. There are four valuations. Recall that a valuation is any assignment of true and false to each atomic sentence. 
\begin{center}
	\begin{tabular}{rcc|c}
	&	$A$&$B$&$A\eand B$\\\hline
	$v_1$&	T&T&T\\
...
What about $\enot(A\eand B)$? This will make use of our truth rule for its main connective, $\enot$. The only difference now is that it is applied not to an atomic sentence but something that is more complicated. Recall the truth rule for $\enot$ describing whether $\enot \metaX$ is true or false on a valuation depending on whether $\metaX$ is true or false on that valuation. 
\begin{center}
	\begin{tabular}{ccc}
	If \metaX is:&&then $\enot\metaX$ is:\\
	T &$\leadsto$& F\\
...
So suppose we are in a case where neither Alice nor Beth are happy ($v_4$). Then $(A\eand B)$ is false. So, according to our rule,  $\enot(A\eand B)$ is true. To keep track of this reasoning in the four possibilities, we write the truth value of the sentence underneath its main connective, in this case, $\enot$. That is, in $v_1$, $\enot (A\eand B)$ is false, and in $v_2$, $v_3$ and $v_4$ it is true. 
\begin{center}
	\begin{tabular}{rcc|d*{4}{e}f}
		&$A$&$B$
		&\tikzmarknode{not}{$\enot$}
		&\tikzmarknode{openbracket}{$($}
		&\tikzmarknode{a}{$A$}
		&\tikzmarknode{and}{$\eand$}
		&\tikzmarknode{b}{$B$}
		&\tikzmarknode{closebracket}{$)$}
		\\[3\scopebracedepthunit]\hline
		\truthrowstart{truthline}$v_1$&T&T&\TTbf{F}& & & T & & \\
		$v_2$&T&F&\TTbf{T}& & & F & & \\
		$v_3$&F&T&\TTbf{T}& & & F & & \\
		$v_4$&F&F&\TTbf{T}& & & F & &\\
			&&&\tikzmarknode{negcolumn}{}
				&&&\tikzmarknode{andcolumn}{}
				&&
	\end{tabular}
	...
	\begin{tikzpicture}[remember picture,overlay]
		...
		% not (A and B)
		\scopefitbracewithcoordinate{not}{closebracket}{not}[2]

		% truth-value arrows
		\trutharrows{truthline}{and_brace,not_brace}
		
		\draw[-{Stealth[length=3pt]}] (andcolumn) to[bend left=40] (negcolumn);
		
	\end{tikzpicture}
\end{center}
We write the truth value for each sub-sentence underneath its main connective, as indicated by the red dot. We highlight the column giving the truth value for the overall sentence to indicate that it is the final answer, and the other column was just a calculation column. 
...
We now say what a complete truth table is:
\begin{definition}
...
For example, a complete truth table is:
\begin{center}
	\begin{tabular}{rcc|l}
		&$A$&$B$&$\enot(A\eand B)$\\\hline
		$v_1$&T&T&F\\
		$v_2$&T&F&T\\
		$v_3$&F&T&T\\
		$v_4$&F&F&T
	\end{tabular}
\end{center}
Which one can also display with calculation stages, as we did above, where one then needs to highlight the column which gives the final answer:
\begin{center}
	\begin{tabular}{rcc|d*{4}{e}f}
		&$A$&$B$
		&\tikzmarknode{not}{$\enot$}
...
We will now turn to giving further worked examples of determining complete truth tables. Before doing so, we recall the truth rules introduced in \cref{def:Truthint}:
\begin{highlighted}
	% 	Let $v$ be a valuation. 
	%	\begin{enumerate}
		%		\item An atomic sentence $X$ is true relative to $v$, if and only if $v$ assigns the value T to $X$.
...
To determine the complete truth table for an arbitrary sentence of TFL, one just needs to apply these truth rules in the right order, which is determined by considering the formation order of the sentence. 

\section{Complete Truth Tables}

Consider any sentence of TFL, for example $$(\enot I\eand H)\eif H$$
Our task is to draw a complete truth table for this sentence. 

The first task is to identify the atomic sentences that appear in it, in this case $I$ and $H$, and list all the valuations, as described in \cref{sec:ValsAll}
...
We want to find out the truth values of `$(\enot I\eand H)\eif H$' on each of the four valuations.

The next step is to identify the formation structure of this sentence, which we do by drawing braces: 

\begin{center}
	\begin{tabular}{d*{6}{e}f}
%		&$I$&$H$
		\tikzmarknode{open}{$($}
		&\tikzmarknode{not}{$\enot$}
		&\tikzmarknode{i}{$I$}
		...
		&\tikzmarknode{close}{$)$}
		&\tikzmarknode{if}{$\eif$}
		&\tikzmarknode{hright}{$H$}
	\end{tabular}

	\begin{tikzpicture}[remember picture,overlay]
		% not I
		\scopefitbracewithcoordinate{not}{i}{not}[1]
		...
		% (not I and H)
		\scopefitbracewithcoordinate{not}{hleft}{and}[2]

		% whole formula
		\scopefitbracewithcoordinate{open}{hright}{if}[3]
	\end{tikzpicture}
\end{center}


We will apply the different truth rules in turn, and write the answer underneath the main connective of each sub-sentence

\begin{center}
	\begin{tabular}{rcc|d*{6}{e}f}
		&$I$&$H$
		&\tikzmarknode{open}{$($}
		&\tikzmarknode{not}{$\enot$}
		&\tikzmarknode{i}{$I$}
		...
		&\tikzmarknode{if}{$\eif$}
		&\tikzmarknode{hright}{$H$}
		\\[4\scopebracedepthunit]\hline
		\truthrowstart{truthline}
				$v_1$&T&T&\\
				$v_2$&T&F&\\
				$v_3$&F&T&\\
				$v_4$&F&F&
	...
	\begin{tikzpicture}[remember picture,overlay]
		...
		\scopefitbracewithcoordinate{open}{hright}{if}[3]
			...
			\trutharrows{truthline}{not_brace,hleft_brace,hright_brace,and_brace,if_brace}
	\end{tikzpicture}
\end{center}


We cannot work out the truth of the overall sentence until we have determined the truth of its components. 

We first calculate $\enot I$, which is to simply apply the rule for $\enot$ from the truth value for $I$, which is specified directly by the valuation. 
...
\begin{center}
	\begin{tabular}{rcc|d*{6}{e}f}
		...
		$v_3$&F&T&&{T}&&&&&&\\
		$v_4$&F&F&&{T}&&&&&&\\
		&\tikzmarknode{Icolumn}{}&&&\tikzmarknode{negcolumn}{$\star$}&&&&&&
	\end{tabular}
	...
	\begin{tikzpicture}[remember picture,overlay]
		...
		% truth-value arrow for not I
	\trutharrows{truthline}{not_brace,hleft_brace,hright_brace,and_brace,if_brace}

		% curved arrow from I valuation column to the negation column
\draw[-{Stealth[length=3pt]}]
	(Icolumn) to[bend right=40] node[pos=.5,below] {$\enot$} (negcolumn);
	\end{tikzpicture}
\end{center}

Now to determine the truth value for $\enot I\eand H$, we take our truth value for $\enot I$, which we have calculated, along with the truth value for $H$, and apply our rule for $\eand$. For convenience, we first copy the value for $H$:
...
\begin{center}
	\begin{tabular}{rcc|d*{6}{e}f}
		...
		$v_3$&F&T&&{T}&&&T&&&\\
		$v_4$&F&F&&{T}&&&F&&&\\
		&&\tikzmarknode{column1}{}&&&&&\tikzmarknode{currentcolumn}{$\star$}&&&
	\end{tabular}
	...
	\trutharrows{truthline}{not_brace,hleft_brace,hright_brace,and_brace,if_brace}
		...
		% curved arrow from I valuation column to the negation column
\draw[-{Stealth[length=3pt]}]
	(column1) to[bend right=40] node[pos=.5,below] {copy} (currentcolumn);
	\end{tikzpicture}
	\medskip 
\end{center} 

Then we can apply the rule for $\eand$, applied to our two columns so far: 
...
\begin{center}
	\begin{tabular}{rcc|d*{6}{e}f}
		...
		$v_3$&F&T&&{T}&&{T}&T&&&\\
		$v_4$&F&F&&{T}&&{F}&F&&&\\
		&&&&\tikzmarknode{column1}{}&&\tikzmarknode{currentcolumn}{$\star$}&\tikzmarknode{column2}{}&&&
	\end{tabular}
\begin{tikzpicture}[remember picture,overlay]
	% not I
	\scopefitbracewithcoordinate{not}{i}{not}[1]
	...
	% truth-value arrows for all current scopes
	\trutharrows{truthline}{not_brace,hleft_brace,hright_brace,and_brace,if_brace}


	% joined arrows showing the inputs for the conjunction
	\coordinate (currentcolumnjoin) at ($(currentcolumn) + (0,-10pt)$);

	\node at ($(currentcolumnjoin)+(0,-0.2cm)$) {$\eand$};

	\draw[-]
		(column1) to[bend right=20] (currentcolumnjoin);

	\draw[-]
		(column2) to[bend left=20] (currentcolumnjoin);

	\draw[-{Stealth[length=3pt]}]
		(currentcolumnjoin) -- (currentcolumn);
\end{tikzpicture}
\bigskip 
\end{center}

We can then start looking at the truth value for our overall sentence, which will combine the truth value for $(\enot I\eand H)$ we have just calculated with the truth value for $H$, using the rule for $\eif$. To make sure we apply the rule for $\eif$ the right way around, we will copy across the truth value for $H$: 
...
\begin{tikzpicture}[remember picture,overlay]
	...
	% joined arrows showing that we copy across H
	\draw[-{Stealth[length=3pt]}]
		(column1) to[bend right=30] node[pos=.5,below] {copy} (currentcolumn);
\end{tikzpicture}
\medskip
\end{center}

Finally, we can apply our rule for $\eif$. 
...
\begin{center}
	\begin{tabular}{rcc|d*{6}{e}f}
		...
		\\[4\scopebracedepthunit]\hline
		\truthrowstart{truthline}
		$v_1$&T&T&&{F}&&{F}&T&&\TTbf{T}&T\\
		$v_2$&T&F&&{F}&&{F}&F&&\TTbf{T}&F\\
		$v_3$&F&T&&{T}&&{T}&T&&\TTbf{T}&T\\
		$v_4$&F&F&&{T}&&{F}&F&&\TTbf{T}&F\\
		&&&&&&\tikzmarknode{column1}{}&&&\tikzmarknode{currentcolumn}{$\star$}&\tikzmarknode{column2}{}
	\end{tabular}
...
\begin{tikzpicture}[remember picture,overlay]
	...
	% joined arrows showing the inputs for the conditional
	\coordinate (currentcolumnjoin) at ($(currentcolumn) + (0,-10pt)$);

	\node at ($(currentcolumnjoin)+(0,-0.2cm)$) {$\eif$};
...
We highlight this final column as it indicates our final answer: it displays the truth value of the sentence on each valuation. 

Now, many of these columns were calculation columns, and the answer could be given without including them, unless the question on the exam explicitly asks them to be given. That is, we could give the complete truth table as:
...
\section{Complete truth tables for multiple sentences simultaneously}

Often we will want to consider multiple sentences simultaneously. 
...
We thus need to do complete truth tables for the three sentences simultaneously. 
...
We will work in stages, applying the appropriate truth rule as we go to determine the truth value of the sentence on each valuation. 

The result will be:
\begin{center}
	\begin{tabular}{rcc|d*{6}{e}f|df|d*{4}{e}f}
		Valuation&$J$&$L$
		&\tikzmarknode{notLone}{$\enot$}
		&\tikzmarknode{Lone}{$L$}
		&\tikzmarknode{ifone}{$\eif$}
		&\tikzmarknode{openone}{$($}
		&\tikzmarknode{Jone}{$J$}
		&\tikzmarknode{orone}{$\eor$}
		&\tikzmarknode{Ltwo}{$L$}
		&\tikzmarknode{closeone}{$)$}
		&\tikzmarknode{notJ}{$\enot$}
		&\tikzmarknode{Jtwo}{$J$}
		&\tikzmarknode{opentwo}{$($}
		&\tikzmarknode{notLtwo}{$\enot$}
		&\tikzmarknode{Lthree}{$L$}
		&\tikzmarknode{iftwo}{$\eif$}
		&\tikzmarknode{Jthree}{$J$}
		&\tikzmarknode{closetwo}{$)$}
		\\[3\scopebracedepthunit]\hline
		\truthrowstart{truthline}
		$v_1:$&T&T
		&F&&\TTbf{T}&&&T&&
	...
	\begin{tikzpicture}[remember picture,overlay]
		% first sentence: not L if (J or L)
		\scopefitbracewithcoordinate{notLone}{Lone}{notLone}[1]
		\scopefitbracewithcoordinate{Jone}{Ltwo}{orone}[1]
		\scopefitbracewithcoordinate{notLone}{closeone}{ifone}[2]




		% third sentence: (not L if J)
		\scopefitbracewithcoordinate{notLtwo}{Lthree}{notLtwo}[1]
		\scopefitbracewithcoordinate{notLtwo}{Jthree}{iftwo}[2]
		\scopebrace{Jthree}

		% truth-value arrows
		\trutharrows{truthline}{notLone_brace,orone_brace,ifone_brace,Jthree_brace,notLtwo_brace,iftwo_brace}
	\end{tikzpicture}
\end{center}


Again, many of these are calculation stages, and an answer without calculation would be: 
...
\begin{definition}A TFL-argument $X_1,\ldots,X_n\therefore Y$ is \define{valid} if and only if there is no valuation $v$ such that all the premises, $X_1,\ldots,X_n$ are true relative to $v$ but $Y$ is false relative to $v$.\end{definition}
...
The complete truth table for this argument is: 
\begin{center}
	\begin{tabular}{rcc|c|c|c}
		Valuation&$J$&$L$&$\enot L \eif (J \eor L)$&$\enot J$&$(\enot L\eif J)$\\\hline
		$v_1:$&T&T&T&F&T\\
...
When doing this in practice, your truth table might include the calculation columns, but these are ignored for the purposes of determining validity. 
...
\begin{center}
	\begin{tabular}{r c c | def | df | df}
		...
		Valuation & $D$ & $V$ & $D$ & ${\eif}$ & $V$ & ${\enot}$ & $D$ & ${\enot}$ & $V$\\[.3em]
		%		Valuation & $D$ & $V$ & $D$ & &$V$& & $D$ &  & $V$\\
		\hline
...
\begin{center}
	\begin{tabular}{rccc|d*{5}{e}f|df|def}
		&&&&\multicolumn{7}{c|}{premise 1}
		&\multicolumn{2}{c}{prem 2}
		...
		&\tikzmarknode{done}{$D$}
		&\tikzmarknode{closeone}{$)$}
		&\tikzmarknode{nottwo}{$\enot$}
		&\tikzmarknode{ctwo}{$C$}
		&\tikzmarknode{pthree}{$P$}
		&\tikzmarknode{ifthree}{$\eif$}
		&\tikzmarknode{dthree}{$D$}
		\\[3\scopebracedepthunit]\hline
		\truthrowstart{truthline}
		$v_1$&T&T&T
		&T&\TTbf{T}&&&{T}&&
	...
	\begin{tikzpicture}[remember picture,overlay]
		...
		% premise 1: P if (C or D)
		\scopefitbracewithcoordinate{cone}{done}{orone}[1]
		\scopefitbracewithcoordinate{pone}{closeone}{ifone}[2]

%		% conclusion: P if D

		% truth-value arrows
		\trutharrows{truthline}{orone_brace,ifone_brace,pone_brace}
	\end{tikzpicture}
\end{center}

There is no valuation where both premises are true and the conclusion is false. So the argument is valid.
...
\[
	A\eor B,\ \enot(B\eand \enot C) \therefore \enot C\eor A
\]

\begin{center}
	\begin{tabular}{rccc|def|d*{5}{e}f|d*{2}{e}f}
		&&&&\multicolumn{3}{c|}{premise 1}
		&\multicolumn{7}{c||}{premise 2}
		&\multicolumn{4}{c}{conclusion}\\
		&$A$&$B$&$C$
		&\tikzmarknode{aone}{$A$}
		&\tikzmarknode{orone}{$\eor$}
		&\tikzmarknode{bone}{$B$}
		...
		&\tikzmarknode{opentwo}{$($}
		&\tikzmarknode{btwo}{$B$}
		&\tikzmarknode{andtwo}{$\eand$}
		&\tikzmarknode{notCtwo}{$\enot$}
		&\tikzmarknode{ctwo}{$C$}
		&\tikzmarknode{closetwo}{$)$}
		&\tikzmarknode{notCthree}{$\enot$}
		&\tikzmarknode{cthree}{$C$}
		&\tikzmarknode{orthree}{$\eor$}
		&\tikzmarknode{athree}{$A$}
		\\[4\scopebracedepthunit]\hline
		\truthrowstart{truthline}
		$v_1$&T&T&T
		&&\TTbf{\textbf{T}}&
	...
	\begin{tikzpicture}[remember picture,overlay]
		% premise 1: A or B
		\scopefitbracewithcoordinate{aone}{bone}{orone}[1]

		% premise 2: not (B and not C)
		\scopefitbracewithcoordinate{notCtwo}{ctwo}{notCtwo}[1]
		\scopefitbracewithcoordinate{btwo}{ctwo}{andtwo}[2]
		\scopefitbracewithcoordinate{nottwo}{closetwo}{nottwo}[3]

		% conclusion: not C or A
		\scopefitbracewithcoordinate{notCthree}{cthree}{notCthree}[1]
		\scopefitbracewithcoordinate{notCthree}{athree}{orthree}[2]

		% truth-value arrows
		\trutharrows{truthline}{orone_brace,notCtwo_brace,andtwo_brace,nottwo_brace,notCthree_brace,orthree_brace}
	\end{tikzpicture}
\end{center}

This argument is invalid. In $v_5$, both premises are true and the conclusion is false.
...
\section{Tautologies and contradictions}\label{sec:TautologiesAndContradictions}
...
%In the example of \cref{sec:CompleteTruthTables}, `$((C\eiff C) \eif C) \eand \enot(C \eif C)$' is a contradiction.
...
\section{Logical equivalence}\label{sec:logical equivalent}
Here is another useful notion:
\begin{definition}
	$\metaX$ and $\metaY$ are \define{logically equivalent} iff they are true on exactly the same valuations.
\end{definition}
...
\begin{practiceproblems}
	\problempart
	Revisit your answers to \cref{pr:TT:TTorC}. Determine which sentences were tautologies, which were contradictions, and which were neither tautologies nor contradictions.
	\solutions
	...
	\begin{problemslist}
		...
		\item $(A \eand (A \eor B)) \eif B$ and $A \eif B$\end{problemslist}
...
\begin{center}
	\begin{tabular}{c c|d e e e e e e f}
		...
		\hline
		T & T & F & & F & F& & \TTbf{F} & F\\
		T & F &  F & & T&\TTcross & &  \TTbf{T} & \TTcross\\
		F & T &  & & \TTcross &  & & \TTbf{T} & T\\
		F & F &  & & \TTcross & & &\TTbf{T} & T
	\end{tabular}
\end{center}
...
\begin{center}
	\begin{tabular}{c c|d e e e e e e f}
		...
		\hline
		T & T &  \TTcross &  & &   & & \TTbf{F} & F\\
		T & F &   \TTcross &  &&   & & \TTbf{F} & F\\
		F & T & T &  & F &  & & \TTbf{T} & T\\
		F & F & T &  & F & & & \TTbf{T} & T
	\end{tabular}
\end{center}
...
\begin{center}
	\begin{tabular}{c c|d e e e e f |d f|c}
		...
		\hline
		%J   L   -   L      ->     (J   v   L)
			T & T & &&   \TTcross&& & &\TTcross & & {T}\\
			T & F & &&   \TTcross && & &\TTcross & & {T}\\
		F & T & &&?&&&&?&& {F}\\
		F & F & &&?&&&&?&& {F}
	\end{tabular}
\end{center}
where the `$\TTcross$' marks indicate entries that we are not going to bother filling in (since the line is already known not to be bad), and the question-marks indicate that we still need to keep investigating.
...
\chapter{Partial truth tables}\label{sec:PartialTruthTable}\teachingnote{Not taught in course. This knowledge will not be needed for the exam. (Although you can use these techniques in the exam if you wish.)}
...
%
%\factoidbox{\label{def:logical truth}A TFL-sentence $X$ is called a \define{logical truth} or \define{tautology} if and only if the TFL-argument $\therefore X$ is valid.
%
...
%
%
%\factoidbox{\label{def:TFLcons}$Y$ is a \define{logical consequence} of $X_1,\ldots,X_n$ if and only if the argument $X_1,\ldots,X_n\therefore Y$ is valid.}
%
...
%\factoidbox{\label{def:TFLcontra}$X$ is a \define{logical contradiction} if and only if there is no valuation $v$ such that X is true relative to $v$.}
...
%\factoidbox{\label{def:TFLconsist}A collection of TFL-sentences $X_1,\ldots,X_n$ (with possibly $n=1$) is \define{consistent} iff there is a valuation relative to which $X_1,\ldots,X_n$ are true. Otherwise the collection is \define{inconsistent}.}
%
%\factoidbox{\label{def:TFLconting}A sentence of TFL  is called \define{contingent}. If it is part of a consistent collection and not a logical truth.}
...
%\factoidbox{\label{def:TFLequiv}$X$ and $Y$ are \define{logically equivalent} if and only if $X$ is a logical consequence of $Y$ and $Y$ a logical consequence of $X$.}
%
...
%%(from now on, in contrast to \cref{sec:tt}, we no longer display the explanatory text, and replace $\leadsto$ by a vertical line):\todo{J added this sent. Remove it!!! The squiggly is for the metavar version. }
%%
%%\begin{center}
%%\begin{tabular}{ccc|c}
%%$A$&$B$&&$A\eand B$\\\hline
...
%%
%% \factoidbox{\label{def:Truthint}Let $v$ be a valuation. Then
%%	\begin{enumerate}
%%		\item An atomic sentence $X$ is true relative to $v$, if and only if $v$ assigns the value T to $X$.
...
%%
%% \factoidbox{\label{def:TFLvalid}A TFL-argument $X_1,\ldots,X_n\therefore Y$ is \define{valid} if and only if there is no valuation $v$ such that $X_1$ is true relative to $v$ and \ldots ~and $X_n$ is true relative to $v$, but $Y$ is false relative to $v$.}
%%
