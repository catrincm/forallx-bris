...
\begin{highlighted}The \define{main connective} of a sentence is the last connective that was introduced in the construction of the sentence.\end{highlighted}
...
\subsection{Summarising our bracketing rules}
\begin{highlighted}
	\begin{itemize}
		\item Outer brackets may be dropped.\\
		$P\eand Q$ abbreviates $(P\eand Q)$.
		\item $\eand$ and $\eor$ bind more strongly than $\eif$.\\
		$P\eand Q\eif R\eor S$ abbreviates $(P\eand Q)\eif (R\eor S)$
...
\begin{highlighted}Let $v$ be a valuation. Then
	\begin{enumerate}
		\item An atomic sentence $X$ is true relative to $v$, if and only if $v$ assigns the value T to $X$.
...
\begin{highlighted}\label{def:TFLvalid}A TFL-argument is valid if and only if there is no valuation on which all the premises are true but the conclusion is false. 
	
	That is, `$X_1,\ldots,X_n\therefore Y$' is \define{valid} if and only if there is no valuation $v$ such that $X_1$ is true relative to $v$ and $X_2$ is true relative to $v$, and so on, up to $X_n$ being true relative to $v$, but $Y$ is false relative to $v$.\end{highlighted}
...
\begin{highlighted}A TFL-argument $X_1,\ldots,X_n\therefore Y$ is \define{valid} if and only if there is no valuation $v$ such that all the premises, $X_1,\ldots,X_n$ are true relative to $v$ but $Y$ is false relative to $v$.\end{highlighted}
