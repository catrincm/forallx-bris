...
\section{Syntactic rules of TFL}
...
%	$$A, B, C, A_1, B_4, A_{25}, J_{375},\ldots$$
%These are sentences of the object language (TFL). They are not sentences of English. So we should not say, for example:
%	\begin{ebullet}
%		\item $D$ is an atomic sentence of TFL.
%	\end{ebullet}
...
%		\item `$D$' is an atomic sentence of TFL.
%	\end{ebullet}
%The general point is that, whenever we want to talk in English about some specific expression of TFL, we need to indicate that we are \emph{mentioning} the expression, rather than \emph{using} it. We can either deploy quotation marks, or we can adopt some similar convention, such as  placing it centrally in the page. We do adopt a convention that allows us to drop all quotation around logic. 


\section{Metavariables}\label{s.Metavariables}
Often we do not just want to talk about \emph{specific} expressions of TFL. We also want to be able to talk about \emph{any arbitrary} sentence of TFL. Indeed, we had to do this in \S\ref{s:TFLSentences}, when we presented the recursive definition of a sentence of TFL. We used letters from the end of the alphabet to do this, namely:
	$$\metaX, \metaY, \metaZ, \metaX_1,\metaY_{100},\ldots$$
These symbols do not belong to TFL. Rather, they are part of our (augmented) metalanguage that we use to talk about \emph{any} expression of our formal language. To repeat the second clause of the recursive definition of a sentence of TFL, we said:
	\begin{earg}
...
This talks about \emph{arbitrary} sentences. If we had instead offered:
	...
	\end{ebullet}
this would not have allowed us to determine whether $\enot B$ is a sentence, it would only tell us about the specific case for $A$. We don't want to say something about $A$ specifically, but about all sentences. For example this should also give us that \begin{ebullet}
\item If $(A\eor (B\eand C))$ is a sentence, then $\enot \,(A\eor (B\eand C))$ is a sentence.
\end{ebullet} 
We get this by using a metavariable $\metaX$. It stands in place of an arbitrary expression. 

To emphasize, then:
	\factoidbox{
	  $\metaX$ (also $\metaY,\metaZ$) is a symbol (called a \define{metavariable}) in augmented English, which we use to talk about any expression of our formal language. 	$A$ (also $B,C,\ldots$) is a particular atomic sentence of TFL.}
...
%\end{quote}
%and similarly, for expressions like $(\metaX \eand \metaY)$, $(\metaX \eor \metaY)$, etc. So then 2$'$ is the correct form of the recursive clauses. 
%
%
...
%	$$\metaX_1, \metaX_2, \ldots, \metaX_n \therefore \metaZ$$
%abbreviates:
%	\begin{quote}
%		the argument with premises $\metaX_1, \metaX_2, \ldots, \metaX_n$ and conclusion $\metaZ$
%	\end{quote}
