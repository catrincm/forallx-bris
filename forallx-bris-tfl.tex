...
The vocabulary of TFL is very simple. Sentences are built from three components: atomic sentences, connectives, and brackets. 
...
\subsection{Brackets}
Brackets are the remaining component of the vocabulary of TFL. They will be used to provide the grammar. They are vital when we start looking at more complex sentences of TFL. But we will first start to discuss the connectives applied just to simple sentences so can ignore brackets for now. 


\chapter{The connectives}\label{s:Connectives}
Whilst this provides a gloss on our new symbols of TFL, their role in TFL is explicitly specified. 
In \S\ref{ch:syntfl} we introduced the idea that an argument is valid if there is no interpretation on which all premises of the argument are true but the conclusion false. Accordingly, if we wish to make a start on making this idea more precise, we need to say when a sentence of TFL is true according to an interpretation and when it is false. The target of this chapter is to give precise rules to this effect. The important feature of truth functional logic is that the truth value of a sentence such as $A\eand B$ is determined entirely by the truth value of $A$ and the truth value of $B$. 
...
\section{Negation}
The `$\enot$'-connective is called negation. When we introduced the `$\enot$'-connective we said it should roughly be understood as `it is not the case' or, perhaps, simply `not'. 

If think of `$A$' as representing the English sentences
\begin{ekey}
	\item[A] Alice is happy
\end{ekey}
`$\enot A$' will be used to symbolise the English sentence:
\begin{samplesent}
...
In which cases is `$A\eand B$' true? Only $v_1$. This is:
\begin{center}
	\begin{tabular}{cc|c}
	$A$&$B$&$A\eand B$\\\hline
	T&T&T\\
...
\begin{center}
	...
	\end{tabular}
\end{center}
Clearly `She understands German or French' is false in $v_4$ and true in $v_2$ and $v_3$:
\begin{center}
	\begin{tabular}{rcc|c}
...
What is still missing is to equip the language with a \define{grammar}. The purpose of the grammar is to distinguish wellformed sentences from nonsense, but also to avoid ambiguity. It is in the grammar that brackets are vital parts of TFL. 
...
A valuation is a specification of whether each atomic sentence is true or false. Once you give me a valuation, I can then use the rules associated with the connectives to determine the truth value of the sentence of TFL. Sentences of TFL are true or false relative to a valuation. 
