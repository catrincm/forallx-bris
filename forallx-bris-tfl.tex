...
\begin{center}
	\begin{tabular}{rccc|c|c|c||c}
		...
		&$A$&$B$&$C$&$A \eif B$&$B \eif C$&$A$&$C$\\\hline
...
Now to determine the truth value for $\enot I\eand H$, we take our truth value for $\enot I$, which we have calculated, along with the truth value for $H$, and apply our rule for $\eand$. We first copy the value for $H$:
...
\begin{tikzpicture}[remember picture,overlay]
	...
	\coordinate (currentcolumnjoin) at ($(currentcolumn) + (0,-1.5em)$);
	...
	\draw[-{Stealth[length=3pt]},shorten >=0.1cm]
...
\begin{tikzpicture}[remember picture,overlay]
	...
	\draw[-{Stealth[length=3pt]},shorten >=0.2cm]
...
\begin{tikzpicture}[remember picture,overlay]
	...
	\coordinate (currentcolumnjoin) at ($(currentcolumn) + (0,-1.4em)$);
