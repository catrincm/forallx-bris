...
In this part the lecture notes we start developing our theory of validity and logical consequence, that is, we start introducing the logical language. In \cref{s:Valid} we already introduced the idea that an argument is valid if and only if the truth of the premises guarantees the truth of the conclusion in virtue of their logical form. It is this idea that suggest to spell out an account of validity in a formal language, that is, to conceive of the logical language as a formal language. This will enable us to single out arguments that are valid in virtue of their form and eventually make sense of the notion of an interpretation we used in our definition of validity in \cref{s:Valid}. We can then give a rigorous formal definition of validity of arguments in the formal language we shall devise. This language will be the language of Truth-functional logic (TFL).
...
\chapter{The connectives}\label{s:Connectives}
...
The material conditional is the best that can be done to capture the conditional in a truth-functional way, where the truth can be determined simply from the truth values of its constituent parts. 
 See \url{https://plato.stanford.edu/entries/conditionals} and \cref{s:IndicativeSubjunctive} for further discussion
...
\section{Truth rules}\label{s:Truth rules}
...
\begin{highlighted}\label{Truthint}
...
%\end{samplesent}
%%We will discuss conditional-sentences of this kind in \cref{s:IndicativeSubjunctive} and will look at some problems arising due to understanding $\eif$ in terms of material conditional in \cref{s:ParadoxesOfMaterialConditional}.
%%
%For our purposes $\eif$-connective will be understood in terms of truth rules given in this section.
...
\begin{highlighted}\label{TFLsentences}
...
\end{enumerate}
Recall that any uppercase Roman letters $A$--$W$, or with subscripts, e.g., $A_1, B_3, A_{100}, J_{375}$, are atomic sentences of TFL. Notice $X$, $Y$, and $Z$ are not atomic sentences. They are so-called \define{metavariables} and used as place holders for sentences of TFL (see more on metavairables in \cref{s:UseMention}).
...
In \cref{s:Truth rules} we gave truth rules for the connectives of TFL. Now we can apply them to complex sentences. 
...
`$(\enot I\eand H)\eif H$'. 
The formation tree will help us know what to do (see \cref{TFLsentences}):
\begin{center}
	\begin{forest}
...
We have already shown how to do this on a particular valuation: one first writes down the formation tree of the sentence. (see \cref{TFLsentences}):
\begin{center}
	\begin{forest}
		[$(\enot I\eand H)\mainconnective{\eif} H$
		[$(\enot I\mainconnective{\eand} H)$
...
%In the example of \cref{s:CompleteTruthTables}, `$((C\eiff C) \eif C) \eand \enot(C \eif C)$' is a contradiction.
