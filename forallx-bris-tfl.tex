...
\factoidbox{In order to fill in the columns of a complete truth table, begin
with the right-most atomic sentence and \emph{alternate} between ‘T’ and
‘F’. In the next column to the left, write \emph{two} ‘T’s, write two ‘F’s,
and repeat. For the third atomic sentence letter, write \emph{four} ‘T’s followed
by four ‘F’s. This would yields an eight line truth table. For a 16 line truth table, the next column of sentence
letters should have \emph{eight} ‘T’s followed by eight ‘F’s. For a 32 line
table, the next column would have \emph{16} ‘T’s followed by 16 ‘F’s, and so on}
