...
\begin{enumerate}
\item $\metaX\wedge\metaY$\myanswer{\\the result of placing the symbol `$\eand$' between the two sentences \metaX and \metaY}
\item $\metaY\eif\metaZ$\myanswer{\\the result of placing the symbol `$\eif$' between the two sentences \metaY and \metaZ}
...
\section{Truth rules for the connectives}\label{sec:tt}
...
\begin{center}
	...
	\end{forest}
\end{center}
The idea is that we work ourselves from \define{leaves} of the tree (the atomic sentences) to the \define{root} of the formation tree (the sentence that has been constructed). 
...
Then the rule for $\eand$ tells us how the truth value of $\enot I\eand H$ depends on the truths of $\enot I$ and $H$: as $\enot I$ is true and $H$ is true, then $\enot I \eand H$ is also true on the valuation $v$.
\begin{center}
	\begin{forest}
		[$(\enot I\eand H)\mainconnective{\eif} H$\\
...
%
%\begin{center}
%	\begin{tabular}{c c c| d e e ee e e f}
%\multirow{4}{*}{Valuation}	&	\multirow{4}{*}{$H$}&\multirow{4}{*}{$I$}	& $($ & $\enot$ & $I$ & $\eor$ & $H$ & $)$ & $\mainconnective{\eif}$ & $H$ \\
%	&	& & $\mainconnectivecolor{(}$ & $\enot$ & $I$ & $\mainconnective{\eor}$ & $H$ & $\mainconnectivecolor{)}$ & &$H$ \\
%	&	& & & $\mainconnective{\enot}$ & $I$ & & $H$ & & &\\
%	&	& & & & $I$ & & & & &  \\\hline 
%	$v_1$&T&T&\\
...
\begin{center}
	...
	T & T & \TTbf{F }&   & \textcolor{gray!20}{T} & T &   & F & \textcolor{gray!20}{T} & T & F & \textcolor{gray!20}{T} &   &   \\
	T & F & \TTbf{T} &   & \textcolor{gray!20}{F} & F &   & T & \textcolor{gray!20}{F} & F & F & \textcolor{gray!20}{T} &   &   \\
	F & T & \TTbf{F} &   & \textcolor{gray!20}{T} & F &   & F & \textcolor{gray!20}{T} & T & T & \textcolor{gray!20}{F} &   &   \\
	F & F & \TTbf{T} &   & \textcolor{gray!20}{F} & F &   & T & \textcolor{gray!20}{F} & T & T & \textcolor{gray!20}{F} &   &   \\
\end{tabular}
\end{center}
...
%	& & & \tikz[baseline]{\node(Bleft){$B$};} & & & & & & & & \\[.6em]
%	& & & & & & & \tikz[baseline]{\node(imp){$\eif$};} & & & & \\[.6em]
%	& & & & & \tikz[baseline]{\node(nB){$\enot$};} & & & \tikz[baseline]{\node(nA){$\enot$};} & & & \\[.6em]
%	& & & & & & \tikz[baseline]{\node(Bright){$B$};} & & & \tikz[baseline]{\node(A){$A$};} & & \\
%\end{tabular}
%
%\begin{tikzpicture}[remember picture, overlay, line width=.5pt]
%	% ¬ → ∧
%	\draw (not.south) -- ++(0,-.3) -| (and.north);
%	% ∧ → B
%	\draw (and.south) -- ++(0,-.3) -| (Bleft.north);
%	% ∧ → →
%	\draw (and.south) -- ++(0,-.3) -| (imp.north);
%	% → → ¬B
%	\draw (imp.south) -- ++(0,-.3) -| (nB.north);
%	% ¬B → B
%	\draw (nB.south) -- ++(0,-.3) -| (Bright.north);
%	% → → ¬A
%	\draw (imp.south) -- ++(0,-.3) -| (nA.north);
%	% ¬A → A
%	\draw (nA.south) -- ++(0,-.3) -| (A.north);
%\end{tikzpicture}
...
\begin{center}
	\begin{tabular}{c c c | d e e eeeeee f | d f | d eeee e f}
