...
\factoidbox{The \define{main connective} of a sentence is the last connective that was introduced in the construction of the sentence.}
...
\factoidbox{Let $v$ be a valuation. Then
	\begin{enumerate}
		\item An atomic sentence $X$ is true relative to $v$, if and only if $v$ assigns the value T to $X$.
...
\factoidbox{\label{def:TFLvalid}A TFL-argument is valid if and only if there is no valuation on which all the premises are true but the conclusion is false. 
...
\factoidbox{A TFL-argument $X_1,\ldots,X_n\therefore Y$ is \define{valid} if and only if there is no valuation $v$ such that all the premises, $X_1,\ldots,X_n$ are true relative to $v$ but $Y$ is false relative to $v$.}
