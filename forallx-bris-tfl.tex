...
The important feature of truth functional logic is that the truth value of a sentence such as `$A\eand B$' is determined entirely by the truth value of `$A$' and the truth value of `$B$'. 
...
\begin{center}
	\begin{tabular}{rcc|c|def}
		&&&&$E$&$\mainconnective{\eif}$&$B$\\
		&$B$&$E$&$B\eif E$&$E$&&$B$\\\hline
		$v_1$&T&T&T&T&&T\\
		$v_2$&T&F&F&F&&T\\
		$v_3$&F&T&T&T&&F\\
		$v_4$&F&F&T&F&&T
...
Arguments which we are evaluating might make use of more complex sentences which do not have such simple logical forms but instead which use multiple connectives stacked on top of one another. For example, they might involve the following sentences. 
\begin{samplesent}
...
The definition specifies rules according to which sentences of the language can be formed. 


\section{A first example: introducing formation trees and the main connective}
When given a TFL sentence, say $\enot (A\eand B)$, one can work out the order that the connectives were applied. We call this a \define{formation tree}. 
\begin{center}
	\begin{forest}
		[$\mainconnective{\enot} (A\eand B)$
	...
	\end{forest}
\end{center}
$A$ and $B$ are both sentences of TFL according to our rule for atomic sentences; then $(A\eand B)$ is a sentence of TFL, according to the rule for $\eand$; then $\enot(A\eand B)$ is a sentence of TFL according to the rule for $\enot$. 

This formation tree tells us in what order to read and apply the connectives. In fact, for every sentence of TFL, we will always be able to uniquely determine the order in which the connectives were introduced and thus the order that we need to calculate the truth value in. 
We can link to English again:
\begin{center}
	\begin{forest}
		[$\mainconnective{\enot} (A\eand B)$\\It is not the case that \ldots
		[$( A\mainconnective{\eand}B)$\\\ldots and \ldots
...
When drawing these trees we have highlighted a particular connective on each of our nodes. We call that connective the \define{main connective} of the sentence at that node.
...
This will then be important to know in what order to calculate the truth value of $\enot(A\eand B)$. 

Whether $A\eand B$ is true depends on whether Alice is happy and whether Beth is happy.
We said that there are four possibilities: 
\begin{center}
	\begin{tabular}{rcc|c}
	&	$A$&$B$&$A\eand B$\\\hline
	$v_1$&	T&T&T\\
...
What about $\enot(A\eand B)$? This will make use of our truth rule for its main connective, $\enot$. The only difference now is that it is applied not to an atomic sentence but something that is more complicated. Recall the truth rule for $\enot$: 
\begin{center}
	\begin{tabular}{ccc}
	If \metaX is:&&then $\enot\metaX$ is:\\
	T &$\leadsto$& F\\
...
So suppose we are in a case where neither Alice nor Beth are happy ($v_4$). Then $(A\eand B)$ is false. So $\enot(A\eand B)$ is true. To keep track of this reasoning in the four possibilities, we write the truth value of the sentence underneath its main connective, in this case, $\enot$. That is, in $v_1$, $\enot (A\eand B)$ is false, and in $v_2$, $v_3$ and $v_4$ it is true. 
\begin{center}
	\begin{tabular}{rcc|df}
		&&&\tikzmarknode{r2_not}{$\mainconnective{\enot}$}&$(A\eand B)$\\[0.8em]
		&	$A$&$B$&&$(A\,\tikzmarknode{r2_and}{\eand} \, B)$\\\hline\Tstrut
		$v_1$&	T&T&\TTbf{F}&T\\
		$v_2$&	T&F&\TTbf{T}&F\\
		$v_3$&	F&T&\TTbf{T}&F\\
		$v_4$&	F&F&\TTbf{T}&F
	\end{tabular}
		...
		% lower right conjunction -> negation
		\draw[arrowformationtree] (r2_and) to[bend left=18] (r2_not);
	\end{tikzpicture}
\end{center}
...
Now consider 
$$(A\eor B)\eand \enot(A\eand B)$$
This sentence was formed as follows:
\begin{center}
	\begin{forest}
		[$(A\eor B)\mainconnective{\eand} \enot(A\eand B)$
		[$(A\mainconnective{\eor} B)$[$A$][$B$]]
		[$\mainconnective{\enot}(A\eand B)$
		[$(A\mainconnective{\eand} B)$[$A$][$B$]]
		]
				]
	\end{forest}
\end{center}

With some English reminders it is:
\begin{center}
	\begin{forest}
		[$(A\eor B)\mainconnective{\eand} \enot(A\eand B)$\\\ldots and \ldots
		[$(A\mainconnective{\eor} B)$\\\ldots or \ldots[$A$\\Alice is happy][$B$\\Beth is happy]]
		[$\mainconnective{\enot}(A\eand B)$\\It is not the case that \ldots
		[$(A\mainconnective{\eand} B)$\\\ldots and \ldots [$A$\\Alice is happy][$B$\\Beth is happy]]
		]
		]
	\end{forest}
\end{center}
...
Whether it is true or false depends on whether Alice is happy and whether Beth is happy. This is specified precisely in our formal language and that is what explains why our sentence of logic, $(A\eor B)\eand \enot(A\eand B)$ is an appropriate symbolisation of this sentence.

The truth value for sentences of TFL are determined by applying the truth rules for the relevant connectives to the constituent parts. 

So the overall truth value for $(A\eor B)\eand \enot(A\eand B)$ will be determined according to the rule for $\eand$, depending on the truth values of $(A\eor B)$ and $\enot(A\eand B)$. The truth value for $A\eor B$ is simply determined by the truth values for $A$ and for $B$ using the rule for $\eor$; and the truth value for $\enot(A\eand B)$ also depends just on the truth value for $A$ and for $B$, as we've just calculated. 

Recall the truth rule for $\eand$:
\begin{center}
	\begin{tabular}{cccc}
		If \metaX is:&and \metaY is:&&then $\metaX\eand\metaY$ is:\\
		T & T &$\leadsto$& T\\
...
We now apply this truth rule to the values calculated for $(A\eor B)$ along with the value calculated for $\enot(A\eand B)$. 

 In $v_2$ and $v_3$ where just one of them is happy, then $(A\eor B)$ is true, and so is $\enot(A\eand B)$, and thus, $(A\eor B)\eand\enot(A\eand B)$ is true. 

 
In $v_1$ where they are both happy, then $\enot(A\eand B)$ is false, so the overall sentence $(A\eor B)\eand\enot(A\eand B)$  is false.

In $v_4$, where neither of them is happy, then $(A\eor B)$ is false, so also $(A\eor B)\eand\enot(A\eand B)$  is false. 

This is described more carefully in the following table: 


%\begin{center}
%	\begin{tabular}{rcc|deeeeeef}
%		&&&$(A$ &$\eor$&$B)$&$\mainconnective{\eand}$& $\enot$& $(A$ & $\eand$ & $B)$\\
%		&&&$(A$&{$\eor$} &$B)$&& $\mainconnective{\enot}$& $(A$ & $\eand$ & $B)$\\
%	&	$A$ & $B$ & &&&& & $(A$ & $\eand$ & $B)$\\\hline
%	$v_1$&	T & T &   & T &   & {F} & F &   & T &  \\
%	$v_2$&	T & F &   & T &   & {T} & T &   & F &  \\
%	$v_3$&	F & T &   & T &   & {T} & T &   & F &  \\
...
\begin{center}
	\begin{tabular}{rcc|deeeeeef}
		&&&
		$(A$ & \tikzmarknode{r1_or}{$\eor$} & $B)$ &
		\tikzmarknode{r1_and}{$\mainconnective{\eand}$} &
		\tikzmarknode{r1_not}{$\enot$} &
		$(A$ & $\eand$ & $B)$ \\[0.8em]
		
		&&&
		$(A$ & {\tikzmarknode{r2_or}{$\eor$}} & $B)$ &&
		\multicolumn{1}{c}{\tikzmarknode{r2_not}{$\mainconnective{\enot}$}} &
		$(A$ &$\eand$& $B)$ \\[0.8em]
		
		& $A$ & $B$ & &&&&& $(A$ &  \tikzmarknode{r2_and}{$\eand$} & $B)$\\\hline
		
		$v_1$& T & T &   & T &   & \TTbf{F} & F &   & T &  \\
		$v_2$& T & F &   & T &   & \TTbf{T} & T &   & F &  \\
		$v_3$& F & T &   & T &   & \TTbf{T} & T &   & F &  \\
		$v_4$& F & F &   & F &   & \TTbf{F} & T &   & F &  \\
	\end{tabular}
	...
	\begin{tikzpicture}[remember picture, overlay]
		...
		% row 2 -> row 1
		\draw[arrowformationtree] (r2_or)  to[bend left=18]  (r1_and);
		\draw[arrowformationtree] (r2_not) to[bend right=18] (r1_and);
...
\section{An example with more atomic sentences}
Consider
	$$(A\eand B)\eor C$$
Its main connective is $\eor$. It has the form $\metaX\eor\metaY$, with $\metaX$ as `$(A\eand B)$' and $\metaY$ as `$C$'.
	\begin{center}
		\begin{forest}
...
With some English guidance this can be understood as:
		\begin{center}
		\begin{forest}
			[$(A\eand B)\mainconnective{\eor} C$\\\ldots or \ldots
			[$(A\mainconnective{\eand} B)$\\\ldots and \ldots[$A$\\Alice is happy][$B$\\Beth is happy]]
			[$C$\\Charlie is happy]
			]
		\end{forest}
...
\end{samplesent}
	...
	Now, to determine the truth value of $(A\eand B)\eor C$ we will need to apply the rule for $\eor$ combining the truth value for $A\eand B$ with the truth value for $C$. 
	Whether $A\eand B$ is true or false depends on the value of $A$ and that of $B$. So whether $(A\eand B)\eor C$ is true or false will depend on the value of $A$, $B$ and $C$. 
	Whether 
	\begin{samplesent}
		\item[\ref{sent:andor}.]  Alice and Beth are both happy, or Charlie is happy
	\end{samplesent}
	is true or false depends on whether Alice is happy, whether Beth is happy and whether Charlie is happy. 
	Now there are eight possibilities. There are eight possible combinations of whether these three people are happy or not. 
	
	The corresponding truth table is:
	\begin{center}
		\begin{tabular}{rccc|deeef}
			&&&&
			$(A$ & $\eand$ & $B)$ & \tikzmarknode{r3_or}{$\mainconnective{\eor}$} & $C$ \\[0.8em]
			
	& $A$ & $B$ & $C$ &
			$(A$ &\tikzmarknode{r4_and}{$\mainconnective{\eand}$} & $B)$ &&\tikzmarknode{r4_C}{${C}$} \\\hline\Tstrut
			
			
			$v_1$ & T & T & T &   & T &   & \TTbf{T} & T \\
			$v_2$ & T & T & F &   & T &   & \TTbf{T} & F \\
			$v_3$ & T & F & T &   & F &   & \TTbf{T} &  T\\
			$v_4$ & T & F & F &   & F &   & \TTbf{F} &  F\\
			$v_5$ & F & T & T &   & F &   & \TTbf{T} &  T\\
			$v_6$ & F & T & F &   & F &   & \TTbf{F} &  F\\
			$v_7$ & F & F & T &   & F &   & \TTbf{T} & T \\
			$v_8$ & F & F & F &   & F &   & \TTbf{F} &  F\\
		\end{tabular}
		...
		\begin{tikzpicture}[remember picture, overlay]
			...
			\draw[arrowformationtree] (r4_and) to[bend left=18] (r3_or);
			\draw[arrowformationtree] (r4_C) to[bend right=18] (r3_or);
		\end{tikzpicture}
	\end{center}
	
	\section{No brackets with negation}
	
	Consider $$A\eif \enot B$$
	

	Its formation tree is: 
	\begin{center}
		\begin{forest}
			[$A\mainconnective{\eif}\enot B$
			[$A$][$\mainconnective{\enot}B$[$B$]]]
		\end{forest}
	\end{center}
	
	Which we can add some guiding English to as:
		\begin{center}
		\begin{forest}
			[$A\mainconnective{\eif}\enot B$\\If\ldots then \ldots
			[$A$\\Alice is happy]
			[$\mainconnective{\enot}B$\\It is not the case that \ldots[$B$\\Beth is happy]]]
		\end{forest}
	\end{center}
	
		This would be an appropriate symbolisation of:
	\begin{samplesent}
		\item If Alice is happy, then Beth is not happy.
	\end{samplesent}
	...
	The corresponding truth table is:
	\begin{center}
		\begin{tabular}{rcc|deeef}
			&&&
			$A$ & \tikzmarknode{r1_if}{$\mainconnective{\eif}$} & \tikzmarknode{r1_not}{$\enot$} & $B$ \\[0.8em]
			
			& $A$ &&\tikzmarknode{r2_A}{{$A$}}&& \tikzmarknode{r2_not}{${\enot}$} &$B$  \\\hline\Tstrut
		...
		\begin{tikzpicture}[remember picture, overlay]
			...
			\draw[arrowformationtree] (r2_not) to[bend left=18] (r1_if);
			\draw[arrowformationtree] (r2_A) to[bend left=28] (r1_if);
		\end{tikzpicture}
	\end{center}
	
	
		You might notice here that we have not got any brackets in the sentence. You might be tempted to write:
	
	$$A\eif(\enot B)\qquad\leftsquigarrow \text{wrong!}$$
	
	That will not be a sentence of TFL. Negations do not introduce brackets.
	
	
	\section{Let's get careful about brackets}

	We use brackets to identify how to read the sentence, but where they should be used is officially determined according to our precise rules for what counts as a sentence of TFL.
...
\begin{center}
	\begin{forest}
		[$\mainconnective{\enot} (A\eand B)$
		[$\mainconnectivecolor{(} A\mainconnective{\eand}B\mainconnectivecolor{)}$
		[$A$][$B$]
		]
		]
	\end{forest}
...
What about $A\eif\enot B$. We start with the atomic sentence $B$. Then we add a negation symbol in front of it to get $\enot B$ -- no brackets included. Then we combine it with $A$ to get  $(A\eif\enot B)$
	\begin{center}
	\begin{forest}
		[$\mainconnectivecolor{(}A\mainconnective{\eif}\enot B\mainconnectivecolor{)}$
		[$A$][$\mainconnective{\enot}B$[$B$]]]
	\end{forest}
\end{center}

This, however, gives us $(A\eif\enot B)$ rather than what we wrote, which was $A\eif\enot B$. It has brackets. 
Many of the sentences we have written so far are not strictly speaking sentences of TFL. Even $A\eand B$ isn't, strictly speaking, a sentence of TFL. Strictly speaking, only $(A\eand B)$ is a sentence of TFL. 
...
In our formation trees, we display this invisible ink in gray. When the sentence is written by itself, it can be dropped, but when it is included in another sentence, those brackets now need to be written. 

\begin{center}
	\begin{forest}
		[$\almostinvisibleopenbracket (A\eand B)\mainconnective{\eor} C\almostinvisibleclosebracket $
		[$\almostinvisibleopenbracket A\mainconnective{\eand} B\almostinvisibleclosebracket $[$A$][$B$]]
		[$C$]
		]
	\end{forest}
\end{center}

So we can write $$(A\eand B)\eor C$$ dropping the outer brackets from the strictly speaking formation $$((A\eand B)\eor C)$$ But we cannot drop the brackets in the inner `$(A\eand B)$'. This would result in $$A\eand B\eor C$$ which would be dangerously ambiguous.



\subsection{Negation and connectives}
An important example is $$\enot A\eor B$$ The main connective of the sentence is $\eor$. It 
has the following formation tree: 
\begin{center}
	\begin{forest}
		[$\almostinvisibleopenbracket\enot A\mainconnective{\eor}B\almostinvisibleclosebracket$
...
We can see this because it can be obtained from dropping the outer brackets from the sentence $$\textcolor{gray}{(}\enot A\eor B\textcolor{gray}{)}$$ 
...
Similarly $$\enot(A\eand B)\eif C$$
Here, $\eif$ is the main connective. It has the following formation tree: 
\begin{center}
	\begin{forest}
		[$\enot (A\eand B)\mainconnective{\eif}C$
		[$\mainconnective{\enot} (A\eand B)$[$\almostinvisibleopenbracket A\mainconnective{\eand} B\almostinvisibleclosebracket$[$A$][$B$]]]
		[$C$]
		]
	\end{forest}
\end{center}

One more example: consider $$\enot (\enot (\enot Q\eif (R\eand S))\eand \enot \enot (P\eor Q))$$ This has the formation tree:
\begin{center}
	\begin{forest}
		[{$\mainconnective{\enot} (\enot (\enot Q\eif (R\eand S))\eand \enot \enot (P\eor Q))$}
			[{$\almostinvisibleopenbracket\enot (\enot Q\eif (R\eand S))\mainconnective{\eand} \enot \enot (P\eor Q)\almostinvisibleclosebracket$}
				[{$\mainconnective{\enot} (\enot Q\eif (R\eand S))$}
					[{$\almostinvisibleopenbracket\enot Q\mainconnective{\eif}(R\eand S)\almostinvisibleclosebracket$}
						[{$\mainconnective{\enot} Q$}
							[{$Q$}]
						]
						[{$\almostinvisibleopenbracket R\mainconnective{\eand}S\almostinvisibleclosebracket$}
							[{$R$}]
							[{$S$}]
						]
					]
				]
				[{$\mainconnective{\enot}\enot (P\eor Q)$}
					[{$\mainconnective{\enot} (P\eor Q)$}
						[{$\almostinvisibleopenbracket P\mainconnective{\eor}Q\almostinvisibleclosebracket$}
							[{$P$}]
							[{$Q$}]
						]
					]
...
To add to our notion of  \gls{main connective}, we introduce the notion of \gls{scope}. 
...
In the formation tree of a sentence, the scope of a connective is the sentence at the node where it is the main connective. It's another way to highlight the order in which to apply the truth rules when determining the truth value of the sentence. 



For example, consider the sentence
\[
\almostinvisibleopenbracket(P \eand Q)\eor (\enot R \eif S))\almostinvisibleclosebracket
\]

Its formation tree is:
\begin{center}
	\begin{forest}
		[$\almostinvisibleopenbracket(P \eand Q){\customcoloredbox[yellow]{\eor}} (\enot R \eif S))\almostinvisibleclosebracket$,tikz={\node [draw,inner sep=7pt,yellow,fit to=tree,thick,rounded corners]{};}
		[$\almostinvisibleopenbracket P{\customcoloredbox[blue!20]{\eand}}  Q\almostinvisibleclosebracket$,tikz={\node [draw,inner sep=0pt,blue,fit to=tree,thick,rounded corners]{};}[$P$][$Q$]]
		[$\almostinvisibleopenbracket\enot R {\customcoloredbox[red!20]{\eif}}  S\almostinvisibleclosebracket$,tikz={\node [draw,inner sep=3pt,red,fit to=tree,thick,rounded corners]{};}
		[${\customcoloredbox[teal!20]{\enot}}R$,tikz={\node [draw,inner sep=0pt,teal,fit to=tree,thick,rounded corners]{};}
		[$R$]]
		[$S$]
		]
		]
...
The scopes are:
\[\overbrace{\almostinvisibleopenbracket \underbrace{(P \eand Q)}_{\textcolor{blue}{\text{scope of }\eand}}\eor \underbrace{(\overbrace{\enot R}^{\textcolor{teal}{\text{scope of }\enot}}\eif S)}_{\textcolor{red}{\text{scope of }\eif}}\almostinvisibleclosebracket}^{\textcolor{yellow!50!orange}{\text{scope of }\eor}}
\]

In this sentence, the scope of $\eor$ is the whole sentence, as it is the main connective of that whole sentence: it was the connective which was introduced last.  The scope of $\eand$ is $\almostinvisibleopenbracket P \eand Q\almostinvisibleclosebracket$, the scope of $\eif$ is $\almostinvisibleopenbracket\enot R \eif S\almostinvisibleclosebracket$, and the scope of $\enot$ is $\enot R$. 


The important feature of scope is that when we will determine the truth value of the sentence, it is only the parts underneath that node which are relevant in the truth value of the sentence at that node. So the scope describes the parts of the tree which are relevant for determining the truth value of that connective. 
...
\begin{center}
	\begin{minipage}{0.45\textwidth}
		\centering
		\[
		\overbrace{\underbrace{A\eand B}_{\text{scope of $\eand$}}\eif C}^{\text{scope of $\eif$}}
		\]
	\end{minipage}
	\quad
		\begin{minipage}{0.45\textwidth}
...
\begin{center}
	\begin{minipage}{0.45\textwidth}
		\centering
		\[
		\overbrace{\underbrace{A\eand B}_{\text{scope of $\eand$}}\eif \underbrace{C\eor D}_{\text{scope of $\eor$}}}^{\text{scope of $\eif$}}
		\]
	\end{minipage}
	\quad
		\begin{minipage}{0.45\textwidth}
		\centering
		\begin{forest}
			[$A\eand B\mainconnective{\eif} C\eor D$
			[$A\mainconnective{\eand}B$[$A$][$B$]]
			[$C\mainconnective{\eor}D$[$C$][$D$]]
			]
		\end{forest}
	\end{minipage}
\end{center}
...
We make one final comment about the syntax of TFL: a reflection on the kind of way we have defined what counts as a sentence of TFL
...
In \cref{sec:Truth rules} we gave truth rules for the connectives of TFL. Now we can apply them to complex sentences. 

The truth rules associated with the connectives allow us to calculate the truth value of a complex sentences based on the truth values of its constituent parts. These may themselves be complex sentences. Ultimately we get to the atomic sentences and have to ask whether they are true or false. 
A valuation is a specification of whether each atomic sentence is true or false.
...
If you give me a valuation, I can then use the rules to calculate the truth value of any sentence. 
Sentences of TFL are true or false \emph{relative to a valuation}. 
The valuation determines the truth values of the atomic sentences, and complex sentences are determined using the truth rules:
\begin{definition}Let $v$ be a valuation. Then
	\begin{enumerate}
		\item An atomic sentence $X$ is true relative to $v$, if and only if $v$ assigns the value T to $X$.
		\item For a sentence $\enot\metaX$, $(\metaX\eand\metaY)$, $(\metaX\eor\metaY)$, $(\metaX\eif\metaY)$, whether it is true or false relative to $v$ can be determined by applying the appropriate truth rule depending on whether $\metaX$ and $\metaY$ are true or false relative to $v$:
%			\begin{tabular}{@{If \metaX is }c@{ relative to $v$, then $\enot\metaX$ is }c@{ relative to $v$}}
%				true &false\\
%				false &true
%			\end{tabular}
...
`$(\enot I\eand H)\eif H$'. 
The formation tree will help us know what to do (see \cref{def:TFLsentences}):
\begin{center}
	\begin{forest}
...
We will present this in a table:

\begin{center}
	\begin{tabular}{r c c| c c c c c c c c c}
		&&
		
		& {(} & {\enot} & $I$ & {\eand} & $H$ & {)} & \tikzmarknode{r1_imp}{$\mainconnective{\eif}$}& $H$  \\[.2em]
		...
		& \mainconnectivecolor{)} & &
		\tikzmarknode{r2_Hr}{$H$} & \\[.2em]
		
		% Row 3: children of Row 2 (left side)
		&& & & \tikzmarknode{r3_not}{$\mainconnective{\enot}$} & $I$ & & \tikzmarknode{r3_Hl}{$H$} & & & \\[.2em]
		
		% Row 4: child of Row 3 (for the negation)
		Valuation & $I$ & $H$ & & & \tikzmarknode{r4_I}{$I$} & & & && & \\[.2em]\hline
...
%$v_4:$ & F & F & & T & F & F & F & & \TTbf{T} & F \\
	...
	\begin{tikzpicture}[remember picture,overlay]
		...
		% Row 2 -> Row 1
		\draw[arrowformationtree] (r2_and) to[bend right=18] (r1_imp);
		\draw[arrowformationtree] (r2_Hr)  to[bend left=18] (r1_imp);
		
		% Row 3 -> Row 2
		\draw[arrowformationtree] (r3_not) to[bend right=18] (r2_and);
		\draw[arrowformationtree] (r3_Hl)  to[bend left=18] (r2_and);
...
%		$v_3:$ & T & F & T & \TTbf{F} & & F & T & & T & T & & T & T & F & T & & & \\
		$v:$ & T & F & F & \TTbf{F} & & F & T & & F & T & & T & T & T & F & & & \\
		$v':$ & F & T & T & \TTbf{F} & & T & T & & T & F & & F & F & F & T & & & \\
...
%		$v_8:$ & F & F & F & \TTbf{F} & & F & T & & F & T & & F & F & T & F & & & \\
	...
	\begin{tikzpicture}[remember picture,overlay]
		...
		% Row 3 -> Row 2
		\draw[arrowformationtree] (r3_C)        to[bend right=18] (r2_bigor);     
		\draw[arrowformationtree] (r3_implies) to[bend left=18] (r2_bigor);  
		
		% Row 4 -> Row 3
		\draw[arrowformationtree] (r4_D)       to[bend right=18] (r3_implies);    
		\draw[arrowformationtree] (r4_inneror) to[bend left=18] (r3_implies);   
		
		% Row 5 -> Row 4
		\draw[arrowformationtree] (r5_A)   to[bend right=18] (r4_inneror);         
		\draw[arrowformationtree] (r5_not) to[bend left=18] (r4_inneror);        
...
\begin{definition}\label{def:TFLvalid}A TFL-argument is valid if and only if there is no valuation on which all the premises are true but the conclusion is false. 
	
	That is, `$X_1,\ldots,X_n\therefore Y$' is \define{valid} if and only if there is no valuation $v$ such that $X_1$ is true relative to $v$ and $X_2$ is true relative to $v$, and so on, up to $X_n$ being true relative to $v$, but $Y$ is false relative to $v$.\end{definition}
...
We have already shown how to do this on a particular valuation: one first writes down the formation tree of the sentence. (see \cref{def:TFLsentences}):
\begin{center}
	\begin{forest}
		[$(\enot I\eand H)\mainconnective{\eif} H$
		[$(\enot I\mainconnective{\eand} H)$
...
We will record the truth value of each sub-sentence underneath its main connective. 
So we will somewhat recreate the formation tree in the header of the truth table. 
\begin{center}
	\begin{tabular}{r c c| d e e ee e e f}
\multirow{4}{*}{Valuation}	&	\multirow{4}{*}{$H$}&\multirow{4}{*}{$I$}	& $($ & $\enot$ & $I$ & $\eand$ & $H$ & $)$ & \tikzmarknode{tt1_imp}{$\mainconnective{\eif}$} & $H$ \\[.4em]
	&	& & $\mainconnectivecolor{(}$ & $\enot$ & $I$ & \tikzmarknode{tt1_and}{$\mainconnective{\eand}$} & $H$ & $\mainconnectivecolor{)}$ & &\tikzmarknode{tt1_Hr}{$H$} \\[.4em]
	&	& & & \tikzmarknode{tt1_not}{$\mainconnective{\enot}$} & $I$  & & \tikzmarknode{tt1_Hl}{$H$} & & &\\[.4em]
	&	& & & & \tikzmarknode{tt1_I}{$I$}& & & & &  \\\hline 
	$v_1:$&T&T&\\
	$v_2:$&T&F&\\
	$v_3:$&F&T\\
	$v_4:$&F&F
...
\begin{tikzpicture}[remember picture,overlay]
	\tikzset{arrowformationtree/.style={-{Stealth[length=2.2mm,width=1.6mm]},gray,opacity=0.6}}
	\draw[arrowformationtree] (tt1_and) to[bend right=18] (tt1_imp);
	\draw[arrowformationtree] (tt1_Hr)  to[bend left=18] (tt1_imp);
	\draw[arrowformationtree] (tt1_not) to[bend right=18] (tt1_and);
	\draw[arrowformationtree] (tt1_Hl)  to[bend left=18] (tt1_and);
	\draw[arrowformationtree] (tt1_I)   to[bend left=18] (tt1_not);
\end{tikzpicture}
...
\begin{center}
	\begin{tabular}{r c c| d e e ee e e fc}
		\multirow{4}{*}{Valuation}	&	\multirow{4}{*}{$H$}&\multirow{4}{*}{$I$}	& $($ & $\enot$ & $I$ & $\eand$ & $H$ & $)$ & \tikzmarknode{tt2_imp}{$\mainconnective{\eif}$} & $H$ \\[.4em]
		&	& & $\mainconnectivecolor{(}$ & $\enot$ & $I$ & \tikzmarknode{tt2_and}{$\mainconnective{\eand}$} & $H$ & $\mainconnectivecolor{)}$ & &\tikzmarknode{tt2_Hr}{$H$} \\[.4em]
		&	& & & \tikzmarknode{tt2_not}{$\mainconnective{\enot}$} & $I$  & & \tikzmarknode{tt2_Hl}{$H$} & & &\\[.4em]
		&	& & & & \tikzmarknode{tt2_I}{$I$}& & & & & &$\star$ \\\hline 
		$v_1:$&T&T& & &T& & & & &\\
		$v_2:$&T&F& & &F& & & & &\\
		$v_3:$&F&T& & &T& & & & &\\
		$v_4:$&F&F& & &F& & & & &\\
...
\begin{tikzpicture}[remember picture,overlay]
	\tikzset{arrowformationtree/.style={-{Stealth[length=2.2mm,width=1.6mm]},gray,opacity=0.6}}
	\draw[arrowformationtree] (tt2_and) to[bend right=18] (tt2_imp);
	\draw[arrowformationtree] (tt2_Hr)  to[bend left=18] (tt2_imp);
	\draw[arrowformationtree] (tt2_not) to[bend right=18] (tt2_and);
	\draw[arrowformationtree] (tt2_Hl)  to[bend left=18] (tt2_and);
	\draw[arrowformationtree] (tt2_I)   to[bend left=18] (tt2_not);
\end{tikzpicture}
...
\begin{center}
	\begin{tabular}{r c c| d e e ee e e f c}
		\multirow{4}{*}{Valuation}	&	\multirow{4}{*}{$H$}&\multirow{4}{*}{$I$}	& $($ & $\enot$ & $I$ & $\eand$ & $H$ & $)$ & \tikzmarknode{tt3_imp}{$\mainconnective{\eif}$} & $H$ &  \\[.4em]
		&	& & $\mainconnectivecolor{(}$ & $\enot$ & $I$ & \tikzmarknode{tt3_and}{$\mainconnective{\eand}$} & $H$ & $\mainconnectivecolor{)}$ & &\tikzmarknode{tt3_Hr}{$H$} & \\[.4em]
		&	& & & \tikzmarknode{tt3_not}{$\mainconnective{\enot}$} & $I$  & & \tikzmarknode{tt3_Hl}{$H$} & & & & $\star$ \\[.4em]
		&	& & & & \tikzmarknode{tt3_I}{$I$}& & & & &  & \\\hline 
		$v_1:$&T&T& & &T& &T& & & &\\
		$v_2:$&T&F& & &F& &T& & & &\\
		$v_3:$&F&T& & &T& &F& & & &\\
		$v_4:$&F&F& & &F& &F& & & &\\
...
\begin{tikzpicture}[remember picture,overlay]
	\tikzset{arrowformationtree/.style={-{Stealth[length=2.2mm,width=1.6mm]},gray,opacity=0.6}}
	\draw[arrowformationtree] (tt3_and) to[bend right=18] (tt3_imp);
	\draw[arrowformationtree] (tt3_Hr)  to[bend left=18] (tt3_imp);
	\draw[arrowformationtree] (tt3_not) to[bend right=18] (tt3_and);
	\draw[arrowformationtree] (tt3_Hl)  to[bend left=18] (tt3_and);
	\draw[arrowformationtree] (tt3_I)   to[bend left=18] (tt3_not);
\end{tikzpicture}
...
\begin{center}
	\begin{tabular}{r c c| d e e ee e e fc}
		\multirow{4}{*}{Valuation}	&	\multirow{4}{*}{$H$}&\multirow{4}{*}{$I$}	& $($ & $\enot$ & $I$ & $\eand$ & $H$ & $)$ & \tikzmarknode{tt4_imp}{$\mainconnective{\eif}$} & $H$ \\[.4em]
		&	& & $\mainconnectivecolor{(}$ & $\enot$ & $I$ & \tikzmarknode{tt4_and}{$\mainconnective{\eand}$} & $H$ & $\mainconnectivecolor{)}$ & &\tikzmarknode{tt4_Hr}{$H$} \\[.4em]
		&	& & & \tikzmarknode{tt4_not}{$\mainconnective{\enot}$} & $I$  & & \tikzmarknode{tt4_Hl}{$H$} & & &&$\star$\\[.4em]
		&	& & & & \tikzmarknode{tt4_I}{$I$}& & & & &  \\\hline 
		$v_1:$&T&T& &F&T& &T& & &\\
		$v_2:$&T&F& &T&F& &T& & &\\
		$v_3:$&F&T& &F&T& &F& & &\\
		$v_4:$&F&F& &T&F& &F& & &\\
...
\begin{tikzpicture}[remember picture,overlay]
	\tikzset{arrowformationtree/.style={-{Stealth[length=2.2mm,width=1.6mm]},gray,opacity=0.6}}
	\draw[arrowformationtree] (tt4_and) to[bend right=18] (tt4_imp);
	\draw[arrowformationtree] (tt4_Hr)  to[bend left=18] (tt4_imp);
	\draw[arrowformationtree] (tt4_not) to[bend right=18] (tt4_and);
	\draw[arrowformationtree] (tt4_Hl)  to[bend left=18] (tt4_and);
	\draw[arrowformationtree] (tt4_I)   to[bend left=18] (tt4_not);
\end{tikzpicture}
...
\begin{center}
	\begin{tabular}{r c c| d e e ee e e f c}
		\multirow{4}{*}{Valuation}	&	\multirow{4}{*}{$H$}&\multirow{4}{*}{$I$}	& $($ & $\enot$ & $I$ & $\eand$ & $H$ & $)$ & \tikzmarknode{tt5_imp}{$\mainconnective{\eif}$} & $H$ \\[.4em]
		&	& & $\mainconnectivecolor{(}$ & $\enot$ & $I$ & \tikzmarknode{tt5_and}{$\mainconnective{\eand}$} & $H$ & $\mainconnectivecolor{)}$ & &\tikzmarknode{tt5_Hr}{$H$}&$\star$ \\[.4em]
		&	& & & \tikzmarknode{tt5_not}{$\mainconnective{\enot}$} & $I$  & & \tikzmarknode{tt5_Hl}{$H$} & & &\\[.4em]
		&	& & & & \tikzmarknode{tt5_I}{$I$}& & & & &  \\\hline 
		$v_1:$&T&T& &F&T&F&T& &&T\\
		$v_2:$&T&F& &T&F&T&T& &&T\\
		$v_3:$&F&T& &F&T&F&F& &&F\\
		$v_4:$&F&F& &T&F&F&F& &&F\\
...
\begin{tikzpicture}[remember picture,overlay]
	\tikzset{arrowformationtree/.style={-{Stealth[length=2.2mm,width=1.6mm]},gray,opacity=0.6}}
	\draw[arrowformationtree] (tt5_and) to[bend right=18] (tt5_imp);
	\draw[arrowformationtree] (tt5_Hr)  to[bend left=18] (tt5_imp);
	\draw[arrowformationtree] (tt5_not) to[bend right=18] (tt5_and);
	\draw[arrowformationtree] (tt5_Hl)  to[bend left=18] (tt5_and);
	\draw[arrowformationtree] (tt5_I)   to[bend left=18] (tt5_not);
\end{tikzpicture}
...
\begin{center}
	\begin{tabular}{r c c| d e e ee e e fc}
		\multirow{4}{*}{Valuation}	&	\multirow{4}{*}{$H$}&\multirow{4}{*}{$I$}	& $($ & $\enot$ & $I$ & $\eand$ & $H$ & $)$ & \tikzmarknode{tt6_imp}{$\mainconnective{\eif}$} & $H$&$\star$ \\[.4em]
		&	& & $\mainconnectivecolor{(}$ & $\enot$ & $I$ & \tikzmarknode{tt6_and}{$\mainconnective{\eand}$} & $H$ & $\mainconnectivecolor{)}$ & &\tikzmarknode{tt6_Hr}{$H$} \\[.4em]
		&	& & & \tikzmarknode{tt6_not}{$\mainconnective{\enot}$} & $I$  & & \tikzmarknode{tt6_Hl}{$H$} & & &\\[.4em]
		&	& & & & \tikzmarknode{tt6_I}{$I$}& & & & &  \\\hline 
		$v_1:$&T&T& &F&T&F&T& &\TTbf{T}&T\\
...
\begin{tikzpicture}[remember picture,overlay]
	\tikzset{arrowformationtree/.style={-{Stealth[length=2.2mm,width=1.6mm]},gray,opacity=0.6}}
	\draw[arrowformationtree] (tt6_and) to[bend right=18] (tt6_imp);
	\draw[arrowformationtree] (tt6_Hr)  to[bend left=18] (tt6_imp);
	\draw[arrowformationtree] (tt6_not) to[bend right=18] (tt6_and);
	\draw[arrowformationtree] (tt6_Hl)  to[bend left=18] (tt6_and);
	\draw[arrowformationtree] (tt6_I)   to[bend left=18] (tt6_not);
\end{tikzpicture}
...
%	\begin{tabular}{r c c c| d e e e e e e e e e e e  e e f}
	...
	%		
	%		& & & & 
	%		& & $C$ & & $\mainconnectivecolor{( }$ & $D$ & $\mainconnective{\eif}$ & $( $ & $A$ & $\eor$ & ${\enot}$ & $D$ & $)$ & $\mainconnectivecolor{)}$ & \\[.2em]
	%		
	%		& & & & & & & & & $D$ & & & $A$ & $\mainconnective{\eor}$ & $\enot$& $D$ & & & \\[.2em]
	%		
	%				& & & & & & & & & & & & $A$& &$\mainconnective{\enot}$ & ${D}$ & & & \\[.2em]
	%				
	%		& & & & & & & & & & & & & & & $D$ & & & \\[.2em]\hline
	%		
	%		
...
%	&     &         &     & $B$ &          & $\mainconnectivecolor{(}$ & $\enot$ & $B$ & $\mainconnective{\eif}$ & $\enot$ & $A$ & $\mainconnectivecolor{)}$ &  \\[.2em]
...
\begin{center}
	\begin{tabular}{r c c | d e e eeeeee f | d f | d eeee e f}
		...
		& & $J$
		&& $\mainconnective{\enot}$ & $L$ & & $J$ & \\[.3em]
		&&& & & $L$ & & & $J$ & & $L$ & & 
		&& 
		&& &$L$ & &  & \\[.3em]
		\hline
		$v_1:$ & T & T &
		& F & \textcolor{gray!50}{T} & \TTbf{T} & & \textcolor{gray!50}{T} & T & \textcolor{gray!50}{T} & & &
		\TTbf{F} & \textcolor{gray!50}{T} &
		& F & \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{T} & \\
		$v_2:$ & T & F &
		& T & \textcolor{gray!50}{F} & \TTbf{T} & & \textcolor{gray!50}{T} & T & \textcolor{gray!50}{F} & & &
		\TTbf{F} & \textcolor{gray!50}{T} &
		& T & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{T} & \\
		$v_3:$ & F & T &
		& F & \textcolor{gray!50}{T} & \TTbf{T} & & \textcolor{gray!50}{F} & T & \textcolor{gray!50}{T} & & &
		\TTbf{T} & \textcolor{gray!50}{F} &
		& F & \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{F} & \\
		$v_4:$ & F & F &
		& T & \textcolor{gray!50}{F} & \TTbf{F} & & \textcolor{gray!50}{F} & F & \textcolor{gray!50}{F} & & &
		\TTbf{T} & \textcolor{gray!50}{F} &
		& T & \textcolor{gray!50}{F} & \TTbf{F} & \textcolor{gray!50}{F} & \\
	\end{tabular}
\end{center}
...
\begin{center}
	\begin{tabular}{r c c | def | c | c}
		...
		Valuation & $D$ & $V$ & $D$ & $\mainconnective{\eif}$ & $V$ & $D$  & $V$ \\[.3em]
		%		Valuation & $D$ & $V$ & $D$ & & $V$ &  & \\
		\hline
...
\begin{center}
	\begin{tabular}{r c c c | d e e e e e f | d f || d e e e f}
		...
		%		% Row 2: children of Row 1
		%		&&&
		%		& $P$ &  & $\mainconnectivecolor{(}$ & $C$ & $\mainconnective{\eor}$ & $D$ & $\mainconnectivecolor{)}$
		%		&  & $C$
		%		&  & $P$ &  & $D$ &  \\[.3em]
		\hline
		% v1
		$v_1:$ & T & T & T
		& \textcolor{gray!50}{T} & \TTbf{T} &  & \textcolor{gray!50}{T} & T & \textcolor{gray!50}{T} & 
		& \TTbf{F} & \textcolor{gray!50}{T}
		&  & \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
		% v2
		$v_2:$ & T & T & F
		& \textcolor{gray!50}{T} & \TTbf{T} &  & \textcolor{gray!50}{T} & T & \textcolor{gray!50}{F} & 
		& \TTbf{F} & \textcolor{gray!50}{T}
		&  & \textcolor{gray!50}{T} & \TTbf{F} & \textcolor{gray!50}{F} &  \\
		% v3
		$v_3:$ & T & F & T
		& \textcolor{gray!50}{T} & \TTbf{T} &  & \textcolor{gray!50}{F} & T & \textcolor{gray!50}{T} & 
		& \TTbf{T} & \textcolor{gray!50}{F}
		&  & \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
		% v4
		$v_4:$ & T & F & F
		& \textcolor{gray!50}{T} & \TTbf{F} &  & \textcolor{gray!50}{F} & F & \textcolor{gray!50}{F} & 
		& \TTbf{T} & \textcolor{gray!50}{F}
		&  & \textcolor{gray!50}{T} & \TTbf{F} & \textcolor{gray!50}{F} &  \\
		% v5
		$v_5:$ & F & T & T
		& \textcolor{gray!50}{F} & \TTbf{T} &  & \textcolor{gray!50}{T} & T & \textcolor{gray!50}{T} & 
		& \TTbf{F} & \textcolor{gray!50}{T}
		&  & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
		% v6
		$v_6:$ & F & T & F
		& \textcolor{gray!50}{F} & \TTbf{T} &  & \textcolor{gray!50}{T} & T & \textcolor{gray!50}{F} & 
		& \TTbf{F} & \textcolor{gray!50}{T}
		&  & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{F} &  \\
		% v7
		$v_7:$ & F & F & T
		& \textcolor{gray!50}{F} & \TTbf{T} &  & \textcolor{gray!50}{F} & T & \textcolor{gray!50}{T} & 
		& \TTbf{T} & \textcolor{gray!50}{F}
		&  & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
		% v8
		$v_8:$ & F & F & F
		& \textcolor{gray!50}{F} & \TTbf{T} &  & \textcolor{gray!50}{F} & F & \textcolor{gray!50}{F} & 
		& \TTbf{T} & \textcolor{gray!50}{F}
		&  & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{F} &  \\
	\end{tabular}
\end{center}
...
\begin{center}
	...
	$v_1:$ & T & T & T
	& \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{T}
	& \TTbf{T} &  & \textcolor{gray!50}{T} & F & F & \textcolor{gray!50}{T} &
	&  & F & \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
	
	$v_2:$ & T & T & F
	& \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{T}
	& \TTbf{F} &  & \textcolor{gray!50}{T} & T & T & \textcolor{gray!50}{F} &
	&  & T & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
	
	$v_3:$ & T & F & T
	& \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{F}
	& \TTbf{T} &  & \textcolor{gray!50}{F} & F & F & \textcolor{gray!50}{T} &
	&  & F & \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
	
	$v_4:$ & T & F & F
	& \textcolor{gray!50}{T} & \TTbf{T} & \textcolor{gray!50}{F}
	& \TTbf{T} &  & \textcolor{gray!50}{F} & F & T & \textcolor{gray!50}{F} &
	&  & T & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{T} &  \\
	
	$\bigstar$ $v_5:$ & F & T & T
	& \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{T}
	& \TTbf{T} &  & \textcolor{gray!50}{T} & F & F & \textcolor{gray!50}{T} &
	&  & F & \textcolor{gray!50}{T} & \TTbf{F} & \textcolor{gray!50}{F} &  \\
	
	$v_6:$ & F & T & F
	& \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{T}
	& \TTbf{F} &  & \textcolor{gray!50}{T} & T & T & \textcolor{gray!50}{F} &
	&  & T & \textcolor{gray!50}{F} & \TTbf{T} & \textcolor{gray!50}{F} &  \\
	
	$v_7:$ & F & F & T
	& \textcolor{gray!50}{F} & \TTbf{F} & \textcolor{gray!50}{F}
	& \TTbf{T} &  & \textcolor{gray!50}{F} & F & F & \textcolor{gray!50}{T} &
	&  & F & \textcolor{gray!50}{T} & \TTbf{F} & \textcolor{gray!50}{F} &  \\
