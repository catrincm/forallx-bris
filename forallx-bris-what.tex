...
The three dots in this final line can be read ``therefore''. 
%Really then we're duplicating things by also adding the word ``Therefore''. But we do this to really carefully highlight that this is the conclusion. 
If you are writing your answers up on the computer and cannot use this symbol, you can write `Therefore:'.
...
As a guideline, there are some words you can look for which are often used to indicate whether something is a premise or conclusion:
	...
	Words often used to indicate a premise:
		\begin{center}
			because, as, given that, recalling that, after all
		\end{center}
	\end{highlighted}
`Since' is also a very common and useful indicator. 
%Unlike conditionals `if-then', `since' \emph{does} express an argument. 
%
Consider	\begin{ebullet}	\item Since the first Brexit referendum was made under false pretences,
	{the voters deserve a further say on any final deal agreed with
		Brussels}.\end{ebullet}
This both gives us a premise: \begin{ebullet}	\item The first Brexit referendum was made under false pretences.\end{ebullet}
	and a conclusion:\begin{ebullet}	\item The voters deserve a further say on any final deal agreed with
			Brussels.\end{ebullet}
\begin{highlighted}
	\begin{center}
		Since \metaX, \metaY
	\end{center} indicates that \metaX is a premise, and \metaY is the conclusion. 
\end{highlighted}
...
\begin{definition}An \define{argument} consists of a collection of declarative sentences of which one is marked as the conclusion of the argument.\end{definition}
...
Some of the answers to the practice questions are in \cref{ch:answers}, but it is important to first try yourself. The answers are provided on a best-endeavours basis. There will be gaps and mistakes. Questions with answers are indicated with stars. Mistakes can be reported to \MaintainerEmailLink.
...
\end{earg}
This argument is different from the previous one in an important respect. If the premises are true, it is not guaranteed that the conclusion is also true. The premises of this argument do not rule out, by themselves, that someone other than the maid or the driver ``did it'', perhaps it was the gardener. In this second argument, the conclusion does not follow from the premises. If, like in this argument, the conclusion does not follow from the premises, we say it is \define{invalid}.
...
\begin{definition}
	...
	\end{definition}% or that $\metaX_1$, \dots, $\metaX_n$ \define{entail}~\metaY
...
\begin{earg}
	\prem Mei is either a mathematician or a botanist.
...
\todo{Go back over this section!!!}
%
%
%Consider the following argument 
%\begin{earg}
%	\prem Every January that we know about, it has rained in Bristol.
%	\conc It will rain in Bristol this coming January. 
%\end{earg}
%
%Arguments of the sort just given belong to a species of argument called \define{ampliative arguments}. This means that the conclusion goes beyond what you find in the premises. That is, the premises don't guarantee, or entail, the conclusion. They do, however, provide some support for it. These arguments are invalid. They may be good and useful, but it is important to know the difference.
%
%In particular, this argument is a so-called \define{inductive} argument. 
%
...
%But we pause here to mention some forms of ampliative argument.
%
%
%
%
%
%In this book, we will set aside (entirely) the question of what makes for a good inductive argument. Our interest is simply in sorting the (deductively/logically) valid arguments from the invalid ones.  
%
%
%This argument generalises from many instances to a new instance. Such arguments are called \define{inductive} arguments.
%
%
...
\begin{earg}
	\prem Every winter we have so far observed, it has rained in Bristol.
	\conc It will rain in Bristol this coming winter. 
\end{earg}

This argument generalises from observations about several cases to a conclusion about all cases. Such arguments are called \define{inductive} arguments. 
The argument could be made stronger by adding additional premises before drawing the conclusion: In January 2001, it rained in London; In January 2002\ldots. But, however many premises of this form we add, the argument will remain invalid. Even if it has rained in London in every January thus far, it remains possible that London will stay dry next January. The point of all this is that inductive arguments—even good inductive arguments—are not (deductively) valid. They are not watertight. The premises might make the conclusion very likely, but they don't absolutely guarantee its truth. Unlikely though it might be, it is possible for their conclusion to be false, even when all of their premises are true.
...
%It can thus be quite tricky to determine whether an argument is conceptually valid. (Logical) validity, however, we will see is quite straightforward to determine, at least once one has identified the logical form of the sentences. 
...
\end{earg}
 Invalid. It is a zoo, but not a good one. (And has a giraffe.)
...
\end{earg}
	...
	\begin{earg}
		...
		\conc Socrates is a carrot. \hfill \myanswer{Valid}
	\end{earg}
	
	\item
	\begin{earg}
		\prem Abe Lincoln was either born in Illinois or he was president.
		\prem Abe Lincoln was not president.
		\conc Abe Lincoln was born in Illinois. \hfill \myanswer{Valid}
	\end{earg}
	...
	\begin{earg}
		...
		\hfill \myanswer{Invalid \\ Abe Lincoln might die for some other reason: someone else might pull the trigger; he might die of old age.}
	\end{earg}
	
	\item
	\begin{earg}
		\prem Abe Lincoln was either from France or from Luxembourg.
		\prem Abe Lincoln was not from Luxembourg.
		\conc Abe Lincoln was from France. \hfill \myanswer{Valid}
	\end{earg}
	...
	\begin{earg}
		...
		\conc The world will not end today. \hfill \myanswer{Valid}
	\end{earg}
	...
	\begin{earg}
		...
		\hfill \myanswer{Not valid (although arguably conceptually valid. To make it valid one would need to add the premise that one cannot be both 19 years old and 87 years old at the same time.)}
	\end{earg}
\end{problemslist}
