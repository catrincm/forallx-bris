...
This rule will now allow us to carefully work through our brain teaser from \cref{s:orE}
\label{s:ExistsE}
...
We can formalise this argument as: $$Lab,\;Lbc,\;Ha,\;\enot Hc\;\therefore\;\exists x\exists y(Hx\eand (Lxy\eand\enot Hx)).$$ And we can show it is valid. In \cref{s:orE} we wrote this in a pseudo-formal style.
\begin{fitchproof}
...
\begin{fitchproof}
	...
	\have[\ ]{c}{\exists \meta{x}\metaX(\ldots \meta{x} \ldots \meta{c}\ldots)} \existsI{a}
\end{fitchproof}
Can only replace instances of $c$ which are not already bound by $\forall \meta{x}$ or $\exists \meta{x}$ in $\metaX$.
}
%All the cases we've seen in this section follow this rule.

You might have noticed the additional constraint that's added to the rule. 
%It is {part} of the rule; so if you are asked to write the rule $\exists$I  you {must} include this constraint. However, you will not need to worry about it in practice. It's simply there to guarantee that applications of the rule yield sentences of FOL.
%You shouldn't need to
 If the rule were not there we would be allowed to argue as follows:
\begin{fitchproof}
	\hypo{a}{\exists xLnx}
	\have{e}{\exists x \exists x Lxx}\by{naughtily attempting to invoke $\exists$I}{a}
\end{fitchproof}
$\exists x \exists x Lxx$ is in fact a sentence of FOL, although a confusing one. It's a sentence which should be avoided when symbolising. If one very carefully works through the definitions, it can be seen to be logically equivalent to $\exists x Lxx$. But $\exists x Lnx\therefore\exists x Lxx$ is clearly invalid. 
...
\end{fitchproof}
	...
	\meta{c} must not already be bound by $\forall \meta{x}$ or $\exists \meta{x}$ in $\metaX$.
%	\meta{x} must not occur in $\metaX(\ldots \meta{c} \ldots \meta{c}\ldots)$
	}
It is important to appreciate that to apply the rule correctly \textbf{we must replace every occurrence of the name $c$ by the variable $x$}. A crucial aspect of this rule, though, is bound up in the first constraint. This constraint ensures that we are always reasoning at a sufficiently general level (the second constraint is one which avoids unintended instances such as $\exists x Fcx\therefore \forall x\exists x Fxx$. The conclusion is logically equivalent to $\exists x Fxx$ and it is an invalid argument.) 
...
\begin{fitchproof}
	...
	\open
		...
		\have{t}{Go} \ifE{st, s}
		\have{et1}{\exists x Gx}\existsI{t}
	\close
	\have{et2}{\exists x Gx}\existsE{es,s-et1}
\end{fitchproof}\noindent
...
\begin{fitchproof}
	...
	\open
		\hypo{na}{\enot Lb}
		\have{con}{Lb \eand \enot Lb}\andI{f, na}
		\have{con1}{\exists x (Lx \eand \enot Lx)}\existsI{con}
	\close
...
In \cref{s:MoreMonadic} problem A, we considered fifteen syllogistic figures of Aristotelian logic. Provide proofs for each of the argument forms. NB: You will find it \emph{much} easier if you symbolize (for example) `No F is G' as `$\forall x (Fx \eif \enot Gx)$'.
...
In \cref{s:FOLBuildingBlocks}, we noted that $\enot\exists x\metaX$ is logically equivalent to $\forall x \enot\metaX$. We will add some rules to our proof system that govern this. In particular, we add:
	\factoidbox{
	\begin{fitchproof}
